...
for label, key in (("dual aspect (>=2 elevations)", "aspects"),
    ...
    for r in R:
        ...
        p = [r[key][i] for i, k in enumerate(r["classes"]) if k == "private"]
        if not s or not p:
            continue
        a, b = max(s), max(p)
        win += a > b
...
for n in sorted(bn):
    v = bn[n]
    a = 100 * sum(1 for r in v if any(r["priv_via_social"])) / len(v)
    tp = sum(len(r["priv"]) for r in v)
    b = 100 * sum(sum(r["priv_via_social"]) for r in v) / tp if tp else 0
