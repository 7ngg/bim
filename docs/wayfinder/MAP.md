...
**Four** artifacts have more than one claimant, and the table below is the whole
...
and `docs/research/room-area-bands.md` (**72, 76**). `data/standards/room-constraints.json` has a
**single claimant (72)** — 74, 69 and 71 all closed.
...
| `docs/spec/acceptance-bar.md` | **no claimant — 68 closed; 64, 59, 50, 42 and 26 closed before it.** ✅ **68 rewrote §11.1’s plane paragraphs and retired 19,5 % from the file** (ADR 0039): the solver reads this bar’s plane, the encoding subtracts the erosion band **per side** off `Envelope.all_faces()`, and the Plan-level cost is **14 of 273** INFEASIBLE — all fourteen the floor’s by ablation — not 19,5 %, which `project_join.planes()` produced with **no solver run**. It also discharged **70’s `KITCHEN_DINING` limb handoff**, which only this ticket could take. The escalation, the refused screen and the three steps are untouched. ⚠️ **What it decided is not shipped**: [The bar plane is derived and the solver has never run it](tickets/77-the-bar-plane-is-derived-and-the-solver-has-never-run-it.md) owns the measurement, and until it closes §11.1 describes a decision. ** ✅ **59 rewrote §11.1 and the change is the *ordering*, not the constant**: starvation is now declared **on the Plan, never on the Proposal**, because 82,0 % of Proposal-starved candidates are served once the projection sizes them — Brief-level **3,28 % → 1,64 %**. Step 1 keeps its full claim and gains its price (`POOL_DEPTH_ON_STARVATION = 16`, a mean **2,17 s** per extra member); the *"most likely to be the answer"* wording is struck. A **third enforcement site is refused** in §11.1 itself, so a ticket adding a Proposal-level screen is re-opening a decision with a published error rate. ⚠️ **The file now carries a defect it cannot fix**: `solver.py` reads a perimeter Room **3,9 %** smaller than this bar does and is therefore strictly stricter than the rule it posts — structural on a 250 mm grid. ⚠️ **§12 gains one open item and loses one**: the escalation's step 1 is closed; the **Plan-level twin of §7.6's best-of-*m* curve** is new and is `proposer.md` §2.2.9's. ✅ **50 added §3.1**, retitled §3 and struck its reason 3, rewrote §7.4 whole and moved §7's heading to *two hard rules*; a ticket touching the region/hard-floor relationship, the glazing severity or the window selection is amending a settled shape rather than filling a gap. ✅ **The stale counts this file carried are fixed at all four sites** — 40 → **43**, conformance subset 14 → **17**, hard 31 → **34**, locale 40 → **43** — so the *stale in four places* warning 20 left here is discharged. 42 added **§13**, six subsections, and moved the count at four sites; a ticket touching composition, the WC satisfying set or cl. 5.10's non-enforcement is amending a settled shape rather than filling a gap. It rewrote §7 whole and amended §3, so a ticket touching windows is amending a settled shape rather than filling a gap; §7.3 also answers §9.1's which-part question for the window rule. **The rule count moved to 36 in three places in this file and once in `brief.md`** |
| `experiments/plane-accounting/`, `docs/research/solver-formulation.md` | **77 — sole claimant; the directory is new and 68 declared the research file on resolution.** ✅ **68 wrote Part VII**, beside II.1 because it is the same identity one term further out: an area that looks like it needs a second `AddMultiplicationEquality` is affine in the one H4 already builds. ⚠️ **Part VII is a derivation and two hand-checks and says so** — build time, solve time against the 15 s cap and τ = 4, and the INFEASIBLE delta are all unmeasured, and the **cap side has never been exercised at all** because `solver.py` does not post `dim.max_area`. 77 deliberately takes **neither** `experiments/solver-toy/` (43’s) nor `experiments/warp/` (62, 65, 67), importing both read-only — the idiom `envelope-exposure/` and `h8-frontage/` already use, and the right one here because an A/B needs both arms live |
| `docs/adr/0039-…` | **no claimant — 68 created it.** The solver reads the bar plane. ⚠️ **Decision 6 is a fallback, not the decision**: if the timing measurement refuses the encoding, floors only, forward-only literals, and `dim.max_area` left to the validator — which recovers the whole false-refusal side and leaves the cap where it is today rather than making it worse. ⚠️ It **amends ADR 0001** by restating its erosion rule as a *solver* obligation rather than a validator one, and **retires ADR 0033 consequence 4’s number** without touching its decision |
