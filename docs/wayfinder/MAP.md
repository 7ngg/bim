...
| `experiments/warp/` | **new, no claimant.** ⚠️ **48 leaves it one obligation and it invalidates a headline**: `fit_warp.py:373-384` scales the Brief's targets onto the donor's *covered* area, so ADR 0018's p50 0.056 worst-room deviation is a **proportion** result with absolute area normalised away — **the warp has never been measured against a stated `target_area`**. ADR 0020 makes that measurement possible, because `interior` is now fixed before the warp runs. 23's, and like `envelope-exposure/` it imports `solver-toy` read-only and never edits it. It also reads `rectangularise/out/swiss_fit_k2.json` as a **copied-in input**, not as a claim on that directory |
...
| `docs/adr/0003-…` | **47 — sole claimant.** ⚠️ **48 hands it one correction and could not make it**: §7's *"the entrance edge is fixed before the solve"* reads as *one ring for the job*, and a per-candidate notch changes the ring's edge **count**, so it must be re-read as **one ring per candidate, fixed before that candidate's solve**. What makes that safe already exists — the entrance edge is identified **by side, never by ring index** — but the ADR does not say so. 48 held `brief.md` and left this here rather than writing blind into a claimed file |
...
| `experiments/solver-toy/geometry.py` (`EXPOSURE_PRESETS`), `experiments/envelope-exposure/` | **49 — new, sole claimant.** ⚠️ It shares `solver-toy/` with **43**, so the concurrency rule binds: not at once. Created by *H8 and the single-aspect flat*, which corrected the *measurement* (`exposure_swiss_dwellings.py`, `dataset-inventory.md` §1.5, both unclaimed) and deliberately left the *presets* alone |
...
- **Variant generation and ranking** — scoring is answered (the six soft rules are the
  ...
  **stated** Envelope can have too, which is exactly the case that currently gets none.
  ...
  makes the gap solid. ✅ **Sharpened a fourth time by *What shape an Envelope is when
