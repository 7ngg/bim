...
(**61, 63, 64**) and `experiments/warp/` (**62, 63, 64**). *(Ticket 59 closed and
...
| `docs/adr/0020-…` | **61 — sole claimant; 56 and 60 both declared it on resolution.** ⚠️ **60 hands it one correction and could not make it**: `covered ÷ interior` **0,9833** is a **bucket** number, and under the shipped gate it is **0,9853** — so the notch guarantee is worth ~**1,3 %** of `interior` rather than **1,5 %**. The argument is unchanged and 61 already holds this file for `s`, so take it in that pass. ⚠️ **61 holds the definition of `s` itself**: it is the two largest boundary-touching components and **27,5 %** of donors have three or more, so the ADR's guarantee is stated over a quantity that does not cover the whole complement. ✅ Amendment: **`interior` is the Envelope's own area** at the finished inner face and the solve domain is a **third** quantity derived from the box — the ADR wrote `box = interior/(1 − s)` and never said which plane, and a shipped rig read it the other way. ⚠️ **And the ADR's own guarantee has a precondition it does not state**: *"every candidate delivers `interior` of floor by construction"* holds only while the *realised* notch share equals the recorded `s`, which `proposer.md` §2.2.3 explicitly denies. Worth **1,5 % of `interior`**. `rules.json` sees no change, which is the second time this ADR has ended there |
