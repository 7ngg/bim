...
- **What a sheared donor costs a warped candidate, as opposed to a record.**
  ADR 0031 keeps the off-frame population and demotes it with the rank that already
  exists, on evidence measured entirely on the **donor**: `frame_residual`, cell
  agreement, worst-room IoU. Whether a sheared donor actually yields a worse
  **Plan** after the warp and the projection is unmeasured — `experiments/warp/` is
  held by *What best-of-pool is worth at production pool depth* and 46 could not
  reach into it. **Not yet a ticket because its size is 57's to determine, exactly
  as the pool-deepening patch above.** A 4–8° donor sits at the **10,6th
  percentile** of a 58–87 bucket that draws `m = 8`, so at shipped depth the
  question is moot — nothing draws these donors. But `acceptance-bar.md` §11.1
  step 1 is *deepen the pool*, and a pool deepened to clear the 3,6 % starvation
  reaches further down that rank. If 57's best-of-*m* curve flattens early this
  never becomes a ticket at all; if starvation only clears near production depth,
  it becomes one and it is sharp. ⚠️ Whatever it finds, **ADR 0031 records that the
  rank is where it lands, not the gate** — a contradiction here re-weights an
  order, it does not reopen a hard cut.

- **What deepening a starved Brief's pool costs the runtime.** `acceptance-bar.md`
  §11.1 answers the 3,6 % starvation with search rather than with a weaker rule,
  and its first step is *deepen the pool*. Each extra member is a warp plus a solve
  against a shipped 15 s budget, and the job model has no second retrieval
  round-trip. **Not yet a ticket because the question's size is unknown**: if
  *What best-of-pool is worth at production pool depth* shows the curve flat by
  m = 12, this is a config value; if starvation only clears near production depth,
  it is a re-shape of the proposer service's request. Graduates when that curve
  lands. ⚠️ Do not pre-slice it into a Runtime ticket before then — the two answers
  want different tickets.
