...
| Runtime and process split — engine / proposer service / BFF, job model, threads, JSON | settled | ⚠️ the honest end state (queue + result store) is fog, under *Persistence, accounts, hosting*. ⚠️ **And this row now carries an unpriced claim it did not make**: `acceptance-bar.md` §11.1's escalation step 1 — *deepen the pool before declaring starvation* — spends a warp plus a 15 s solve per extra candidate, and the job model has no second retrieval round-trip in it. Fog until the Source A row's best-of-m curve says whether that is three more candidates or eighty |
| DXF export | settled | — |
...
(**59, 61, 63**) and `experiments/warp/` (**62, 63**). *(The count said six and had
