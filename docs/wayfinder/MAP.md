...
| Component | | Owed by |
|---|---|---|
| Plan geometry model — Wall, WallSegment, Room/Space, integer mm, hosted Openings, wall **layer sets** | settled | ✅ its *one box per Room* premise is **weighed and reversed**: a Space is **one or two rectangles** and the Proposal decides which — ADR 0014. ⚠️ ADR 0001's erosion is untouched and is now **asserted** rather than inherited, but `acceptance-bar.md` §9's sliver *argument* is dead and replaced. ⚠️ **ADR 0001 consequence 3 is amended by ADR 0021**: its contact threshold reserved the structural opening and **nothing else** — the `+ t_int` in it is only the centreline-to-clear correction, so zero jamb and zero nib — which put it 400 mm *below* two hard rules binding the same segment. At a minimum-length contact it admitted exactly **one** door position, mid-wall, so the threshold was **specifying** the room an architect would redraw rather than merely permitting it. ✅ **The row's last open ticket is closed and ADR 0001 did not move** — [One wall weight where a real plan draws three](tickets/36-one-wall-weight-where-a-real-plan-has-three.md), ADR 0026. It had been open since *One internal thickness against a corpus with no module* closed and appeared nowhere in this table until 51 placed it, which is the UNOWNED state the table exists to catch. **All three priced shapes are refused and consequence 5 is untouched**: the 76,1 % is measured on *surveyed built* dwellings, whose three weights exist because an engineer decided the load paths first, so it is a **working-drawing** property and this engine emits a concept-stage design. **Nothing in the pipeline could draw it regardless** — ADR 0003 c3, `fit_rects.py:125` destroying per-wall thickness at conversion, and no thickness token in §2.3. The Plan draws **two** weights and says so in a general note on the sheet. ⚠️ **Shape B is REFUTED rather than unchosen, and the ticket had it backwards**: *"uniformity survives where it is load-bearing — the solve"* buys nothing, because **no hard rule binds the solve alone** — B leaves **up to 4,9 points of Σ Space** unassigned against a hard `model.no_unassigned_area`, or kills `model.space_matches_erosion` if that void is absorbed. Both hard rules stand today. ⚠️ **The gap is reclassified, not closed**, and what is left is `annotation.md`'s: two general notes individually true and jointly misleading, and a Drawing check going **12 → 13** ⚠️ **And a two-part Room is not the shape three artifacts say it is** — [A two-part Room is a T or a Z as often as it is an L](tickets/79-a-two-part-room-is-a-t-or-a-z-as-often-as-it-is-an-l.md), raised by ADR 0041. Two rectangles sharing an edge make an L, a **T**, a **Z** or a rectangle, and over 1 535 converted two-part Rooms the split is **847 / 332 / 329 / 27** — **44,8 % do not have exactly one reflex corner**, where `erosion_check.py` asserts `reflex == 1`, ADR 0014 says *"exactly one"* and `acceptance-bar.md` §9.1 repeats it. ✅ **The geometry is not the problem** — the erosion identity is re-checked pointwise at TWO reflex corners on a T and a Z, so ADR 0001 and ADR 0041's encoding stand. What is open is whether the Proposal contract restricts to an L, and what that costs conversion |
| Envelope — inner-face ring of typed edges, rect/L/U/T | settled | ✅ **`s` is the Envelope's own notch share and it never was** — [The notch is two components and a quarter of donors have more](tickets/61-the-notch-is-two-components-and-a-quarter-of-donors-have-more.md), ADR 0020's **second amendment**. The Envelope is the bbox minus **at most two** notch rectangles, so a third boundary-touching component (**27,2 %** of donors) is **inside the ring** and cannot be a notch — the cap already decided, at conversion. `s` was read off the *parts* complement and runs **+0,0191** mean above the Envelope's share, **> 2 points on 38,2 %** of donors: about **1,9 m² of invented notch** on a 90 m² dwelling, in an edge that is typed, drawn, dimensioned and exported. **Widening `s` was measured and refused** — it moves further from the object it names. ⚠️ **The box shrinks p50 1,7 % / mean 2,2 %, and it lands with ADR 0028's widened void or not at all.** ✅ **one Brief has one Envelope *area* and many Envelope *boxes*** — ADR 0020. **H8 and the notch cap are both paid, and the object is settled.** ✅ **The cap stands at two and the shape family is refused at a measured ceiling** — [The two-notch cap is now evidenced, and more notches is not the fix](tickets/47-the-two-notch-cap-is-now-evidenced.md), ADR 0003's second amendment. **The ticket was mis-posed and the measurement says so**: there is no ground truth on the generation side to be unfaithful to — `shape` left the `ResolvedBrief` and ADR 0020 derives each box from the *donor's* notch share — so the tail is a **donor-quality** fact, not an Envelope one. **Sixteen tail dwellings are inside the cap already** and still lose > 0.10; at `notches_all` = 1 the loss is identical at every k, because a notch is one *rectangle* and a complement component need not be one. The tail splits **38.2 % rectilinear / 49.5 % off-axis**, and a vertex-budget ring — the only coherent widening — tops out at **4.17 % of the corpus**, 46.3 % of which still fails at four notches. ✅ *H8 and the single-aspect flat* found the frontage crisis did not exist — the table ran on placeholder minima and the exposure distribution it was checked against **measured one room per dwelling**, not the dwelling. ⚠️ **That second defect is this row's problem, not H8's**: `flat_single_aspect` and `corpus_median` are both fitted to the wrong column — real p25 **0.51** against 0.23, real median **0.67** against 0.37 — so every Envelope number measured at either preset was measured at roughly half the real exposure. ~~including ADR 0003's notch evidence~~ — **that part was false**: the notch cap reads no exposure at all, and 49 checked rather than assumed it. ✅ **Paid** — [The exposure presets were fitted to a measurement of one room](tickets/49-the-exposure-presets-were-fitted-to-one-room.md), and it cost three published results rather than the one it expected. ADR 0018 asserted both readings four paragraphs apart, and the one everything downstream assumed costs **56.15 % of the index** to `area.invented_envelope_hard` on donor geometry alone, because the notch is a median **12.55 %** of the bbox (p10 3.13 %, p90 23.30 %). Floor is now the pool invariant and the box is derived per candidate; `rules.json` sees **no change**, which is the opposite of what the ticket expected. ✅ **`shape` leaves the `ResolvedBrief`** — a dense object had nowhere to put *unknown*, and both defaults are refused. ✅ **The stated-`shape` gate moves to notch *area share*, and the count gate was mis-labelling the whole index**: read materially (≥ 5 % of bbox) the corpus is 52.96 % `L`, not 8.72 % — the biggest win is the common case, at 6×. ⚠️ **ADR 0018's fidelity numbers are proportion, not area**: `fit_warp.py:373-384` normalises absolute area away, so **the warp has never been measured against a stated `target_area`** — now possible, and owed by `experiments/warp/`'s next holder. ✅ **ADR 0003 §7 is re-read as *one ring per candidate*, fixed before that candidate's solve** — written by *The two-notch cap is now evidenced* after two holders declined to write blind. What makes it safe is now stated: the entrance edge is identified **by side**, never by ring index. ⚠️ **"Fill the notch" is recorded and not taken** — it dissolves the stated-shape cliff and re-opens ADR 0018's monotone-warp theorem. ✅ ADR 0003's notch cap is now measured from the corpus side: 90.16 % of converted dwellings use both notches, 8.72 % one, 1.12 % none. ✅ **The non-monotonicity stands and both its numbers were wrong.** Re-run at the re-fitted presets: single-aspect fails at **6 only** (3/5 at 8), not "6, 7, 8, mostly at 9", and **n = 6 at corpus-median is 0/5 → 5/5 — that failure is gone**. *"Dead from n rooms" is still measuring the envelope n selects, not n*, and n = 6 is still the worst row across three presets because `envelope_for(6)` picks an L. ⚠️ **But six rooms is now unexplained**: at the re-fitted preset it has **5 250 mm of frontage slack**, so it is not the frontage arithmetic and nothing has identified what it is. ⚠️ `solver-formulation.md`'s *"`flat_single_aspect` is arithmetically dead from 7 rooms, and no solver is involved"* is **overturned outright** — alive by 4 000 mm at seven and by 14 500 at twelve — as is its whole `vs corpus` column, which compares against the uncorrected distribution. Both are ticket 52's to write. How an **invented** Envelope is derived is still fog, under *Variant generation and ranking*. ✅ **ADR 0003's two-notch cap is evidenced and the shape family survived the trial** — two is the knee (median envelope loss 0.161 / 0.050 / **0.018** / 0.011 / 0.010 at k = 0…4), a higher cap converts *worse* (88.0 % against 93.2 %), and the widening was refused on measurement rather than on cost. ⚠️ **What it hands on is not an Envelope question**: envelope loss is a *predictor* and a poor gate — 42.2 % of the loss tail converts faithfully anyway, 12.70 % outside it does not, and an IoU cut removes 10.09 % of the *most faithful* envelope band. The quantity is **worst-room IoU**, already in every fit record, and its population (**154 dwellings, 6.65 % of the index**) is two thirds invisible to either proxy. Owed by `proposer.md`'s next holder — index field, hard gate at 0.30, rank above it — written out in full on 47 so it is transcribed, not re-derived. ⚠️ H8 is also handed a corpus-side number: **4.1 % of façade-facing rooms lose their frontage in conversion**, 22.5 % of dwellings lose one — the only conversion fidelity figure nothing constrains. ⚠️ **And H8 now has one more room competing for frontage**: *Opening placement rules* moves `kitchen.needs_window` to **true** — three shipped places disagreed and AzDTN cl. 9.12 is `verified` and mandatory for living rooms *and* kitchens — so the kitchen is pulled onto the façade by `win.habitable_has_window`, which that ticket also had to re-key because **a party wall is `External` and the rule was satisfiable on one** ✅ **The ring's shape family was only ever three-quarters implemented, and the missing quarter is the one that matters** — [The toy Envelope is more compact than a real dwelling](tickets/52-the-toy-envelope-is-more-compact-than-a-real-dwelling.md), ADR 0029. ADR 0003 names rect/L/U/T; the generator emitted rect, L and a T it *called* a U, so **every notch it ever cut was a corner notch** — which removes floor and adds **no perimeter at all**. Toy boundary is exactly `2(W+H)` at every count against a real dwelling's **6–12 %** excess over its own bbox. The ADR is **not amended**: the cap and the family stand, and `u_shape_true` now cuts the mid-edge notch that was always licensed. ⚠️ **A corner-notch-only family cannot be fitted to the corpus at any notch share** — it needs 27–36 % against a real 16–21 %, which is two thin arms rather than a dwelling. |
| Corpus conversion — how a real dwelling becomes retrieval and training data | settled | ⚠️ **`fit_rects.py` now owes a SIXTH per-record field — the notch index spans** — [The notch is two components and a quarter of donors have more](tickets/61-the-notch-is-two-components-and-a-quarter-of-donors-have-more.md), ADR 0020's second amendment. **Already promised** by `proposer.md` §2.2's own index-record table (*"`notches_used`, and each notch's index span"*) and **never emitted**: `env_at` computes the rectangles and discards them. It is **one computation** with the `voids` field, which 61 also widened — record the spans and a void is defined as the rest — so no new pass and no re-fit. ⚠️ **And it is what the conversion has been getting wrong**: the Envelope is the bbox minus **at most two** notch rectangles, `notches_used` is 2 on 90,16 % and never more, `notches_needed` is 3+ on **37,6 %**, and `envelope_loss` — the real notch area the cap leaves *inside* the ring — is p50 **1,78 %** of the domain, p90 **9,92 %**, mean **3,72 %**. ⚠️ **`fit_rects.py` also owes a FOURTH per-record field** — the enclosed void components with their **donor owner**, ADR 0028 — alongside the cut-line frame, per-pair relation provenance and `frontage_reach`. Both inputs are in hand and `experiments/void/provenance.py` is the reference; **take all four in one pass**, they are four statistics off the same records. ⚠️ **And the conversion is where the void comes from**: 98 % of it is the residue of ADR 0014's two-rectangle cap, not a duct — only **1.4 %** of components and **2.0 %** of the area lie inside a dropped `SHAFT`/`VOID`/`TECHNICAL_AREA`, which had to be checked because `watershed`'s 350 mm `WALL_REACH` swallows any dropped entity narrower than ~700 mm. ✅ **the 31 % drop is paid, and it was mostly a price for a deleted constraint** — ADR 0016 takes Swiss **30.70 % → 9.74 %** and ResPlan **40.10 % → 6.40 %**, paired, zero dwellings lost, every ADR 0008 guarantee re-asserted. **The slope moved more than the level**: the 83 %-at-4-rooms against 46 %-at-10 spread goes 35 points → 12, so the conversion has stopped preferring small dwellings. The `swiss_fit.json` labelling defect is **fixed at source**. ✅ **A converted dwelling has now been looked at, and it reads as a home** — 67 rendered beside their originals, ADR 0017. ⚠️ But **three of this row's own fidelity headlines are constraints restated, not measurements**: `edges_lost = 0`, zero flipped directions and the ±10 % area band are all posted hard, so *"zero adjacencies destroyed"* and *"9.5 % refused"* are one fact stated twice. Quote cell agreement **with worst-room IoU**, the refusal rate, and `boundary_lost`. ✅ **The conversion now knows how far a real outline is from an Envelope, exactly** — [Every Envelope the solver has seen is invented](tickets/58-every-envelope-the-solver-has-seen-is-invented.md), `rectangularisation.md` **§14**. The exact minimum rectangle partition (Lipski/Ohtsuki, verified on eight shapes) is **6** at the median, 12 at p90, 44 max, against **1–4** parts for either shipped fixture; only **12,4 %** of the index is ≤ 3 and **39,3 %** needs more rectangles than it has rooms. The outline is **stepped, not perforated** — 2,2 % enclose a hole, none pinches — so a wider notch budget was never the missing piece, which is §13.1's conclusion reached from the other side. ⚠️ **`fit_rects.py` gains no field and is deliberately unedited**, but it gains a *pattern*: `real_envelope.refit_to_true_mask` substitutes the domain at its **call boundary**, and that is how to ask it a different question without moving the substrate under four closed decisions. ⚠️ **The re-fit is materially harder than the shipped fit** — **11 of 71** in-band dwellings (15,5 %) cannot be re-fitted to their own boundary at the same 10 s budget, where the cap fit decided every one. ⚠️ Two failure modes left this row and became tickets — [The dwelling that is built on two angles](tickets/46-the-dwelling-that-is-built-on-two-angles.md), **still open and this row's**, and *The two-notch cap is now evidenced*, closed; two more were handed to the acceptance bar and to H8. ⚠️ **`why_k.clean()` does not do what it says** and its 58.3 % / 31.03 % figures remain an artefact. ⚠️ ADR 0008's *"decidable, not a timeout"* **is dead**: 1.27 % of Swiss and 16.5 % of ResPlan return UNKNOWN at 10 s, and ResPlan needs 30 s to decide at all | ✅ **The off-frame population is decided and it cost no new machinery** — [The dwelling that is built on two angles](tickets/46-the-dwelling-that-is-built-on-two-angles.md), ADR 0031. Kept and labelled; **`frame_residual` is the FIFTH per-record field** on the same pass, and the pass now also swaps `dwelling_frame` to the area-weighted modal angle — **the conversion is frozen until it runs**, and it may never run alone. ⚠️ **ADR 0017's 0,167 was a six-dwelling median** (true: 0,397) and its 2,7 % is **4,79 %**. ⚠️ **The `worst_room_iou` gate was already refusing 39,6 % of this population and nobody knew** — 28,6 % of everything it removes is off-frame, so the 6,65 % index cost buys something other than what it was read as. ⚠️ **And every `market`-arm number this row's rigs produced was measured at a superseded tier** — [The law is a hand copy and it now shapes rooms](tickets/69-the-law-is-a-hand-copy-and-it-now-shapes-rooms.md), ADR 0037. `absolute_area.MARKET` was a literal that fell four cells behind ADR 0035 within a day; re-read, **43,24 %** of the index's rooms change target by a mean **+0,561 m²**. **Conservative, not wrong** — every cell moved up, no floor moved, and ADR 0020 sizes the box from Σ target, so no room's statutory floor gets harder to clear. The re-run is cheap now that the tables resolve themselves and it belongs to **62**, **65** or **67**; `floors_for` also moved 59 dwellings' living floor 15,0 → 16,0 by resolving the otaq guard the copy had hand-written.
| Solver projection — CP-SAT, 250 mm grid, 15 s, τ = 4 | settled | ✅ **ADR 0039's encoding is built, exact and affordable, and its cost is the first on this map that IS measurable against the seed-to-seed spread** — 2,36× variables, +16,4 % solve time, time to first Plan 0,110 → 0,284 s against a 15 s cap, cap exhaustion 17 → 16 — [The bar plane is derived and the solver has never run it](tickets/77-the-bar-plane-is-derived-and-the-solver-has-never-run-it.md), ADR 0040. ✅ **And it is generalised to the Room ADR 0014 actually gives** — [What the bar plane owes a two-part Room](tickets/78-what-the-bar-plane-owes-a-two-part-room.md), ADR 0041, **Part IX**, over the **53,9 %** of the index that holds one. The join is **one length and 13 variables** — two interior-disjoint rectangles meet in at most one segment, so a Room needs **no contact literals against its own parts**. It fits: **1,82×** the variables, **+13,2 %** total solve, first Plan 0,257 → 0,623 s against 15 s, and the join term is 1,5 points of that — and over six seeds it is **inside the candidate's own spread on 18 of 19**, p = 0,167, where the plane is outside it on 15 of 19. ⚠️ **The band is 0,300 m² at p50, not 0,165** — the corpus joins at **8 grid units**, not ADR 0014's floor of 4 — and it still converts to **one verdict in 1 961**, because `site: both` re-sizes rather than refuses and a two-part Room's headroom over its own floor is p50 **9,37 m²**; only **1 of 345** has less headroom than its own join term. It is bought for the posted quantity, at 13 variables. ⚠️ **The 15 s headroom at two parts is not one part's**: the INCUMBENT already exhausts the cap on **90 of 284 (31,7 %)** against 17 of 307, so Part VIII's *"none of it lands where the budget is"* does not transfer — that is Design A's search space, not this encoding's. ✅ **the guillotine premise is discharged and nothing moved** — ADR 0019, 483 solves over 568 slots: 4 discordant each way, McNemar **p = 1.00**, and **zero** discordant at 8–16 rooms. 15 s, τ = 4 and ADR 0007 all stand at their published values. ✅ **Item 4 inverted**: a non-guillotine target reaches INFEASIBLE *less* often, **17 against 2, p = 0.0007** — the fallback fires less, not more, ⚠️ by an **unexplained** mechanism with the three obvious candidates excluded. ⚠️ **Two premises were false**: τ gates on separation margin, not adjacency, and those distributions are *identical* between arms; and **no experiment on this map ever ran at `t_int` = 120** — every one ran at **100**. ⚠️ The bottom of C13's band, below 7 rooms, still has **no non-guillotine measurement of any kind**. **[What an ordered entry sequence costs the solver](tickets/43-what-an-ordered-entry-sequence-costs-the-solver.md) is now unblocked** — ⚠️ and it now shares its whole directory with **[The toy Envelope is more compact than a real dwelling](tickets/52-the-toy-envelope-is-more-compact-than-a-real-dwelling.md)**, raised by 49 and holding three defects measured from the corpus side: the Envelope is **more compact than a real dwelling** and increasingly so with n (perimeter/area **0.390 against 0.572** at twelve rooms), **`AREA_PER_ROOM_M2` is 9.65 against a corpus median 11.36 m²** — 29's fixture defect, now with a number — and **`Envelope.exterior_fraction` double-counts**, 144 grid units of true perimeter counted as 180. It also holds `solver-formulation.md`'s exposure section, which is **wrong twice over** and whose *"arithmetically dead from 7 rooms"* table is **overturned outright**. Not at once with 43. — ⚠️ and it arrives holding a **second, unpriced job**: *Opening placement rules* took the contact threshold from `w_struct + t_int` to `w_struct + t_int + 400` (ADR 0021), because the old one reserved **zero jamb and zero nib** against two hard rules that require both. The arithmetic is exact and shipped on it; **the INFEASIBLE rate it costs has not been measured**, and `experiments/solver-toy/` is 43's ✅ **The fixture it was all measured on is now checked against real dwellings, and it was handicapping the solver** — [The toy Envelope is more compact than a real dwelling](tickets/52-the-toy-envelope-is-more-compact-than-a-real-dwelling.md), ADR 0029: a second corpus-fitted Envelope family beside the frozen published one, and 140 paired solves at the shipped config move survivors **68,6 % → 85,7 %** at **no time cost**, exact McNemar **p = 0,0005**, with **no slot** where the old fixture wins. 15 s and τ = 4 are re-affirmed as *sufficient* rather than re-fitted. ✅ **The bottom of C13's band is now one cell**: n = 5 solves 10/10 where the published fixture cannot even build a Brief, leaving **n = 6 alone** uncovered — and the cause is `assign_kinds` starving for cells that are both exterior-facing and habitable-sized, not the solver and never frontage. ⚠️ **Part III ran half its grid at a `corpus_median` that has since moved**; exposure is held fixed within each pair so the McNemar result stands and the rates are conservative — re-worded, not re-run. ✅ **The real boundary is now measured, and the solver survived it — what did not is everything upstream** — [Every Envelope the solver has seen is invented](tickets/58-every-envelope-the-solver-has-seen-is-invented.md), ADR 0030, Part V. **55 of the 57 real slots that reached the solver returned a candidate valid on every hard predicate other than exact tiling** (2 tiled exactly and survived, **53 failed on left-over floor alone**), and 2 went INFEASIBLE; the residue is ADR 0028's void at plan scale, median **1,06 m²**. ⚠️ **The survivor columns are not a solver comparison** — `no_brief` is **14 / 70 / 63** across `corpus` / `cap` / `real`, so the arms diverge in `ground_truth` and `assign_kinds`, not in `project`. ⚠️ **The number this row must carry is the middle rung**: `cap` is ADR 0003's own object sampled **per dwelling**, and it loses **70 of 120** slots where `corpus` loses 14 — **per-dwelling sampling alone costs more than half the survivors**, and every fixture on this map is a per-count median. ⚠️ **15 s is not refuted and its headroom was the fixture's**: wall p50 **0,19 → 3,30 → 10,11 s**, p90 at the cap on both real-sampled arms. ⚠️ **The pre-registered rule fired and was the wrong rule** — 104–0 at p = 0,0000, and the loss is not in the solve. ✅ **n = 6 is `envelope_for(6)`'s, not six rooms'** — `cap` survives 4/14 there against `corpus`'s 0/14, so C13's last uncovered cell has a cause. ✅ **And the solver has now been run on the object it will actually receive** — [Can a starved candidate be refused before the solve](tickets/59-can-a-starved-candidate-be-refused-before-the-solve.md), 273 warped candidates. **On a warped Proposal the projection is a repair, not a search**: wall p50 **0,145 s**, p90 0,98, 4,4 % at the 15 s cap, INFEASIBLE **5,1 %** overall and **8,8 %** at 7–10 rooms — all of it the statutory floor, 14 of 14 re-solving feasible without it. ⚠️ **Not comparable with Part V's 10,11 s or ADR 0029's timings**: this runs at `t_int` **150** where every published solver number on this map runs at 100, and the candidate tiles its own Envelope exactly with τ = 4 fixed. ✅ **The formulation defect is decided and it was arithmetic, not the grid** — [The projection discards a fifth of the guarantees the warp now buys](tickets/68-the-projection-discards-a-fifth-of-the-guarantees-the-warp-now-buys.md), **ADR 0039**, `solver-formulation.md` **Part VII**. `solver.py` binds H4 on `(250w − t)(250h − t)`, eroding all four sides where ADR 0001 does not erode at the Envelope — median **3,9 %** of a perimeter Room, **1,51 %** of Rooms in the gap. ~~*Unremovable at a 250 mm grid*~~ — **false, and the reason matters**: `absolute_area.space_m2` erodes `parts ∪ outside`, so the domain boundary already **is** the exterior inner face and there is no 75 mm ring to represent. The fix subtracts the band **per side** off `Envelope.all_faces()` — the decomposition `_add_exterior` already consumes — is **affine in H4's existing product** and spends **no second multiplication**. ⚠️ **The defect was never one predicate's and it runs both ways**: seven rules are `site: both` and read a clear dimension, and `dim.max_area` is a **cap** the solver reads *leniently*. ⚠️ **Nothing is implemented or measured** — the auxiliary integers and reified literals are bounded by rooms × 4 sides × faces and were never built, so **15 s and τ = 4 were fitted against a model without them**: [The bar plane is derived and the solver has never run it](tickets/77-the-bar-plane-is-derived-and-the-solver-has-never-run-it.md), which carries ADR 0039's own fallback (floors only, forward-only literals, the cap left to the validator); — ⚠️ **and `solver.py` still has no ADR 0014**, one rectangle per Room — ✅ but the second half of that is answered and priced: `room-rectangles/solver_parts.py` binds area per **part** where ADR 0014 and `acceptance-bar.md` §9.1 bind it per **Room**, and the binding site **alone** is **5 of 284 (1,76 %)** false refusals, all five rescued for **+2 variables** while the plane rescues **0** more. ADR 0041 decision 2 | ✅ **The H-list is closed at H10 and the last candidate H11 is refused** — [What an ordered entry sequence costs the solver](tickets/43-what-an-ordered-entry-sequence-costs-the-solver.md), `solver-formulation.md` **Part VI**. An *ordered* entry sequence wanted a per-Room hop-count integer against H8's *"no auxiliary integers"*; it was refused **on the corpus before it was priced**, so **15 s, τ = 4 and Part II's percentiles are untouched** and no solve was run. ⚠️ **No cost figure for an auxiliary integer exists** — do not infer one from ADR 0014's 1,2–1,7×, which is a box-count multiplier. ⚠️ **`solver-toy`'s `AREA_PER_ROOM_M2` = 9,65 is below the 11,58 the table needs at 7–8 rooms in either arm**, and there is no non-guillotine tiling below 7 at all — carried into §VI.3, and it bounds future pricing work to 8+ rooms or a fixed fixture.
| Proposer source B — trained transformer: architecture, corpus prep, metric, stopping rule | settled | ✅ **its evaluation has four plan-quality terms** — sleeping-group count, longest-run allocation, social transit and now **`frontage_reach`**, all computable on a corpus dwelling and a generated Plan by the same code, which corner displacement is not. `proposer.md` §6.1. ✅ **The daylight gap in that list is closed and the fear behind it was false** — [A third of real kitchens have no window](tickets/51-a-third-of-real-kitchens-have-no-window.md): *"a trained Proposer that learned windowless kitchens from 31 % of its data will propose them everywhere"* cannot happen, because **§2.3's model has no window token**. It emits two box slots and a presence token, so the only thing a windowless kitchen teaches it is an **interior** kitchen — a **5.88 %** prior, not 31 %. **No training filter**, on the reasoning that a landlocked room is a fact about real housing the solver already refuses hard, and trading 5.88 % of a corpus ADR 0013 calls thin to suppress a case the projection rejects anyway buys nothing. ⚠️ They are **evaluation only, not stop conditions**, and none of the four has been measured on a generated Plan because no Proposer has been run. ⚠️ **And this row inherits ADR 0028's defect in the form nobody can measure**: source A's enclosed void is a *named component* the Proposal can hand to a Room, while source B's boxes need not tile at all, so its unassigned floor is **diffuse**. It emits an empty `voids` list not because it has none but because it cannot name them, and H3 closes its holes by exactly the L1 tie 53 removed for source A. Unmeasurable until a Proposer is trained, alongside this row's unmeasured per-room absolute area fidelity | ⚠️ **A fifth term is owed and it has a ticket** — [What the entry-depth gradient is worth as a fifth evaluation term](tickets/66-what-the-entry-depth-gradient-is-worth-as-a-fifth-evaluation-term.md), `zoning.md` **D10**. The four terms do **not** capture *reads as designed*: entry-depth inversion is **negatively** associated with social transit — χ² **34,55**, p ≈ **4,2 × 10⁻⁹**, OR **0,354** — and **15,2 %** of dwellings invert the gradient with **no transit defect at all**, so term 3 is structurally blind to them (transit is *routing*, inversion is *distance*). ⚠️ **The statistic is the inversion rate (17,4 %), never the strict-order rate**: the corpus is **51,0 % ties**, so all-ties and all-reversed both score 0 % strict. ✅ **Taken, and §6.1 is now FIVE terms** — ADR 0042. The rate is a **ceiling, not a target**: D10 asked to match 17,4 % while §6.5 calls the same bucket a *violation*, and matching rewards producing as many bad plans as the housing stock does. Lower scores better, 0 % is best, and the term is **stratified** by habitable-room count because the pooled figure is a mixture over the corpus size mix. ⚠️ **It carries no statutory backing and says so** — no regulator in either lineage scores depth from the entrance, and WBS 2015 scores zoning nowhere at all, which is itself the argument for the ceiling: the corpus's 17,4 % is what a stock produces when its own evaluation system never looks. ⚠️ **Four of the five terms cannot be computed until `is_sleeping` lands** (80), and **what the other four are scored *for* is still open** (81).
| Runtime and process split — engine / proposer service / BFF, job model, threads, JSON | settled | ⚠️ the honest end state (queue + result store) is fog, under *Persistence, accounts, hosting*. ✅ **That claim is priced and it is cheap** — [Can a starved candidate be refused before the solve](tickets/59-can-a-starved-candidate-be-refused-before-the-solve.md). §11.1 step 1 spends a **mean 2,17 s** per extra pool member (warp 1,12 + projection 1,05), not a warp plus a 15 s solve, because a warped candidate arrives as an exact tiling with τ = 4 relations fixed and the projection is a **repair, not a search** — wall p50 **0,145 s**, 4,4 % reaching the cap. `POOL_DEPTH_ON_STARVATION = 16`, on the ~3 % of Briefs that reach the step, so the second retrieval round-trip is ~17 s on one Brief in thirty. ⚠️ **What the job model still owes is the round-trip itself** — the escalation is now a specified sequence (solve, then deepen, then source B) and nothing in the runtime carries it |
| DXF export | settled | — |
| Proposal contract — what a source emits and the solver consumes | settled | ⚠️ **`voids` holds more than it did and the field's shape is unchanged** — [The notch is two components and a quarter of donors have more](tickets/61-the-notch-is-two-components-and-a-quarter-of-donors-have-more.md), ADR 0028's amendment. A void is now **floor inside the Envelope that no Room covers** — every complement component other than the `notches_used` notch spans — rather than the *enclosed* ones. Population **15,49 % → ~40 %** of source A candidates; the **84,5 % empty** figure below is the enclosed reading and is superseded. Contract shape, membership test and derivability argument all stand. ✅ **It gained its second field on ADR 0014's test, and the test is now written down** — `voids: [(span, receiving_room)]`, ADR 0028. A property enters this contract when the solver can derive it from **neither** the `ResolvedBrief` **nor** the boxes: the receiving Room of an enclosed void is not derivable (largest shared edge agrees with the donor **28.4 %** and ties on 28.4 % of components; largest bordering Room 38.1 %) and not inferable, because H3 plus L1 corner displacement makes **every bordering Room's repair cost the same**. Empty on source B and on 84.5 % of source A candidates. ⚠️ **Source B has the same defect in a form nobody can measure** — its boxes need not tile, so its slack is diffuse rather than in named components, and it emits an empty list not because it has none. ✅ **it carries no zoning, and that is the decision** — the node set is derivable from Room type, so ADR 0014's *only the Proposal knows* argument does not transfer. §1 records the refusal with its reasoning. ⚠️ The premise it was challenged on was **half false**: `wet.plumbing_group_count` is a set-versus-set predicate shipping today, so the Proposal is pairwise and the *system* is not |
| Proposer source A — retrieval-and-warp, which ships first | settled | ✅ **The warp posts the statutory floor and ADR 0027's debt is paid** — [Should the warp post the statutory floor](tickets/64-should-the-warp-post-the-statutory-floor.md), ADR 0033. A Proposal from source A now carries an **invariant**: every Room meets the statutory limb of its hard area floor on the bar plane, or the warp returned INFEASIBLE. It was the largest unposted constraint on this map and the cheapest — **linear**, no product at all. The unposted warp emitted **31,6 %** of candidates below the law; a two-pass fallback was refused for buying a rate and no invariant. ⚠️ **A 4,6 % residual survives as an encoding estimate** and [The posted floor is a seed-shape estimate](tickets/67-the-posted-floor-is-a-seed-shape-estimate.md) owns it — `proposer.md` §2.2.2 decision 7 states the invariant without that asterisk and one of the two must move. ⚠️ **`experiments/warp/` is unclaimed again** and now holds `floor_warp.py` alongside 57's four probes and 59's `project_join.py`. ✅ **The gate is four terms and the fourth is sound** — [The gate measures stretch with two blunt scalars](tickets/63-the-gate-measures-stretch-with-two-blunt-scalars.md), ADR 0032. `req ≤ 1` is the smallest box extent the donor's frame admits at the ergonomic floor, an interval DP over spans the record already carries; it refuses **only** candidates the warp declines (103/103) and takes per-candidate decline 27.6 % → 22.9 % at unchanged best-of-pool fidelity. ⚠️ **Decided at `m = 3` and the shipped `m` is 8** — the bootstrap's `m = 8` block is best-of-3 against best-of-6 and is not quotable; owed by [What the fourth gate term is worth at the shipped pool depth](tickets/65-what-the-fourth-gate-term-is-worth-at-the-shipped-pool-depth.md). ⚠️ **Replacement was refused on measurement against a market prior that favoured it** — dropping the two scalars costs p90 0.2303 → 0.2543. ✅ **The last open ticket on this file is closed and the warp had an unpriced region in it** — [A donor's enclosed void becomes area nobody asked for](tickets/53-a-donors-enclosed-void-becomes-area-nobody-asked-for.md), ADR 0028, `proposer.md` §2.2.8. **The premise inverted**: the warp does not *carry* a donor's enclosed void, it **amplifies it 2.2×** — p90 1.31 → **3.19 m²**, growing in 62 % of cases — because `fit_warp.warp_model:146` prices worst-room deviation and the weighted sum and nothing else. Charged to a receiving Room and weighted, p90 goes **3.50 → 1.50 m²** at **INFEASIBLE unchanged** and one `AddMultiplicationEquality` per component. ⚠️ **The number to carry off this row is in the deviation column and it reads backwards**: charged worst-room deviation is p50 **0.0959** against free's 0.0652, and that is the size of the **understatement** in every warp fidelity figure quoted on a voided candidate — `free` measures a Room's parts and ignores the floor it is about to be handed. **No published number moves**, and that is not reassurance: `absolute_area.py` has no output for realised unassigned area at all. ✅ **Not gated** — 11.74 % of the index, 20.5 % at nine rooms, against 47's accepted 6.65 % and refused 17.2 %, and refusing a dwelling for a hole ADR 0014's own cap put there charges the corpus twice. ✅ **47's `worst_room_iou` is landed** — index field, hard gate at 0.30, rank above it — and 56's §2.2.3 *"for free"* sentence is struck. ✅ **Its yield's price against pool depth is measured and the escalation that rested on it does not hold** — [What best-of-pool is worth at production pool depth](tickets/57-what-best-of-pool-is-worth-at-production-depth.md), `proposer-architecture.md` §7.6/§7.7. **Deepening buys about one point**: 4,1 % → 3,1 % over an *eightfold* pool, flat by m ≈ 12, and at **7–10 rooms it buys nothing at all** (8.4 % from m = 6 to m = 64). Under it is a floor no depth reaches, **π = 2,8 %** [0,3–5,6] and **5,3 %** [0,0–11,2] at 7–10. ⚠️ **The correlation survived 56's level fix** — conditional decline 35.2 → 50.7 → 73.5 → **88.9 %** by the seventh candidate — so the residual is the warp's per-room *distribution* and it does not thin across a pool. ✅ **Every number on this row is Proposal-level, and that now has a measured consequence** — [Can a starved candidate be refused before the solve](tickets/59-can-a-starved-candidate-be-refused-before-the-solve.md), `proposer.md` §2.2.9. A warped Proposal has been through `project()`: **82,0 %** of the candidates that look starved on the warped rectangles are **served** once the projection sizes them, because it posts the floors hard and moves floor between Rooms at Σ Space p50 **0,0000**. ⚠️ **So read this row's decline rates as *warp* fidelity and never as plan yield** — four fifths of them dissolve in a solve that costs **less than the warp** (wall p50 0,145 s against warp 0,674). ✅ **The pool these were measured on was not the shipped gate, and the repair moved this row's own numbers** — [The rig gate is not the shipped gate](tickets/60-the-rig-gate-is-not-the-shipped-gate.md). `gate_pool` diverged three ways and is split into `admissible_pool` and `bucket_pool`. The two differences did pull opposite ways and both are now sized: **per candidate the rig was pessimistic** — `ring` plans losing a Room **24,9 % → 17,1 %**, Rooms under a floor 10,1 → **6,2 %**, because a gate-refused donor declines **36,2 %** against an admitted donor's 27,6 % (paired within one Brief, **p = 0,0001**) — **and on reach it was optimistic**, `briefs_with_no_usable_candidate` **105 of 600**. Σ Space is unmoved at **+0,5 %**, so 56's level result stands and this is a distribution correction. ⚠️ **The gate's two dimensional terms are a proxy for frame stretch**, not for the arrangement claim §2.2 gives as their reason, and a coarse one — that is now [The gate measures stretch with two blunt scalars](tickets/63-the-gate-measures-stretch-with-two-blunt-scalars.md). ✅ **Both of 53's obligations are discharged**: the realised hole is on the record, and the genuinely constrained warp — never run before — costs **2,6 %** of candidates, **all of it ADR 0020's notch**, ADR 0028's void charge costing **zero**. ✅ **46's fog patch is sized and it graduates rather than dying** — [What a sheared donor costs a warped candidate](tickets/62-what-a-sheared-donor-costs-a-warped-candidate.md). Its branch condition was whether the pool is ever deepened past the rank where off-frame donors enter: a 4–8° donor sits at the **10,6th percentile**, rank ≈ 9 of a 58–87 bucket, and `m = 8` stops one short — but 57's curve puts the useful depth at **m ≈ 12–16**, so the window worth spending is exactly the window that starts drawing them. Blocked on 59, which sets that constant **and** owns the warp-to-projection join the question needs. ✅ **The floor that was neither notch nor void is now void, and `s` is now the notch** — [The notch is two components and a quarter of donors have more](tickets/61-the-notch-is-two-components-and-a-quarter-of-donors-have-more.md), ADR 0020's second amendment and ADR 0028's. `proposer.md` §2.2.3's *"the boundary-touching complement is the **building**"* was **false on both halves** and is struck: the ring holds at most two notches, **37,6 %** of donors need three or more, and everything past the second is residue that reached an edge. ⚠️ **Every number on this row was measured on an over-sized box** — the re-based `s` shrinks it p50 **1,7 %** / mean **2,2 %** — and the over-sizing was **paying for** the uncovered floor: Σ Space is **+0,4 %** of `target_area` today and would fall to about **−1,9 %** if `s` moved without the void widening. ⚠️ **`fit_rects.py` now owes a SIXTH field**, the notch index spans — already promised by §2.2's own record table, never emitted, and one computation with the widened `voids` since `env_at` returns the rectangles and throws them away. |
| Acceptance bar — **43** predicates (**44 once `dim.leg_join` lands**), enforcement sites, conformance test | settled | ✅ **§11.1 carries a measured number again: 1,30 %, and the plane is all of it** — [The bar plane is derived and the solver has never run it](tickets/77-the-bar-plane-is-derived-and-the-solver-has-never-run-it.md), ADR 0040. 4 of 307 INFEASIBLE on the incumbent, **0 of 307** on the bar plane, and **5,1 % is retired** — it was measured on a warp that posted nothing and on a population ADR 0037 has since moved. ✅ **`dim.max_area` was posted for the first time and it binds** — 10 Rooms of 1 993, every one a BATHROOM, worst 10,2 m² over §9.3's own defect. ✅ **§11.1's third site is taken and the escalation did not move** — [Should the warp post the statutory floor](tickets/64-should-the-warp-post-the-statutory-floor.md), ADR 0033. The screen stays refused; what changed is the population the three steps run on, so **18,3 % Proposal-level starvation is no longer the number to quote at step 1** — it was measured on a warp that posted nothing. ✅ **The plane defect is closed and 19,5 % is retired from this file** — [The projection discards a fifth of the guarantees the warp now buys](tickets/68-the-projection-discards-a-fifth-of-the-guarantees-the-warp-now-buys.md), ADR 0039. ⚠️ **19,5 % was measured at the wrong site, one stage on from where §11.1 caught itself doing exactly that at 3,6 %**: `project_join.planes()` runs **no solver**, and the rule is `site: both`, so a Room short on the solver's plane is *re-sized* and a refusal can only be INFEASIBLE — **14 of 273**, all fourteen the floor's by ablation, **5,1 %**, itself an upper bound containing genuine starvation. §11.1's escalation, its refused screen and its three steps are **untouched**; what moved is the population and the arithmetic underneath it. ⚠️ **`dim.max_area`'s note is now false and this row's file could not fix it** — *"the solver side is FREE … an upper bound on a product tightens propagation"* did not tighten anything, because it was posted on a quantity smaller than the one the rule bounds; `rules.json` has two claimants (72, 76) so it is handed on as prose. ✅ **Ticket 70's `KITCHEN_DINING` limb handoff is discharged into §11.1**: four limbs transcribe a whole-room figure, one is a sound lower bound entailed from a part, and a clearing Plan guarantees less on that one — no value, severity, site or order moved. ✅ **The row's last decision is taken and it cost a standing constraint** — [A statutory floor, posted soft, in the one region v1 ships](tickets/50-a-statutory-floor-posted-soft.md). C14 is amended **monotonically**: a profile may RAISE a hard floor and never lower one, so `dim.statutory_min_area` is the **first hard rule on this map carrying a region** (42 → 43, subset 15 → 17), and `win.area_ratio` goes soft → hard rescoped to living rooms and kitchens. ⚠️ **This row's proudest rule was inert**: `dim.min_area` adds **0.00 %** to the hard union while being the only predicate between a Homeowner and the 3,1 m² bedroom §3 was written to forbid. ⚠️ **Do not quote the 54,51 %** — the bar does not gate the retrieval index and `market_default` is at or above `statutory_floor` in every reachable AZ cell, so the rule is strictly weaker than the target the solver already aims at. ⚠️ **The kitchen limb sits on the corpus median** (8,0 against p50 8,04), 16,88 of 19,98 marginal points; taken on §7.5's precedent. ⚠️ **The yield trigger is named and unmeasured**: the warp has never been checked against a stated `target_area`, and if it undershoots this rule collapses yield — a one-field build-time rollback, chosen over a soft rule that ships a 6,6 m² kitchen as a survivor. ⚠️ **`room-constraints.json` and `rules.json` disagreed about `statutory_floor` — warn against unread — and neither had a rule behind it.** ⚠️ **What the row still owes is transcription, not decision, and it lives in `rules.json`'s own `owed` block rather than here**: five zoning rules ready to transcribe at `zoning.md` §5b, `dim.prefer_single_part`, the message locale schema over 43 rules, `f_hi`/`f_lo` into data, `wet.shared_wall_length`'s missing gradient — and **one BLOCKING item that is 32's**, the window width series `win.area_ratio` needs to be satisfiable (reach p90 2,47 / 3,23 / 1,34 m). That is why this row is settled with no open ticket: staffable, with named owners. ✅ **the widest gap on the map is closed, and the vocabulary was the thing blocking it** — ADR 0023. **`ENGINE_CHOICE` 18 → 9**, and the nine that remain are predicates about shape or program with no magnitude to measure, so *the remaining gap is not a measurement gap*. `conf` gains a fourth value, **`fitted`**, because `verified`/`derived`/`engine_choice` marked a number fitted to 42,985 real dwellings identically to a guess — under the old vocabulary this count could never have moved however much measuring was done. **Two thresholds move and seven guesses hold**: `wet.plumbing_group_count` 2 → **3** (the ticket predicted it — the tail reaches three at **14.14 %** of real dwellings) and `area.invented_envelope_soft` 2 % → **3 %** (the 250 mm grid alone misses 2 % in **13.71 %**); `dim.aspect_ratio_hard` **is** the p99.5, at 3.02. **The three area rules landed**: `dim.max_area` and `dim.stated_target_implausible` added, `dim.market_default_area` made two-sided. ⚠️ **The `site: both` conformance subset moves 14 → 15** and the rule count 40 → **42**, both stale in `acceptance-bar.md`. ⚠️ **The number nobody had measured is the conjunction**: the hard registry rejects **84.41 %** of real Swiss dwellings as shipped and **82.31 %** under the fitted values, with **eleven of thirteen hard rules costing under a third of a point between them**. **The bar is two rules and a rounding error** — `open.fits_segment` and `win.habitable_has_window` — and neither has a threshold that could be loosened to move it; read `open.fits_segment` on real piers rather than on full-width openings and it is **61.23 %**. Both are **opening-layer** rules, placed after the solve, so a warped donor's prior of clearing the bar is set by a layer the Proposal does not carry.  ✅ **Every threshold is now fitted or declared unmeasurable, and what the row still owes is rules, not numbers** — five zoning rules, `dim.prefer_single_part`, the message locale schema and `f_hi`/`f_lo` into data, all now written into `rules.json`'s own **`owed`** block so the next holder does not reconstruct them from this map. ✅ **Opening rules are paid — and closing them found the bar contradicting the solver.** *Opening placement rules*: the contact threshold reserved `w_struct` of **clear** run and nothing else, while `open.fits_segment` and `open.leading_edge_nib` hard-require `w + 400` on that same segment, so **a solve could pass potential circulation and be hard-rejected the instant a door was placed on the run it had just certified** — ADR 0021 takes the threshold to `w + t_int + 400`. Six rule statements moved, **none added**, so the 38 count is untouched on purpose. ⚠️ Two of those moves are the bar's to publish and could not be written: **`win.habitable_has_window` was satisfiable on a party wall** (a party wall *is* `External`, so a mid-block bedroom took its daylight off the neighbour), and **`win.kitchen_windowless` can no longer fire** now `kitchen.needs_window` is true — retire-or-keep moves the count. ⚠️ And **`win.area_ratio` is the only statutory minimum on this map posted `soft`** — AzDTN cl. 9.13 is `verified` and mandatory — left alone because severity is the bar's. ✅ **The bar now has a rule of the shape *this dwelling owes a room at all*, and it cost a room type** — *A dwelling with no toilet passes every check*, ADR 0022. **Four `programme` rules, 36 → 40**, one per limb of AzDTN cl. 5.2, `scope: brief` with **no plan-side twin** — the Room set is frozen at `resolve`, so a plan-side composition predicate could never fire, and these are the first rules on this map with an **image and no pre-image**. Kitchen, washing and WC **hard** (5.99 / 7.33 / 5.19 % of real dwellings); storage **warn**, because hard it rejects **73.35 %** and the norm's own alternative is a built-in wardrobe v1 does not model. The `holl` limb gets **no rule**: `resolve` guarantees it, and a rule that cannot fire is what retired two rules above. ⚠️ **The WC limb was unshippable over eighteen Room types** — it rejected **48.32 %**, and only 5.19 of those points were toilet-less homes; the other **43.13** *have* a toilet in a room with a bath that the vocabulary could not name. Closed by a **nineteenth type**, `bathroom_combined` (1500 × 1700 = 2.5 m², rejecting 6.17 % of 35,821 real bath+WC rooms, with the corpus's own short-side p5 of **1477 mm** reproducing the derived 1500). ⚠️ **Two shipped sentences were false**: `bathroom`'s ergonomic note claimed a pan and basin that **cannot fit** (1.81 m² of fixture in a 1.70 m² room), and `shower_room` **has composed a WC pan since the layer was authored** while the AZ bridge asserted the layer *"carries no way to say the WC is inside"*. ⚠️ **cl. 5.10's restriction of the combined unit to one-otaq social stock is recorded and deliberately not enforced** — C8 forbids reading a regulatory document as a compliance target, and 67.24 % of 44,372 real dwellings combine, so it does not describe practice either. ⚠️ **Bound 8 now fights bounds 1/3/6**: adding the `wc` a refusal asks for raises Σ minima and the engine room count, so a nine-room Brief with no toilet is told to add a room *and* told it may not — nothing orders the two sentences, and it is `homeowner-surface.md`'s. ✅ **the 40 m² WC is answered**: `dim.max_area` hard at `both`, and **free in the solver** — H4's `a = w·h` already exists. ⚠️ **And five more rules are owed, from *Where a set-versus-set property lives***, specified ready to transcribe at `zoning.md` §5b — one hard (`zone.sleeping_group_count`, at most two sleeping groups, 97.5 % of real dwellings) and four soft or warn, of which **`zone.no_social_transit` is the one nobody had written**: `circ.no_private_transit` blocks routing through a bedroom and *nothing* blocks routing through the living room, which 18.2 % of real dwellings do. ⚠️ Every dimensional rule now has to declare **which part it binds** — ADR 0014 binds minima and aspect per part, area per Room — and one new soft rule, `dim.prefer_single_part`, is owed to `rules.json`'s holder — as is a **locale dimension on every Homeowner-facing message**, since §11 requires a plain-language message per rule, all **40** are English, and the surface is now Azerbaijani: a schema change, not a translation pass. ~~⚠️ **And nothing in the bar forbids floor that belongs to nothing**~~ — **that was false, and this row repeated it.** `model.no_unassigned_area` is **hard**, `site: both`: *"the union of all Space polygons and all Wall bodies equals the Envelope interior exactly."* Exact tiling is soft **in the solver** for a 29× faster search and hard **at the validator**, which its own note calls *"the place where that trade is prevented from shipping a hole"* — so an OPTIMAL candidate with a 1 m² unnamed hole **cannot be shown**. Checked rather than assumed by *A dwelling with no toilet passes every check*; **no rule added and none owed.** What survives was the **proposer's**, and it is now **closed** — [A donor's enclosed void becomes area nobody asked for](tickets/53-a-donors-enclosed-void-becomes-area-nobody-asked-for.md), ADR 0028. ⚠️ **Read the 10.0 % as superseded**: it measures uncovered floor against the real dwelling on the first 400 records in file order, and on the parts frame over the whole index the figure at ≥ 0.5 m² is **6.73 %**. The void is charged to a Room in the warp and carried on the Proposal, so the solve no longer absorbs it into a Room chosen by an L1 tie; **still no rule added and none owed** ✅ **Two rules retired by *H8 and the single-aspect flat*, 38 → 36**, because neither could fire: `win.habitable_touches_exterior` was strictly implied by `win.habitable_has_window`, and `win.kitchen_windowless` became unreachable when the kitchen took its window. Both kept in a `retired` block. `win.habitable_has_window` moves to site `both` and posts the frontage budget, so the conformance subset holds at 14. ⚠️ **The bar's corpus cost is measured for the first time and it is large**: that one rule rejects **43.3 %** of real Swiss dwellings, 23.0 points of it the kitchen alone — worse than the 26.6 % that made *What a room's area is allowed to be* refuse a p95 cap, and it carries **no threshold to move**. ⚠️ The bar also admits a **1.85 × 1.68 m double bedroom**; the room table owns that number, not a window rule. ⚠️ **`win.habitable_has_window` now carries THREE corpus costs answering three questions, and the file says which is which nowhere** — [A third of real kitchens have no window](tickets/51-a-third-of-real-kitchens-have-no-window.md): `rules.json`'s `corpus_cost` **0.4519** is the *raw* arm over 42,985 unconverted dwellings, **38.55 %** is the converted index at 46,565, and **15.97 points** is the same rule's leave-one-out contribution to the whole bar. None is wrong and none is the others; **none of them is the retrieval cost**, which is **6.39 %** and lives in `proposer.md` §4.5. One sentence, owed to this file's next holder — 51 deliberately did not write it or touch the number, because overwriting 0.4519 would destroy a measurement rather than improve one ⚠️ **`dim.statutory_min_area`'s severity is now a live question with a measured price** — [The warp has never been measured against a stated target area](tickets/54-the-warp-has-never-been-measured-against-a-stated-area.md). 50 posted it hard on the argument that a Plan reaching its soft target clears the floor by construction, and named its own reversal trigger: *a hard rule that is too strict is discovered at build time on the first Proposer run.* **That trigger has fired early.** ~~On Briefs whose every target sits on `market_default`, the rule alone costs **31,1 % of candidates** and **6,7 % of Briefs at pool-of-8**~~ — **both superseded, and the rule is about half as expensive as it was posted at**: [The sizing rung under-delivers by four per cent](tickets/56-the-sizing-rung-under-delivers-on-the-warp-path.md) found the *measurement* was carrying the difference, not the rule. Same sample, same seed, both Envelope defects fixed: **25,5 % of candidates** and **3,6 % of Briefs at pool-of-8**, with the passing kitchen's lower quartile clearing by **518 litres** rather than 85 — so *"passing by luck"* no longer describes it, though **17,4 %** of kitchens asked for 9,0 m² are still delivered under 8,0 and no sizing constant reaches that. ✅ **The severity is re-taken with the price known and the rule does not move** — [The statutory floor now has a price](tickets/55-does-the-statutory-floor-stay-hard-now-that-it-has-a-price.md), `acceptance-bar.md` §3.2. **Hard, all five limbs**, on four reasons of which the market one would have sufficed: `competitive-landscape.md` §5.2 — *code compliance is claimed by six vendors and implemented by approximately zero* — so a curated, geometrically-enforced statutory floor is the **one thing this engine has that the surveyed market does not**, and 3,6 % of yield does not buy it back. The kitchen limb was the one to argue and **dropping it removes the floor rather than lowering it**: ergonomic `kitchen.min_area` is **1,8 m²** against a statutory 8,0. What moved is the *shape* — the composition is now the term **Hard area floor** in `CONTEXT.md`, so a limb amendment is a value edit; `brief.md` gains **bound 9** at `warn`, **the first bound ADR 0015 declines rather than decides** (Σ Space is fixed exactly and targets are two-sided, so a 6 m² kitchen *can* be delivered at 8); and §11 gains **§11.1**, the starved Brief with **no defect to name** — a stated hole in the same-sentence guarantee plus a three-step escalation that never touches the hard set. ✅ **3,6 % is measured against pool depth and §11.1 step 1 does not hold** — deepening buys **one point** (4,1 % → 3,1 % over an eightfold pool, flat by m ≈ 12) and **nothing at all at 7–10 rooms**, under a floor of **π = 2,8 %** no depth reaches. So step 1 is a **config value**, not the answer, and steps 2 and 3 carry the residual. ⚠️ **The correlation 56's level fix was expected to dissolve is still there** — conditional decline reaches **88.9 %** by the seventh candidate. ⚠️ **3,6 % was also measured through the wrong pool** — under the shipped gate it is **4,4 %** (22 of 495), *up*, because the gated pool is ~10× shallower and at Brief level depth is what buys survival; and `briefs_with_no_usable_candidate` reaches **105 of 600**, about one Brief in six going to source B rather than a warp — [The rig gate is not the shipped gate](tickets/60-the-rig-gate-is-not-the-shipped-gate.md). ✅ **It was Proposal-level, the figure fell on the generous side, and §11.1's trigger moved** — [Can a starved candidate be refused before the solve](tickets/59-can-a-starved-candidate-be-refused-before-the-solve.md). Starvation is now declared **on the Plan**: **82,0 %** of Proposal-starved candidates are served by the projection [71,4–92,6], and Brief-level starvation reads **3,28 % → 1,64 %** — ⚠️ on 61 Briefs and 2 starved cases, a direction rather than a number. **A third enforcement site was weighed and refused**: unsound at 82 %, its only sound form (Σ hard floors against the candidate's own box, p50 0,566, **max 0,736**) never fires, and it would sit after the expensive step. **Step 1 is priced and affordable** — a mean **2,17 s** per extra pool member, `POOL_DEPTH_ON_STARVATION = 16`; the 10,11 s that made it look unaffordable is 58's **real-boundary** arm, which no candidate presents. ✅ **The solver now reads this bar's plane** — ADR 0039. It bound H4 on all four sides where ADR 0001 does not erode at the Envelope, **3,9 %** of a perimeter Room and **1,51 %** of warped Rooms in the gap; ~~*structurally, because 75 mm is below the 250 mm grid*~~ — the domain boundary already **is** the inner face, and the band is subtracted per side. ⚠️ **Decided, not shipped** — [The bar plane is derived and the solver has never run it](tickets/77-the-bar-plane-is-derived-and-the-solver-has-never-run-it.md). ⚠️ **What 59 refused is the *filter*, and the third-site question is open on this row** — [Should the warp post the statutory floor](tickets/64-should-the-warp-post-the-statutory-floor.md): posting the floor as a constraint **inside the warp** re-sizes rather than refuses, so none of the screen's three grounds transfers, and it could buy the projection's **8,8 %** INFEASIBLE at 7–10 rooms — the band where depth buys nothing and the rescue rate is weakest. ⚠️ **Source B's per-room absolute area fidelity is unmeasured** and §11.1 step 2 routes the starved Brief there. ⚠️ **Do not read it against the bar's 15,59 %** — one is a predicate's cost on generated candidates, the other the whole hard registry's survival rate on real dwellings. ⚠️ **And do not reach for the 18,8 %** — that was `calib`, which scales the box until Σ Space hits `target_area` and hands the rooms margin the Brief does not entitle them to; a correct Envelope over-delivers by **0,4 %**, not by `calib`'s 2,2 % of slack. ✅ **A hard rule was unevaluable on nine of nineteen Room types, and one of them is in every Plan** — [Nineteen room types and nine area-band classes](tickets/71-nineteen-room-types-and-nine-area-band-classes.md), ADR 0038. `dim.max_area` states `k[type]`, the table is `k[class]`, and the bridge was **absent** rather than `null` — the ticket said null, which is the weaker claim, because a null at least declares the slot. Nine types resolved to nothing and the rule **silently did not fire**; eight are reachable and all eight Brief-nameable. **`hall` decides the severity**: `brief.md` §3.1 invents one wherever the Brief names none, so **100 % of Plans carried a Room with no upper bound**, ceilinged only by `circ.fraction_hard` — **27 m² of hall on a 90 m² dwelling passes every rule in the file**, which is §9.3's 40 m² WC through a different door and shipping rather than hypothetical. `soft_w[type]` had the same hole on three more types. ✅ **The ticket's own framing was refused and a third option answers it**: not *borrow* and not *earn* but **containment** — a type takes the class whose measured population **contains** it, which borrows nothing, because `CORRIDOR` **is** hall ∪ entrance_lobby ∪ corridor merged. A *target* must be type-specific and a merge cannot supply one, which is why rung 2 is null for `hall`; a *band* is a dispersion statistic over a population and that population **is** the merge. Sixteen of nineteen resolve with no invention. ✅ **The two compound types went from *no plausible class at all* to the best-evidenced rows**: both targets were already per-dwelling sums of clean donors, so the same composition yields the band — `kitchen_dining` cap **49,77** / k **2,64** (n = 1 308, p50 reproducing the shipped 18,8 exactly) and `living_dining_kitchen` cap **69,00** / k **1,86** (n = 24 046). **Borrowing was measured and refused**: −3 % to **+39 %** with an unguessable sign, and biased **lenient** by a mechanism — a compound's k is always below its donors' because summing partly decorrelates. ⚠️ **One row is an analogy and says so**: `utility` takes `bathroom`, not `storeroom`. No population contains it — Swiss has no laundry label and **MİDA's Baku schedules carry an eight-name vocabulary with no utility room and no storeroom in it** — and `STOREROOM` is dry residual space (CV 1,04) while a utility is `is_wet`, a plumbed appliance room sized by a machine plus a body zone, exactly as the ergonomic layer derives both. It also decides the number: 18,23 m² against 9,15. **Totality is gated**, and the two `gate_check.py` assertions are written out to be transcribed. ⚠️ **ADR 0037 was under-scoped**: two more private copies sit in `experiments/acceptance-thresholds/` (`reject.py`, `census.py`) and their class keys **disagree with the shipped table** — `living`/`storage` against `living_room`/`storeroom`. ⚠️ **And `dim.max_area`'s published `corpus_cost` 3,11 % was measured with `kitchen_dining` silently exempt**, by the same `is not None` guard this ticket is about, inside the rig that priced the rule — 40 rooms, 0,093 %, so **the number survives and its provenance does not**. ⚠️ **What it could not fix is now [A cap fitted in one country and a target set in another](tickets/76-a-cap-fitted-in-one-country-and-a-target-set-in-another.md)**: `k` is a CH ratio and `target` an AZ number, their product runs **62 %–114 %** of the class's own cap, and it **inverts** — `living` caps at 41,4 m² against `living_dining`'s 35,6, so the room containing a dining area caps *smaller*. ⚠️ **ADR 0033's invariant had a hole and nothing could see it** — [A gas hob decides whether the open-plan type is buildable in AZ](tickets/74-a-gas-hob-decides-whether-the-open-plan-type-is-buildable.md). `dim.statutory_min_area` binds at site **both** and demanded **22,0 m²** on a `living_dining_kitchen`, while `absolute_area.floors_for` returns **`None`** for it — `LIVING_FAMILY` is `("LIVING_ROOM", "LIVING_DINING")` and there is no open-plan limb — so **the warp posted nothing on a type the bar refused**. ADR 0036 takes that cell to `az_area: null`, so the two now agree at *no AZ floor*, **coincidentally, through two unrelated omissions**; making it deliberate is 69's. ⚠️ **`floor_warp._check_floor_transcription` is structurally blind to this class**: it asserts the six values that **are** copied, so a hand copy wrong by **omission** passes it. ⚠️ **And none of ADR 0034's four owed referent gates checks the licence** — a `compose_with` on a type the norm does not define passes all four; a **fifth** goes to 69. ✅ **That false sentence is corrected** — `dim.statutory_min_area`'s `value_source` read *"Two AZ reads are `part`"* where **one** remains; taken by [Nineteen room types and nine area-band classes](tickets/71-nineteen-room-types-and-nine-area-band-classes.md), which held the file. 73's `circ.fraction_hard` comparator was taken in the same pass and **moved no value**: Baku runs 13,2 % of net internal against the Swiss-fitted 30 %, which is a bound Baku practice does not approach — confirming, not contradicting |
| Standards table — region-invariant ergonomic floor + the `AZ` profile | settled | ✅ **the type set is nineteen, not eighteen** — `bathroom_combined`, added by *A dwelling with no toilet passes every check* because `prog.wc_exists` is unshippable without it. ⚠️ **Two of this table's own sentences were false**: `bathroom`'s note claimed a pan and a basin that **cannot fit** (1.81 m² of fixture in a 1.70 m² room) and `shower_room`'s programme **has composed a pan all along**, against an AZ bridge asserting the layer *"carries no way to say the WC is inside"*. Both corrected at source. ⚠️ **And `build_ergonomic_layer.py` was silently destructive** — it authors the arithmetic and four flags and nothing else, while three later tickets hand-edited the block it emits (`counts_as_otaq`, `brief_nameable`, `reachable_in_v1`, `counts_as_otaq_sourcing`, `corpus_medians`), so **every re-run deleted all of it without a word**, which is the exact drift its own docstring claims generation prevents. Found by tripping it; it now carries forward what it does not author and fails loudly. `gate_check.py` 229 → **238**. ✅ **all four owed items paid** — the mapping exists (`profiles.AZ.rooms.mapping`, 18 rows, 162 gates), the room names turned out to be **in AzDTN 2.7-2's own text in this repo** (14 of 18 `verified`), the three-into-one gap resolved by **keeping three** (the norm carries `hol` and `dəhliz`; `giriş holu` is ours and labelled), and the corpus medians are recorded with their tail warning — *Two room vocabularies in one file*. ✅ its thickness is measured-vindicated: 150 lands **4 mm from the corpus-optimal 146**. ⚠️ **The merged 7,58 m² hall/lobby/corridor median can default nothing** now the three stay apart — rung 2 is empty for all three. ⚠️ One resolution step — `(type, otaq_count) → target, width, name` — is **named in no spec**; handed to `brief.md`'s holder ✅ **The kitchen-dining zone is settled and the fix was a schema, not a value** — [A zone floor is posted on the whole room](tickets/70-a-zone-floor-is-posted-on-the-whole-room.md), ADR 0034. 6,0 stays and is now `referent: part` — a **sound lower bound**, zero false refusals — while the *soft* tier is refused it and falls to **18,8**. The `bridge` prose that recorded the defect and that nothing read is now typed data on every `az_area` guard entry. ⚠️ **The norm's ordering is inverted in the hard set on purpose**: AzDTN floors an unzoned `mətbəx` at 8,0 and the engine floors the larger zoned room at 6,0, because all three candidate whole-room numbers are `engine_choice` and this tier is 13-for-13 `verified`. The **target** carries the ordering, and *the profile matches its own source* now has a gate that can fail if it ever stops. ⚠️ **Two of this row's own citations were wrong and one of its own values may be**: six `clear_widths_mm` cells cited `cl. 5.4` (foundations) for text in `cl. 5.1` — **corrected**; `dim.aspect_ratio_hard` says no source states an aspect rule and AzDTN 2.7-3 cl. 5.1 states one, **2:1**, raised as [A regulator states an aspect rule](tickets/72-a-regulator-states-an-aspect-rule-and-the-engine-says-none-does.md); and the whole `market_default` tier was checkable against 318 Baku plans that were thought not to exist — ✅ **it has been read, and the row's biggest number moved**: [The market tier has an Azerbaijani source now](tickets/73-the-market-tier-has-an-azerbaijani-source-now.md), ADR 0035. Four cells re-fitted up (`living_room_2plus` **17,6**, `bedroom_double` **13,2**, `bedroom_single` **11,5**, `wc` **2,1** new), each keeping its AzDTN value under `superseded_by_measurement`. ⚠️ **`bathroom_combined` 3,8 was WEIGHED AND REFUSED** — a ticket lowering it is overturning a decision taken with the contradiction in hand. The tier's definition and its contents finally agree, and the **monotone-upward rule** now governs every market reading: MİDA may raise a cell and never lower one, because the state fund's sample is biased low. ⚠️ **Its own consequence is unowned by this row**: raising a target raises `dim.max_area`'s hard cap, which C14 does not cover — that is **71**'s. ⚠️ **And a fixture norm now contradicts a shipped TYPE**: AzDTN 2.13-1 cl. 8.31 puts a gas hob only in a `mətbəx otağı`, which a `living_dining_kitchen` is not — [A gas hob decides whether the open-plan type is buildable in AZ](tickets/74-a-gas-hob-decides-whether-the-open-plan-type-is-buildable.md), the first time this profile would carry a fixture consequence rather than a dimensional one. ⚠️ **And the profile is not bound to its own consumers** — [The law is a hand copy and it now shapes rooms](tickets/69-the-law-is-a-hand-copy-and-it-now-shapes-rooms.md), which had **no row on this table until 2026-08-30** and is the failure the done-test exists to catch, arriving in the done-test itself. It now also owns ADR 0034's four referent gates — including the load-bearing one, *every `part` read has a target* — plus `MARKET["KITCHEN_DINING"]` 6,0 → 18,8 and an **eighth** hand copy, `fit_warp.MIN_SIDE` missing `KITCHEN_DINING`. ✅ **The open-plan type ships, disclosed rather than enforced — and this table's own entailed floor for it was never licensed** — [A gas hob decides whether the open-plan type is buildable in AZ](tickets/74-a-gas-hob-decides-whether-the-open-plan-type-is-buildable.md), ADR 0036. AzDTN 2.13-1 cl. 8.31 puts the gas hob in a `mətbəx otağı` and cl. 4.7 files a kitchen-**diner** inside that word, so `kitchen_dining` is compliant and `living_dining_kitchen` needs an electric hob — **recorded in both rows, because the absence of a note is unreadable when two types are one letter apart**. ⚠️ **ADR 0034 decision 4 states its licence and breaks it in the same decision**: an entailed sum needs *"the norm's own type definition"* and AzDTN defines **no open-plan type**, so neither the `compose_with` nor the underlying `part` read was licensed. The 21,0 / 22,0 it produced sat **above all five** real Baku open-plan rooms (15,14–17,74, p50 **17,37**) against a soundness rule reading *"never rejects a room the law admits"* — and ADR 0034's own consequence 1 quotes 17,37 four paragraphs before consequence 2 posts 21,0. `az_area` → **`null`**, the **eleventh** silent key; floor falls to the ergonomic **8,5**. ⚠️ **First withdrawal of a profile raise on this map, and C14 is not weakened** — the base is the ergonomic minimum and this returns to it; a raise on an ungranted read was never a valid exercise of C14. **Reuse the reasoning, not the precedent.** ⚠️ **`compose_with` now licenses nothing on this file** and that is correct: its only qualifying type has one AZ-floored zone. `referent_model.counts_at_authoring` **15 → 13** entries, **12 → 11** rows, part **3 → 1**; `gate_check.py` 238 → **235**, all three the removed guard entries, no named gate lost. ⚠️ **Rung 2 for the type is Swiss at 36,5 and runs 2,1× the only AZ measurement that exists** — recorded on the cell, **not fixed**, because the tier is ADR 0035's and n = 5 is thin ✅ **The profile is now bound to its own consumers, and the binding was already broken when the ticket was taken** — [The law is a hand copy and it now shapes rooms](tickets/69-the-law-is-a-hand-copy-and-it-now-shapes-rooms.md), ADR 0037. This row's own numbers were being read through **eight hand copies** and three were wrong: `MARKET` four cells behind ADR 0035 **one day** after it landed, `HABITABLE` omitting `DINING` so a living room took the 15,0 one-otaq floor against the guard's 16,0, and `MIN_SIDE` missing `KITCHEN_DINING`. ⚠️ **The row publishes numbers in a vocabulary its consumers do not speak**: they key on *corpus label*, this file on *ergonomic key*, and the bridge between them existed only in Python comments — ticket 31's defect one vocabulary upstream, now `ergonomic.corpus_label_map`. ✅ **All five referent gates land** and the licence is a **field** naming its clause; a gate over `compose_with` alone would have passed vacuously forever. `gate_check.py` 235 → **384**, **15/15 mutations caught**. ⚠️ **`kitchen_zone_in_diner`'s `market_default` KEEPS its 6,0** — first-hand cl. 5.1 provenance, and the forbidden read is stopped at the resolution rather than by destroying the datum. ⚠️ **`gate_curve.K` / `room_area_spread.K` are a ninth copy**, of `rules.json`'s `area_bands`, constraining no geometry and handed to 71/72 |
| Drawing — graphics, chains, schedules, tags, sheet, Drawing check | settled | ✅ **the seam is closed and it took one line to close it** — [The annotation spec is US-shaped and the drawing is now Azerbaijani](tickets/32-the-annotation-spec-is-us-shaped.md), ADR 0024: *a sheet mark is read on paper by a builder, a layer name is read on import by a program*. Sheet marks, abbreviations, the mark scheme and the separator all move to SPDS; **the layer names deliberately do not**, and the profile now carries a **test for membership** rather than a new object — *a field is region-parameterised iff a person reads it* — which is what the ticket's *do not add fields one at a time* actually needed. ✅ **ADR 0004's centreline number is dead and ADR 0010's replacement was narrowed**: tier 1 measures the **outer face of an exterior edge and the inner face of a party edge**, because *inner ring on every edge* would have made tier 1 restate the span every tier-2 chain already closes on **and left the sheet with no external footprint at all**, on a map that ships houses. ⚠️ **The BLOCKING window series exists and ticket 50's shape had to change**: it cannot be *bounded by `gost_11214_86`* because the published widths stop at 21 dm and 50's own p90 requirement is **3,23 m** — the top four members are an engine extension, marked by `published_through`, and above it the schedule prints a dimension string rather than a fabricated GOST mark. ⚠️ **And 50's *splitting buys nothing* is half false** — true when the wall run binds, false when the **catalogue top** binds, and the two were not distinguished. ⚠️ **`openings.md` §6.1 had to be rewritten by a ticket that does not hold it, and its own worked example is why: the shipped spec's example fails the shipped bar** — `living` at ratio **0,250**, nearly twice target because the increment was a whole window; `kitchen` at **0,120**, *below the now-hard floor*, described as surviving on a soft penalty; and `bedroom_single`'s window **omitted entirely**, which `win.habitable_has_window` rejects hard. ⚠️ **Two defects surfaced only because §14 was re-derived at the shipped `t_int` 150** — it had been computed at 100 and could not be patched, because the dilated domain must land on the 250 grid: **the schedule's totals row does not add up** (43,58 exact against 43,59 printed, so every total is now computed from the printed cells and the Drawing check gains a **twelfth** predicate), and **§4.5's setting-out datum names opposite ends on every non-minimal run**, with a value that is **100 mm for every internal door in every plan**. ✅ **A first-hand clause that had been dropped is landed rather than handed on a third time**: `AZS ГОСТ 21.501-2010` cl. 2.3.2 annotates a residential plan's area as a **fraction, living over useful** — 17 closed on `ümumi sahə` as a single number and the clause was recorded nowhere. ⚠️ **`faydalı sahə` and `ümumi sahə` are numerically identical in v1 and are not the same quantity**; they diverge the day a balcony is modelled. ✅ **The room tag's fallback splits by audience** and its invented-abbreviation ladder step is replaced by a schedule reference sourced twice over. ⚠️ **26's 1 650 mm bedroom was refused, and the defect it names is real**: the ergonomic layer is a *fits* floor by construction and raising it rejects 19,3 % of real rooms, but **a 1 850 × 5 400 bedroom passes every hard rule** and the number that would stop it — `market_default` 3 000 — **ships and is read by nobody**, because the only rule consuming that tier is an *area* term. `dim.prefer_wide_habitable` is owed to `rules.json` ✅ **General note 9, and it is the list's first CONDITIONAL note** — [A gas hob decides whether the open-plan type is buildable in AZ](tickets/74-a-gas-hob-decides-whether-the-open-plan-type-is-buildable.md), ADR 0036. Emitted only where the Plan holds a `living_dining_kitchen`, suppressed for `kitchen_dining`: *«Bu mətbəx qonaq otağına açıqdır; **elektrik plitəsi** nəzərdə tutulmalıdır.»* The condition is a **Room type**, where every other note conditions on a region or an Envelope edge. It earns the sheet for note 8's reason turned around — note 8 says what the drawing does not claim; note 9 says the one thing about this Plan a Baku builder **cannot read off the geometry**, because AzDTN 2.13-1 cl. 8.31 binds the room's **category** and not its size, so no dimension carries it. It states a consequence and makes **no compliance claim**; note 8 still governs. ⚠️ **This file was unclaimed and 74 declared it on resolution** — 36's drafted general note is **still owed here** and the two do not conflict |
| **Brief and parsing contract** — the object a prompt becomes, and per C4 the real interface | settled | `docs/spec/brief.md`. ✅ **§9.4 is eight bounds and §3 is nineteen types** — *A dwelling with no toilet passes every check*, ADR 0022. Bound 8 is the mandatory-room check and it **inverts ADR 0015**: `acceptance-bar.md` §13's rules are brief-scope with no plan-side twin, so the bound *is* the rule and its severities are corpus-chosen, not inherited. §9.1 gains a fourth hard Brief error. ⚠️ **It also creates the map's first self-contradicting refusal**: adding the `wc` bound 8 asks for raises Σ minima, adds an engine room, and can push a Brief out of C13's 3–10 — a nine-room Brief with no toilet is told to add a room *and* told it may not, and nothing orders the two sentences. ✅ its **band** now has numbers, and ✅ **§9.4's upper half is closed**: six bounds, one function, and **no severity chosen** — ADR 0015 makes a parse-time bound inherit the severity and threshold of the validator rule it is the pre-image of. The Envelope-bigger-than-programme case is a **hard refusal naming two edits**, and the stated-Brief contradiction is caught net-versus-net at the 5 % `area.invented_envelope_hard` already ships. ✅ `resolve` invents **exactly one `hall`**, sourced from AzDTN cl. 5.2. ✅ **Bound 6 no longer rests on a point estimate** — *The partition footprint has a mean and no spread* measured the spread and **wrote it in rather than handing it on again**, because `brief.md` had no claimant and a second handoff would have recreated the defect that created that ticket. It came back **wider and differently shaped than asked for**: `f_hi`/`f_lo` are an **eight-row table over engine room count**, not two constants — ρ = +0.379, median 4.30 % at four rooms against 6.37 % at ten, so pooling excuses a four-room Brief with eight-room partition density — and `f_hi` ships at **p99, not p95**, because a too-low `f_hi` refuses a buildable Brief while a too-high one only sends a doomed Brief to a solve that explains it correctly. ✅ And the **5.7 % reproduced at 5.71 % on a disjoint, unconditioned sample**. ⚠️ What replaces the limit is smaller: `f_hi` restores ADR 0015's implication **empirically, not provably** — it is a p99 of *corpus* dwellings, and the engine's own reachable maximum has never been measured because no Proposer has been run. ⚠️ **Two of eighteen room types are now dead paths** the data still presents as live — `corridor` and `entrance_lobby` need `reachable_in_v1: false`, and this ticket could not write that file. ✅ **§5 is rewritten and §9.4 is now seven bounds** — *What shape an Envelope is when the Brief does not say*, ADR 0020: `shape` **leaves the `ResolvedBrief`** (a dense object had nowhere to put *unknown*, and both candidate defaults are refused), new §5.1 and §5.2 carry the gate term and the pool invariant, and bound 7 is the **only bound with no pre-image in either direction** — a stated shape warns and never refuses, falling through to source B when the pool empties, because refusing would decline a request the engine *can* serve. ⚠️ It leaves two limits: the derived box can differ **30 %** between two candidates that agree on floor to the millimetre, and **every shape number in §5.1 is Swiss and is the *conversion's* notch, not the building's** ✅ **§5 rung 1 does not under-deliver, and `f` is vindicated** — [The sizing rung under-delivers by four per cent, and `f` is not where to fix it](tickets/56-the-sizing-rung-under-delivers-on-the-warp-path.md). The ~4,2 % was **two defects in how the rig measured the Envelope** and neither was in this file: it eroded a 75 mm ring ADR 0001 does not lose (**3,7 % of `interior` at p50**), and it let the warp resize the notch, which ADR 0020's by-construction guarantee assumes it cannot (**1,5 %**). Corrected, Σ Space lands **+0,4 %** of `target_area` and `f = 0.0575` is untouched — the widening the ticket was raised to make would have oversized every Envelope on **both** proposer paths to compensate for an erosion the engine does not perform. ✅ **The `interior` reading is settled and was never a coin-flip**: the Envelope's own area at the finished inner face, because `CONTEXT.md` defines the solve domain as *"not the Envelope, and not the interior"* and `s` is a share of the **Envelope's** bbox. The domain is a **third** quantity, derived from the box, and new **§5.3** carries the three-plane table and the statement that it is source-independent. ⚠️ **What moved instead is `proposer.md` §2.2.3's, and 53 holds it**: *"the notch warps along with everything else, for free"* is what makes ADR 0020's guarantee false, worth **5,6 points** of plan-level `dim.statutory_min_area`. ⚠️ Rung 1's `f` had also been published with **no denominator** — *"the p50 of Σ Space area"*, which is not a quantity — now fixed at source. ✅ **§7.1 is new, and it is a disclosure that is deliberately NOT an Assumption** — [A gas hob decides whether the open-plan type is buildable in AZ](tickets/74-a-gas-hob-decides-whether-the-open-plan-type-is-buildable.md), ADR 0036. A Brief naming `living_dining_kitchen` is **served**, and shown one sentence naming **both** ways out — an electric hob, or a `kitchen_dining` instead — because an architect asked this in Baku gives two answers, not a footnote. ⚠️ **The Assumption channel was closed to it and §1 is why**: the set is `ResolvedBrief \ StatedBrief`, *computed*, so a **stated** type invents nothing and reinterprets no value of theirs; none of §6's three kinds reaches it and §6 closes the taxonomy — *"There is no fourth kind."* A fourth was **refused**, on the ground that an Assumption is something *we* filled in and this is a **consequence of what they asked for**. §7 already houses a refusal and a warning that are not Assumptions either, which is why it is the right home. ✅ **§3's nineteen types are unchanged** and gain a paragraph naming which of the two open-plan types is gas-compliant — `openings.md` §5 already reasoned from *"a gas hob is the Baku norm"* when deciding which openings carry a leaf, and this is the same fact reaching the type list |
...
**Six** artifacts have more than one claimant, and the table below is the whole
write-set of the map, most of it historical. Read it as a **conflict map, not an
order** — the done-test decides order. Live conflicts: `experiments/warp/`
(**62, 65, 67**), `docs/spec/proposer.md` (**67, 81**), `data/acceptance/rules.json` (**72, 76**),
`docs/research/room-area-bands.md` (**72, 76**), `docs/spec/homeowner-surface.md`
(**45, 75**) and `data/standards/room-constraints.json` (**72, 80**).

⚠️ **The count went five → six at 66's closure, and it did so while a file was
being *released*.** 66 held `docs/spec/proposer.md` and `docs/research/zoning.md`
and closed both; `zoning.md` fell to no claimant, but the two tickets 66 raised
re-took one artifact each and one of them was a **single**-claimant file. So
`proposer.md` stands at two (67 and **81**, where it was 66 and 67), and
`room-constraints.json` rose from one to two (72 and **80**). A closure that
narrows nothing is the normal case on this map; a closure that *widens* the table
has to say which artifact and why, and this is the first one to do it.
`experiments/zoning/` is new to the table at a single claimant (80).

⚠️ Recomputed by one pass over `writes:` in all **ten** open tickets — 45, 62, 65,
67, 72, 75, 76, 79, 80, 81 — not edited by hand, per the rule the 45/75 miss
established below.

⚠️ **The count said four and `homeowner-surface.md`'s row said “45 — sole
claimant”; both were wrong, and 77 corrected them in passing off the tickets' own
frontmatter.** 75 declares that file and has since it was raised by 71 — so *A
request and a result in one typeface* and *The room-count promise has Azerbaijani
evidence now* have been simultaneously takeable, on one file, for as long as both
have been open. That is precisely the failure this rule exists to stop, and it was
invisible because the row was read rather than recomputed. The audit is one pass
over `writes:` in every open ticket and it should be re-run at every closure, not
edited by hand.

*(69 closed and **narrowed `experiments/warp/` from four claimants to three** — 62, 65 and 67 remain, and none of them is disturbed in the way its handoff promised: `STAT_FLOOR`'s values held, but the floor a dwelling *gets* moved on 59 dwellings and every `market`-arm target moved up. It **declared three artifacts on resolution**, all unclaimed in assignment: `data/standards/room-constraints.json` — which **72 lists**, so it was held to 69's own subject alone, the bridge block and the `licence` field, with no citation repair and no value edit — `CONTEXT.md`, and ADR 0037. It **raised no ticket** and deliberately did not open `data/acceptance/rules.json` even though `gate_curve.K` and `room_area_spread.K` duplicate its `area_bands`; that ninth copy is handed to 71 and 72 as prose, exactly as 73 and 74 handed theirs on.)*
*(77 closed and **left the write-set exactly where it found it**, which is the second closure running to do that and for the same structural reason. It held `experiments/plane-accounting/` — a directory it created — and `docs/research/solver-formulation.md` as sole claimant of each, and **the ticket it raised, 78, re-took both**, so each stands at one exactly as before. The four live conflicts are untouched: 77 deliberately took **neither** `experiments/warp/` (62, 65, 67) nor `experiments/solver-toy/` (43), because an A/B needs both arms live in one process and subclassing `LayoutProjector` gets that without editing either. It **declared one artifact on resolution**, unclaimed at the time: `docs/spec/acceptance-bar.md` §11.1, where 5,1 % is retired for a measured **1,30 %** and the cap paragraph gains its first measurement. It **created ADR 0040** and did **not** touch `CONTEXT.md`, whose Space plane boundary rule is already correct — an enclosed void is not the Envelope — and did not touch `data/acceptance/rules.json` even though it now holds the measurement `dim.max_area`'s note is missing, because 72 and 76 both claim it; that correction is handed on as prose, exactly as 73 and 74 did.)*

*(66 closed and **is the first closure on this map to widen the table**. It released both artifacts it held — `docs/spec/proposer.md` and `docs/research/zoning.md` — and then the two tickets it raised took one artifact each: **81** re-took `proposer.md`, so that file stands at two (67 and 81) exactly as before, and **80** took `data/standards/room-constraints.json`, which was at **one**. That is the widening, and it is deliberate rather than drift: `is_sleeping` has been handed to that file's holder as **prose twice** — by 30's D2 and again by `zoning.md` §5b — and landed neither time, while `proposer.md` §6.1 terms 1, 2, 3 and 5 and five `zone.*` rules all read it. A third prose handoff would have kept the table narrow and the flag absent. `docs/research/zoning.md` falls to **no claimant** and `experiments/zoning/` is new to the table at one (80). It **created ADR 0042** and declared nothing else on resolution: it did **not** touch `data/acceptance/rules.json` — 72 and 76 both claim it — even though its `zone.*` rows depend on a flag that does not exist, and did not touch `CONTEXT.md`, whose **Sleeping room** entry is already correct and is in fact part of the evidence that the flag is missing from the data rather than from the vocabulary.)*
*(71 closed and **did not narrow the write-set**, which is worth saying plainly because the
last three closures did. It took `data/acceptance/rules.json` and
`docs/research/room-area-bands.md` to one claimant each and then **the ticket it raised, 76,
re-took both**, so each stands at two (72 and 76) exactly as before. That is not drift: the
inversion 76 carries is a *product* of a `k` this ticket published and a target it may not
move, so it could not be resolved from inside an area-band ticket and it could not be left
unowned. It **declared two artifacts on resolution**, both unclaimed in assignment:
`data/standards/room-constraints.json` — which **72 lists**, so it was held strictly to 71's own
subject, one additive block, **124 insertions and 0 deletions**, no citation repair and no value
edit — and `CONTEXT.md`, new term **Area-band class**. It **took two prose handoffs rather than
passing them on again**, because it held the file both were addressed to: 74's false
*"Two AZ reads are `part`"* and 73's `circ.fraction_hard` comparator, neither of which moved a
value. It deliberately did not touch `experiments/acceptance-thresholds/` or
`experiments/region-profile/gate_check.py`, and wrote both owed gate assertions out in full so
they are transcribed rather than re-derived.)*

*(74 closed and **narrowed the map's write-set without widening it anywhere**, which
is the first time a ticket has done that in a while. It took
`data/standards/room-constraints.json` from two claimants to one and `docs/spec/brief.md`
stays at none. It **declared two artifacts on resolution**, both unclaimed at the time:
`docs/spec/annotation.md` — where 36's drafted general note is still owed and does not
conflict — and `docs/research/az-market-default-against-practice.md`, to withdraw a
sentence of its own §6.4. It **raised no ticket**, and it deliberately did not touch
`data/acceptance/rules.json` even though one sentence in it is now false, because 71 and
72 both claim it; that correction is handed on as prose, exactly as 73 did for
`circ.fraction_hard`. Two items went to **69** and one to **75**.)*
*(70 closed and took `CONTEXT.md`, `docs/adr/0033-…` and
`data/standards/room-constraints.json` back to no claimant, then raised 71, 72 and
73, which re-widened two of them. It declared `data/acceptance/rules.json` on
resolution for prose only — unclaimed at the time, so the rule held — and
deliberately left `area_bands` untouched so 71 could have it whole.)*
*(64 closed and declared `CONTEXT.md` on resolution, unclaimed at the time. It
left `experiments/warp/` at four and took `docs/spec/acceptance-bar.md` back to
no claimant, then handed it straight to 68 — the ticket 64 raised over the plane
gap, which is the only ticket permitted to touch that ⚠️.)* *(Ticket 59 closed and declared `experiments/warp/`, which was
unclaimed at the time; 64 is the ticket it raised, and it re-widened both sets to
three.)* *(63 closed and took `docs/spec/proposer.md` back to a single claimant;
65 is the ticket it raised, and it keeps `experiments/warp/` at three.)*
*(43 closed without writing either of the artifacts it held, and re-widened
`docs/spec/proposer.md` to two: 66 is the ticket it raised.)* *(The count said six and had
been stale for some time; corrected by ticket 60, which changed both sets.)*

*(73 closed and **narrowed one conflict while opening another**. It took `data/standards/room-constraints.json` and `docs/research/az-market-default-against-practice.md` back to no claimant — but the ticket it raised over the gas hob, **74**, needs the JSON again, so that artifact stays at two (72 and 74) rather than falling to one. It **declared nothing on resolution**: `data/acceptance/rules.json` was open in front of it and it did not touch it, because 71 and 72 both claim it, so the `circ.fraction_hard` comparator it measured is handed on as prose. It likewise left the six `clear_widths_mm` `cl. 5.4` → `cl. 5.1` citation repairs alone even with the file open — those are **72's**. Its second raised ticket, **75**, opens `docs/adr/0013-…` and `docs/spec/homeowner-surface.md`, both of which were unclaimed; `homeowner-surface.md` now has two (45 and 75) and they do not overlap in subject.)*
*(68 closed and **narrowed the write-set without widening it anywhere except into a new directory**. It took `docs/spec/acceptance-bar.md` back to **no claimant** — the file 64 handed it and the only one permitted to touch §11.1 — and **declared two artifacts on resolution**, both unclaimed at the time: `docs/research/solver-formulation.md`, Part VII, and `CONTEXT.md`, where **Space plane** is retired from two quantities to one. It **took ticket 70’s prose handoff rather than passing it on again**, because it held the file the handoff was addressed to. The ticket it raised, **77**, takes a **new** directory rather than `experiments/solver-toy/` or `experiments/warp/`, so no live conflict moved: `experiments/warp/` stands at three (62, 65, 67) and `docs/spec/proposer.md` at two (66, 67), exactly as before. It deliberately did not touch `data/acceptance/rules.json` — two claimants — even though `dim.max_area`’s note is now false, and wrote **no code anywhere**, per C1.)*

| Artifact | Claimed by |
|---|---|
| `CONTEXT.md` | **no claimant — 68, 69 and 74 all declared it on resolution; 73 closed**. ✅ **68 declared it and the term it touched is now retired to one quantity**: **Space plane** named the **bar plane** and the **solver plane** as two live clear areas; ADR 0039 makes the second a closed defect rather than a distinction. The *avoid* note is kept — both were clear areas, both defensible on their own terms, which is why the unnamed version shipped one component strictly stricter than the rule it posted and one, `dim.max_area`, strictly looser. **69 declared it**: new term **Corpus label**, the third room vocabulary — the one **Room type** already named as a projection and nothing published, so it lived in the comments of four Python dicts and each was free to disagree with the profile and with the others. **Statutory floor** gains the licence-is-data clause (ADR 0037): the licence was prose in an ADR for one day, long enough for the rule to be stated and broken in the same decision, and the *sum* a read composes turned out to be the narrower half. **Room type** gains *every projection is now a published object, and the last one was not*. Before it, and declared it: new term **Target area**, because the glossary used *"preferred area"* three times as a defined term and defined it nowhere, while the profile calls the same concept `market_default`, `tier_model` calls it `soft_objective_target` and the Brief calls it `target_area` — **one concept, four names, and the glossary's own name was the only one with no entry.** It carries that a target is a **band and not a floor**, the rung ladder, and ADR 0035's *measured practice floored by regulator recommendation* plus the monotone rule in domain terms. Before it, 70 closed and declared it: **Statutory floor** gains the part-referent clause (ADR 0034) and *Hard area floor stays two limbs*. Before it, 61, 58, 53, 32, 31, 38, 23, 44, 48, 16, 42, 56 and 36 closed. **61 declared it**: **Enclosed void** is **renamed and widened** to **Void** — *floor inside the Envelope that no Room covers, every complement component other than the notch spans* — because the enclosure test was a proxy that fails at the frame border, and **27,2 %** of donors carry a third boundary-touching component (p50 1,25 m², 89,7 % perfectly rectangular, seated at a corner or edge distinct from the first two). The old term's numbers survive as the **enclosed slice** and are flagged as the population every published purity and amplification figure was measured on. **Notch** loses the `_Avoid_` that split the two at the frame's border and gains two: the line is the **notch spans**, and `s` read off the parts complement runs **+0,0191** mean above the Envelope's own share — about **1,9 m² of invented notch** on a 90 m² dwelling. **58 declared it**: **Real boundary** is a new term — *a converted dwelling's own outline on the 250 mm solve grid* — carrying two `_Avoid_`s, that it is not a drop-in fixture (**96 %** of real dwellings have an Envelope part no room fits, so the blocker is upstream of the solver) and that a converted dwelling's recorded rectangles are **not** its own boundary's ground truth, because they are fitted to a superset of it. **Representable** gains the half nobody had measured: it is mostly an **outline** property, not a room-shape one — a real interior needs **six** rectangles at the median where the notch family yields one to four — with an `_Avoid_` on reading a corpus-*fitted* Envelope as a real boundary, since three matched moments do not carry the fourth. **Witness** gains the sharpest one: the drawing sense on this file and the **solver-harness** sense in `solver-toy/` and `solver-formulation.md` *share no referent*, and both ship. **53 declared it**: **Enclosed void** is a new term — *floor inside a converted dwelling that no Room's rectangles cover and that touches no edge of the frame*, on **15.49 %** of the index, **2.0 %** of it a real duct and the rest the residue of ADR 0014's two-rectangle cap. **Notch** gains the `_Avoid_` that matters most on this map: reading a notch and a void as one quantity, which `uncovered` in a fit record **does**, and which is why neither was noticed — they are held in **opposite directions**, the notch pinned at the share the box was derived from and the void charged to a Room and pushed down. **Proposal** gains its **membership test** — a property enters the contract when the solver can derive it from neither the `ResolvedBrief` nor the boxes — with an `_Avoid_` on *"the solver can work it out from the boxes"*: the objective is L1 corner displacement, so every bordering Room's repair costs the same and the best derivable rule is right **28.4 %** of the time. **36 declared it**: **Wall weight** is a new term — *how many distinct cut-wall thicknesses a sheet draws; v1 draws two, envelope and internal, and never three* — carrying the fact that the 76,1 % three-weight figure is measured on **surveyed built** dwellings and is a working-drawing property, with an `_Avoid_` on blaming uniform `t_int` (the shipped 150 is 4 mm off the corpus-optimal single value; the uniformity is downstream of having no structural model at all). **Wall** gains an `_Avoid_` on **inferring `load_bearing` from thickness, length or position** — nothing in the pipeline carries it, and a drawn wall weight is read by the person holding the sheet as a structural instruction. **56 declared it**: **Solve domain** gains *derived from the [[Envelope]], per candidate, never equal to it* — `t_int` apart on each axis — with an `_Avoid_` on tiling the Envelope's own box and eroding at its boundary, which charges the dwelling for an external wall that is not there and reads as a sizing error in `brief.md`; *"no special case for perimeter rooms"* is a statement about the **rule**, and the arithmetic still has to be handed the right region. **Partition footprint** gains a second `_Avoid_`: it is the last term in the chain and the easiest to blame, and the two quantities in front of it are both larger and neither is a wall. **32 declared it**: **Plan mark**, **Product designation** and **Sheet set mark** are new and they **replace Type mark**, which conflated a position label with a manufacturing designation — and the plan-to-schedule join key is **(kind, number)**, never the number, because doors and windows number in separate spaces. **Living area / Useful area** is new and carries an `_Avoid_` naming the trap that **useful area and `ümumi sahə` are numerically identical in v1 and are not the same quantity**. **Opening** gains the door/window typing asymmetry: a door takes a catalogue entry, a window takes a fixed height and a **selected width**, because only the second is a function of the room's area. **42 declared it**: **Programme rule** and **Combined sanitary unit** are new terms, **Room type** moves to **nineteen** with an `_Avoid_` on growing the set for a *preference* rather than for a rule that cannot otherwise be stated, and **Auxiliary space** gains what enforces it — four rules for five limbs, the `holl` limb held by construction. **16 declared it**: **Nib**, **Receiving Space** and **Placement order** are new terms, **Swing footprint** gains the fact that its side is the **leaf** width and not the structural opening — 100 mm of relief in every wet room that had been silently spent — and two existing terms are marked `_Avoid_`, **Opening**'s cased-opening claim and **Head datum**'s balcony-lintel reason. **48 declared it too**: **Notch** is a new term carrying the material threshold (≥ 5 % of bbox) and the 12.55 % median, and **Envelope** gains *one area, many boxes* with an `_Avoid_` naming ADR 0018 consequence 3 — *"the Envelope, which every candidate for one Brief shares"* — as **false**. 38 declared it: **Pre-image bound** and **Invented circulation** are new terms, **Acceptance bar** reads *one declaration, three consumers*. **23 declared it too**: **Warp** and **Relation provenance** are new, and **Warp budget**'s old reading — *"before its arrangement stops being a real home's"* — is marked **false**, because a monotone warp cannot damage an arrangement at any budget. **44 declared it too**: **Partition footprint** is a new term, and it exists because the quantity has *two* denominators — a share of Σ Space area, never of the interior — and no quote of the 5.7 % anywhere said which. **51 declared it**: **Frontage budget**, **Frontage reach** and **Borrowed daylight** are new. The first two exist because the daylight property a donor hands over is not the one anybody had been quoting, and **Frontage reach** carries an `_Avoid_` on treating it as *sufficient* — it reads boundary contact, and the conversion cannot tell an exterior edge from a party one. **Borrowed daylight** carries the sharper one: *adjacency is not openness*, and **two documents on this map read one for the other** — a separate kitchen with a **door** onto a windowed living room is not a `taxça-mətbəx`, it is the windowless kitchen cl. 9.12 forbids. **46 declared it too**: ⚠️ ***Dwelling axis* presumed a dwelling has one**, and **4,79 %** of the index does not — the term now says the axis is **assigned by the conversion**, not held by the dwelling, with an `_Avoid_` on reading it as a property. **Frame residual** is added beside it, carrying two `_Avoid_`s: it is not a duplicate of worst-room IoU (5–11 cell-agreement points at fixed IoU), and it gates nothing ✅ **74 sharpened Statutory floor** (ADR 0036) with the clause the term stated only half of: **the licence runs out where the norm has no word for the room, and that governs the `part` read itself and not only the sum.** `kitchen_dining` is licensed (cl. 4.7 files it inside `mətbəx`, definitions supply `«ayrıca zonaları»`); `living_dining_kitchen` is **not**, because AzDTN defines no open-plan type — same licence, opposite results, and the difference is whether the norm **names** the room. New `_Avoid_`: *a `part` read is not the cautious option* — an entailed bound is sound only while licensed, and the unlicensed one it withdrew sat above **all five** real Baku rooms of its kind. The monotone-raise paragraph gains its companion: **withdrawing a raise that was never licensed is not lowering one**, because the base is the [[Ergonomic minimum]] and a cell returning to it is a correction |
| `data/standards/room-constraints.json` | **72 and 80 — two claimants, and the second is new. 69, 74, 73 and 70 closed.** ⚠️ **This row went one → two at 66's closure and that is the map's only *widening*.** 80 takes it to land `is_sleeping`, which `zoning.md` §5b specifies in full and which this file has been handed **as prose twice** without it landing — by 30's D2 and again by §5b. `proposer.md` §6.1 terms 1, 2, 3 and 5 and five `zone.*` rules in `rules.json` all read it, so **four of five plan-quality terms are uncomputable** until it lands; ADR 0042 records that and could not fix it. 66 refused to hand it a third time on the grounds that a mechanism which has failed twice is not one. ⚠️ **80 also carries a sweep 69's could not reach**: `experiments/zoning/measure_zoning.py:33-43` is a **fifth private copy** of the corpus-label projection ADR 0037 published, disagreeing with it on `STUDIO` and mapping neither `OFFICE` nor `KITCHEN_DINING` — latent rather than benign, harmless only because `STUDIO` is absent from the corpus. ✅ **69 declared it on resolution and held itself to its own subject** (ADR 0037): `ergonomic.corpus_label_map`, the corpus-label bridge, and a `licence` field on every `az_area` guard entry — **no citation repair and no value edit**, both of which are 72's. ⚠️ **`kitchen_zone_in_diner.market_default` KEEPS its 6,0**: nulling it would destroy a first-hand cl. 5.1 transcription to fix a *consumer* bug, so ADR 0034's forbidden `part` target is stopped at the **read** instead. ⚠️ **The file was being consumed through eight hand copies and three were wrong** — see the Standards-table row. ✅ **73 re-fitted the `market_default` tier to Baku** (ADR 0035): four cells moved up — `living_room_2plus` **17,6**, `bedroom_double` **13,2**, `bedroom_single` **11,5**, `wc` **2,1** new — each keeping its AzDTN value under a new `superseded_by_measurement` key; a new `sources.az_mida_2026` (`force: informative`, the only vocabulary value that fits a builder's schedule); a new positive `profiles.AZ.market_evidence` block; and `what_could_not_be_obtained.market_practice` **narrowed** to the private/premium market rather than deleted. The false sentence *"no Baku market or MİDA space standard could be obtained"* is **asserted gone from all fifteen cells** — nine areas and six widths — and ⚠️ **the six widths were nearly missed**: the first assertion covered only the areas and a repo-wide re-scan caught them. **No width moved**, because MİDA publishes areas only and is silent on every width; the `cl. 5.4` → `cl. 5.1` citation repair on those same six cells was left alone as **72's**. ⚠️ **`bathroom_combined` 3,8 was refused and the refusal is on the cell** — read its note before re-opening it. ⚠️ **Five cells carry a stated reason MİDA could not reach them**, and the sharpest is `bathroom`: MİDA has no separate `hamam otağı` at all. **Before 73, 70 closed** and left it `mapping.referent_model`, `referent`/`compose_with` on all **fifteen** `az_area` guard entries across twelve rows (10 room, 3 part, 2 undetermined), rung-2 medians for both compound types, the LDK `gas_note`, and six corrected `clear_widths_mm` citations. Before it, 32 closed and discharged all three of 50's items.** ✅ **The BLOCKING window series is written** as `openings.width_series_mm`, `window_for_room` is a **selection** (`erg_key → (height, width series)`) with every derived sill unmoved, the tier binding was landed **at the authoring site** in `build_ergonomic_layer.py` rather than in the JSON, `min_pier_mm` is **250**, the catalogue's `conf` **splits** (dimensions stay `verified`, the *selection* becomes `engine_choice` off `gost_23166_99` cl. 4.9), and the `drawing` block gains `sheet_marks`, `area_annotation`, an audience-split `room_tag_fallback` and — the part that matters for whoever adds a field next — **`what_belongs_in_this_block`**, the membership test ADR 0024 turns on. ⚠️ **26's `bedroom_double.min_clear_short` 1650 was weighed and REFUSED**, and the reason is on 32: it is a fits floor by construction and raising it rejects 19,3 % of real rooms. The defect is a **missing soft rule** and it is `rules.json`'s. *(historical: 16, 31, 42, 50 and 32 all closed)* ⚠️ **50 hands it three items and one of them BLOCKS a rule already posted hard.** (a) **The window width series**, per room family: `win.area_ratio` is now hard and `window_for_room` must select the smallest member satisfying AzDTN cl. 9.13 rather than picking one of three fixed entries. Against the three entries the rule fails **21,20 %** of real dwellings; against a series, **5,39 %** — three quarters of the cost is a catalogue artefact. Measured reach requirement, ready to transcribe: **p90 2,47 m living, 3,23 m `living_dining`, 1,34 m kitchen**. Cover for publishing one is already in this file — `catalogue_may_be_dead` records that `gost_23166_99` cl. 4.9 makes the opening grid a **project decision**, so a series is `engine_choice` bounded by `gost_11214_86` and is *more* defensible than three entries. (b) **The tier binding must follow `rules.json`'s**: `validator_binding.hard_reject_below` scalar `\"ergonomic\"` → list `[\"ergonomic\", \"statutory_floor\"]`, and `statutory_floor_binding` **`\"warn\"` → `\"hard\"`** — the two files contradicted each other and neither binding had a rule behind it. The conformance test asserting both files carry the same *string* must assert the same **list**. (c) **`window_for_room` becomes `erg_key → (height, width series)`**, with the derived Type mark riding to `annotation.md` — same holder, and the mark reads **height-then-width**. All three are also in `rules.json`'s `owed` block so they cannot be lost. ⚠️ **42 declared it on resolution and moved the type set to nineteen**: `bathroom_combined` with an ergonomic row, an AZ mapping row and a reversed `reachable_in_v1`; `bathroom`'s impossible pan-and-basin note struck; `shower_room`'s programme now states that it composes the pan; three `bridge` notes corrected. ⚠️ **A new room type or profile cell must now also come with a `NEW_ROOM_FIELDS` entry in `build_ergonomic_layer.py`**, which used to delete `counts_as_otaq`, `brief_nameable`, `reachable_in_v1`, `counts_as_otaq_sourcing` and `corpus_medians` on every re-run and now refuses to. ⚠️ **26 hands it one number and could not take it**: `bedroom_double.min_clear_short` 1650 realises to **1850 mm**, which is the width 32 already reproduced from the other end as a tag overflowing in a real solved layout. It is a *fits* floor — bed 1350 + body 300 one side — against AZ's 3000 mm market default and a 10 m² statutory area, and it is why H8's arithmetic now clears. ✅ **38's `reachable_in_v1: false` handoff on `corridor` and `entrance_lobby` is discharged** by 16 rather than passed a third time. 16 also restructured `profiles.AZ.openings.catalogue` — every entry now carries `opening_w/h`, `block_w/h`, `leaf_w/h`, `kind`, `glazed` and `placeable_in_v1` rather than a GOST mark string with the sizes in prose — and added `door_for_room`, `window_for_room`, `dimension_derivation` and `min_pier_mm`. ⚠️ **It moved one flag that is not an opening fact**: `kitchen.needs_window` false → **true**, because three shipped places disagreed about the kitchen window and AzDTN cl. 9.12 is `verified` and mandatory. It added `profiles.AZ.rooms.mapping`, `counts_as_otaq`, `brief_nameable` and `ergonomic.corpus_medians`; a new room type or profile cell must now come with a mapping row or `gate_check.py` fails ✅ **74 closed and took `living_dining_kitchen.az_area` to `null`** (ADR 0036) — both `part` limbs and their `compose_with` withdrawn as **unlicensed**: ADR 0034 decision 4 grants an entailed sum only for *"cells whose disjointness the norm's own type definition establishes"* and **AzDTN defines no open-plan type**, so the 21,0 / 22,0 it produced was refusing **all five** real Baku open-plan rooms (p50 17,37) against a soundness rule reading *"never rejects a room the law admits"*. The floor falls to the ergonomic **8,5**; `null` is the state `dim.statutory_min_area`'s own `value_source` already defines and ten keys already occupy — this is the **eleventh**. It also **rewrote `living_dining_kitchen.gas_note`** (the type **ships**, disclosed via `brief.md` §7.1 and `annotation.md` note 9, never enforced) and **added a symmetric `gas_note` to `kitchen_dining`** saying it is gas-compliant, because two types one letter apart cannot be told apart by the absence of a note. `referent_model.counts_at_authoring` re-counted **15 → 13** entries, **12 → 11** rows, part **3 → 1**; `compose_with` now licenses **nothing on this file**, which is correct. ⚠️ **`corpus_medians.living_dining_kitchen` carried a false sentence and now carries the measurement**: 36,5 is Swiss-composed and runs **2,1×** MİDA's 17,37 — recorded, **not fixed**, because the tier is ADR 0035's and n = 5 is thin |
| `data/acceptance/rules.json` | **72 and 76 — still two claimants, and 71 is why.** ⚠️ **68 hands it one correction and could not make it** (ADR 0039 consequence 3): `dim.max_area`’s note claims *“The solver side is FREE: H4 already builds a = w*h … an upper bound on a product tightens propagation”*. It tightened nothing — the cap was posted on `amm`, the **solver plane**, which reads a perimeter Room ~3,9 % **smaller** than the plane the rule is stated on, so the cap is the one rule of the seven where the solver is *lenient*. Handed on as prose exactly as 73 and 74 did. ✅ **71 closed and wrote it** (ADR 0038): `area_bands.classes` goes nine → **eleven**, every class gains `corpus_label` and a `members` **pointer** rather than a copy, `dim.max_area` gains the TWO LIMBS note that closes C14's cap lever, and `dim.stated_target_implausible` widens from a *stated* target to any resolved one. **No ADR 0023 value moved; `rule_count` stays 43.** It also **took the two prose handoffs addressed to this file's holder rather than passing them on**: 74's false *"Two AZ reads are `part`"* and 73's `circ.fraction_hard` AZ comparator (13,2 % against the Swiss-fitted 30 %, **value unmoved** — Baku does not approach that bound). ⚠️ **The count did not fall**, because **76** — the cross-provenance inversion 71 raised and could not resolve — re-took it. ⚠️ **70 declared it on resolution for PROSE ONLY** — `dim.statutory_min_area.value_source` and `dim.market_default_area.note` gain ADR 0034's referent rule — and deliberately did **not** touch `area_bands`, which is 71's whole subject. 32, 50, 20, 42, 26 and 55 all closed before it. ⚠️ **55 also declared `docs/spec/brief.md` and `CONTEXT.md` on resolution**, both unclaimed at the time, so the rule held. `brief.md` gains **bound 9** and loses §9.2's dead *"`statutory_floor` is read by nothing in v1"* sentence; `CONTEXT.md` gains **Hard area floor** and an `_Avoid_` on **Statutory floor**'s *clears by construction*, which is measured and false. A ticket touching `dim.statutory_min_area`'s severity is re-opening a decision taken twice, not filling a gap. ✅ **32 discharged three `owed` items and added one.** The window series, the tier binding and `window_for_room` are done, each with what it cost written into a new `discharged` block rather than deleted — **50's series shape and its *splitting buys nothing* both had to be corrected**, and that correction is recorded where the next reader of the series will find it. Three stale `tier_binding` notes fixed: the two files carry the same **list**, not the same string. ⚠️ **One new item, and it is the map's cleanest unwired number**: `dim.prefer_wide_habitable`, soft — `clear_widths_mm.habitable_room.market_default` is **3 000** and `soft_objective_target` names `market_default`, but the only rule reading that tier is `dim.market_default_area`, an **area** term, so **there is no soft rule on clear width at all** and a 1 850 × 5 400 bedroom passes every hard rule on the file. ✅ **50 took it 42 → 43 and amended two bindings**: `dim.statutory_min_area` (hard, site `both`, `conf: verified`, `corpus_cost` 0.5451 with its denominator written out so nobody reads it as a rejection rate); `win.area_ratio` soft → **hard**, `validator` → **both**, `binds_room_types` added because cl. 9.13 binds living rooms and kitchens and the shipped statement bound every Space; `tier_binding.hard_reject_below` **scalar → list** `[\"ergonomic\", \"statutory_floor\"]`; `region_binding.hard_set_is_region_free` → **false**, monotone. Conformance subset 15 → **17**. A ticket touching a severity is now amending a decision with a published corpus cost. ⚠️ **The `owed` block gained three BLOCKING items, all `room-constraints.json`'s** — the window width series, the tier binding that must follow this file's, and `window_for_room` becoming a selection rather than a map. **50's `win.area_ratio` severity question is closed and it was the last unfitted severity on the file.** ✅ **20 took it from 40 to 42 rules and `ENGINE_CHOICE` from 18 to 9**, added `conf: fitted` (ADR 0023) with `src`/`corpus_cost`/`fitted_by` on every fitted rule, landed `dim.max_area` (hard, **site `both`**, which moves the conformance subset to 15) and `dim.stated_target_implausible`, made `dim.market_default_area` two-sided, and added four blocks: `area_bands`, `envelope_constants`, `rule_count` and **`owed`**. A ticket touching a threshold is now amending a fitted number with a published corpus cost rather than filling a gap. ⚠️ **50's `win.area_ratio` severity question is untouched and is the last unfitted severity on the file.** ✅ **42 added the first four rules whose subject is the programme rather than an entity**, 36 → **40**: a new `item: programme`, a `scope_meanings` block stating why they have no plan-side twin, and a `programme_rules` block carrying cl. 5.2, the four corpus costs and why `resolve` does not invent the missing room. The `both` conformance subset **stays at 14** and cannot grow here. ⚠️ **The locale schema change now spans 40 rules, not 36.** ✅ **26 retired two rules, 38 → 36**, the first retirement on this map: `win.habitable_touches_exterior` and `win.kitchen_windowless`, neither of which could fire. Both sit in a new `retired` block with their statements, so a ticket that expects 38 is reading a stale count. `win.habitable_has_window` is now site `both` and `win.area_ratio`'s statement gained its exterior-face clause. **50 is new and holds that rule's severity.** ⚠️ **16 amended six rules and added none**, on purpose, because `acceptance-bar.md` is claimed twice and adding one would move the 38 count: `circ.potential_reachability` (+400), `open.fits_segment` (face declared **clear**), `open.leading_edge_nib` (justification re-based off AD M accessibility), `win.habitable_has_window` (party-wall hole closed), `win.kitchen_windowless` (annotated unreachable) and `entry.exists` (`entrance_side` required). **Six rule statements now diverge from the prose that publishes them** — the next holder of `acceptance-bar.md` closes that, and it is a price of this rule rather than an oversight. And whichever of them moves first inherits the **message locale** schema change, which **38 has now merged with a second requirement**: `brief.md` §9.4 returns a *set of findings* rather than a verdict, each with a severity, a Brief field and an Azerbaijani message — one schema change, not two, and now **two rules 31 handed over** (cl. 5.2's mandatory room composition, and `kitchen_dining`'s zone-not-room target) and **five 30 handed over**, written out in full at `docs/research/zoning.md` §5b ⚠️ **`dim.statutory_min_area`'s severity is now a live question with a measured price** — [The warp has never been measured against a stated target area](tickets/54-the-warp-has-never-been-measured-against-a-stated-area.md). 50 posted it hard on the argument that a Plan reaching its soft target clears the floor by construction, and named its own reversal trigger: *a hard rule that is too strict is discovered at build time on the first Proposer run.* **That trigger has fired early.** ~~On Briefs whose every target sits on `market_default`, the rule alone costs **31,1 % of candidates** and **6,7 % of Briefs at pool-of-8**~~ — **both superseded, and the rule is about half as expensive as it was posted at**: [The sizing rung under-delivers by four per cent](tickets/56-the-sizing-rung-under-delivers-on-the-warp-path.md) found the *measurement* was carrying the difference, not the rule. Same sample, same seed, both Envelope defects fixed: **25,5 % of candidates** and **3,6 % of Briefs at pool-of-8**, with the passing kitchen's lower quartile clearing by **518 litres** rather than 85 — so *"passing by luck"* no longer describes it, though **17,4 %** of kitchens asked for 9,0 m² are still delivered under 8,0 and no sizing constant reaches that. The severity is [The statutory floor now has a price](tickets/55-does-the-statutory-floor-stay-hard-now-that-it-has-a-price.md), **now unblocked**, and its own ticket carries the corrected table. ⚠️ **Do not read it against the bar's 15,59 %** — one is a predicate's cost on generated candidates, the other the whole hard registry's survival rate on real dwellings. ⚠️ **And do not reach for the 18,8 %** — that was `calib`, which scales the box until Σ Space hits `target_area` and hands the rooms margin the Brief does not entitle them to; a correct Envelope over-delivers by **0,4 %**, not by `calib`'s 2,2 % of slack. |
| `docs/research/room-area-bands.md` | **72 and 76 — two claimants, new to this table.** ✅ **71 closed and wrote §12**, the whole type-to-class map with its ladder, the two composed classes, the `utility` analogy and its retirement condition, the site-clustering sensitivity, and the cross-provenance inversion measured across all seven `k`-limb types. §6.1's table gains the two composed rows and **none of the nine moved**. ⚠️ Its §8 handoff table gains three rows, two of them to directories nobody holds — `experiments/region-profile/` for the two owed gates and `experiments/acceptance-thresholds/` for the private class copies and the `kitchen_dining`-exempt `corpus_cost` re-run. |
| `experiments/region-profile/` | **no claimant — 69 closed.** ✅ It is now the home of `profile_read.py`, the single accessor between the profile and every consumer that sizes a room, and `gate_check.py` goes **235 → 384 gates** — ADR 0034's four owed referent gates, ADR 0036's fifth, the bridge and the resolver. ⚠️ **The gates deliberately do NOT import `experiments/warp/`**: coupling a *profile* gate to `ortools` and `shapely` would make this file fail when the solver toolchain moved, so the rigs are bound by construction instead and one cheap source gate (C1) catches a re-copied literal. ✅ **15/15 data mutations caught**, which is the evidence — this map has retired two rules for being unable to fire |
...
| `docs/spec/acceptance-bar.md` | **no claimant — 78 and 77 both declared it on resolution; 68, 64, 59, 50, 42 and 26 closed before it.** ✅ **77 replaced §11.1's Plan-level number with a measured one** (ADR 0040): **1,30 %**, 4 of 307 INFEASIBLE on the incumbent and **0 of 307** on the bar plane, so every Plan-level refusal on that sample was a plane victim. **5,1 % is retired** — it was measured on a warp that posted nothing *and* on a candidate population ADR 0037 has since moved (1 076 of 2 317 → 1 057 of 2 292). ✅ **The cap paragraph gained a measurement**: `dim.max_area` binds on 10 Rooms of 1 993, every one a BATHROOM, worst 10,2 m² over — §9.3's own defect — and posting it costs +6 constraints and 0 new INFEASIBLE. ⚠️ **The residual paragraph is restated as two-signed** and its `4 × 5625 mm²` demoted to an observed maximum. The escalation, the refused screen and the three steps are still untouched. ✅ **68 rewrote the plane paragraphs and retired 19,5 %** (ADR 0039) and discharged 70's `KITCHEN_DINING` limb handoff, which only that ticket could take. ✅ **78 wrote §11.1's two-part half rather than handing it on** (ADR 0041), because this row said *no claimant* and the correction is four paragraphs the file could not state at one rectangle: the plane generalises by **one term at 13 variables**; the Plan-level refusals at two parts are the **binding site and not the seam** — 5 of 284 (1,76 %), all five rescued by §9.1's own *area binds per Room*, which `room-rectangles/solver_parts.py` does not do; the cap's *10 Rooms of 1 993* is a `--parts=1` population whose twin is **10 of 1 961 and no longer all bathrooms**, one of them a two-part `Z`; and the **15 s headroom starts from 31,7 % of candidates already at the cap**, which is ADR 0014's Design A and not this seam's. The escalation, the refused screen and the three steps are untouched for the third ticket running. |
| `experiments/plane-accounting/`, `docs/research/solver-formulation.md` | **no claimant — 78 closed, and 77 and 68 before it.** ✅ **77 built the encoding and wrote Part VIII** (ADR 0040): it is exact — **11 892 Rooms, worst `|identity − space_m2|` = 0,0 mm²** — and it **fits**, at 2,36× the variables and +16,4 % solve time, with cap exhaustion falling 17 → 16. ⚠️ **Its cost is the first on this map that is detectable against the six-seed spread** (6 of 35 inside), so it may not be reported the way II.1's arithmetic was. ⚠️ **Two of ADR 0039's supporting claims did not survive**: `Envelope.all_faces()` returns an enclosed void's faces and must not be the contact set, and the corner residual is **two-signed** — 5,47 % of Rooms read larger, and the corner term *alone* takes that to 36,43 %. ✅ **78 closed it out at two rectangles and wrote Part IX** (ADR 0041), adding `parts_plane.py`, `selftest_parts.py`, `arms_parts.py`, `seeds_parts.py` and `report_parts.py` — and taking neither `experiments/solver-toy/`, `experiments/warp/` nor `experiments/room-rectangles/`, all three imported read-only, as 77 did not take the first two. The join is **one length and 13 variables**; `corners − reflex` is replaced by one vertex rule exact on **11 740 Rooms at 0,0 mm²**. ⚠️ **The band is 0,300 m² at p50, not the 0,165 this row predicted** — the corpus joins at 8 grid units, not ADR 0014's floor of 4 — and it still converts to **one verdict in 1 961**, because `site: both` re-sizes rather than refuses. Rows are a JSONL append log now, because a whole-list dump per pair lost 88 pairs to one kill. |
| `docs/adr/0039-…`, `docs/adr/0040-…`, `docs/adr/0041-…` | **no claimant — 68 created 0039, 77 created 0040 and 78 created 0041.** The solver reads the bar plane, and ADR 0040 measures it: the encoding **fits** and **decision 6's fallback is not selected**. ✅ 0040 confirms decisions 1–4 on measurement — including decision 4's biconditional literals, which are bought for something, because the cap posted on the solver plane leaves **2** Rooms above the band where the bar plane leaves **0**. ⚠️ It **amends decision 2's face set** (`all_faces()` carries an enclosed void's faces) and **withdraws decision 5's sign claim** — the residual is two-signed and `0,0225 m²` is an observed maximum, not a derived bound. The drop stands: **0 verdicts move on 1 993 Rooms**. ⚠️ 0039 amends **ADR 0001** by restating its erosion rule as a *solver* obligation, and retires ADR 0033 consequence 4's number without touching its decision. ✅ **0041 generalises 0039 by one term and supersedes 0040's residual identity with the one that contains it** — `w = I − nU·[nO ≥ 1]` over a lattice vertex, which reproduces `corners − reflex` at one rectangle and reaches the L's own reflex corner, a flush join end and a diagonal void contact, none of which the pair could state. No decision of 0039 or 0040 moves; **the ticket's suspected "third sign" is not one**. |
| `docs/spec/proposer.md` | **67 and 81 — two claimants; 66 closed, and 64, 63, 61, 59, 60, 53, 51, 23, 47, 56 and 43 before it.** ✅ **66 added §6.1 term 5 and took the section from four terms to five** (ADR 0042) — **entry-depth inversion**, real **17.4 %**, and it is a **defect rate scored against a ceiling** rather than a distribution to match: lower scores better, 0 % is best. It is **stratified** by habitable-room count (12.3 / 21.9 / 15.0 % at 3 / 4 / 5, pooling 6+) and the pooled scalar may not be quoted alone. It ships `src: engine_choice`, `conf: derived`, and §6.1 now **states that it has no statutory backing** — СП 54.13330.2022 cl. 5.6 and AzDTN 2.7-2 cl. 5.9 forbid a bedroom being *on the path*, which is term 3, and neither constrains depth from the entrance; WBS 2015 scores zoning nowhere at all. ⚠️ **What 66 leaves both remaining claimants is a framing sentence that is now false for at least two of five terms.** *"The held-out target is the corpus distribution, not a threshold"* holds for term 1, is **wrong** for term 5, and is **right for term 4 for the opposite reason** — §4.5 rules a landlocked room *"a fact about real housing"*, so a source producing 0 % has failed to learn. **81 owns the per-term audit** and 66 deliberately did not pre-empt it. ⚠️ **Terms 1, 2, 3 and 5 all read `is_sleeping`, which is not in the data** — four of five are uncomputable today, and §6.1 previously read as though four terms shipped. 80 lands the flag. ⚠️ **A possible degeneracy in term 4, flagged not asserted**: §4.5 says the solver posts the frontage budget hard, so `frontage_reach < 1.0` on a generated Plan may be 0 % by construction; pre- or post-solve is unconfirmed and is 81's first check, because it may **retire** the term rather than classify it. ⚠️ **§6.1's five rates now carry one shared C5 statement instead of none**, said once for the section rather than per-term. ⚠️ **This row said “64 and 66” after 64 closed** — the Notes’ live-conflict sentence had it right and the table did not; corrected in passing by 68. ⚠️ **66 is 43's, and 43 could not take it** — the entry-depth gradient is measured, has nowhere to live, and §6.1's fifth term is where D10 puts it; the spec is unchanged meanwhile and `zoning.md` D10 carries the full specification, so nothing is lost if 66 waits. ✅ **63 added the gate's fourth term and amended §2.2, §2.2.1, §2.2.4 and §2.2.7** — `req ≤ 1`, ADR 0032, sound and therefore free. A ticket proposing a `req` cut **below** 1.0 is re-opening a decision with three published grounds against it: 0.7 is a fitted constant, the term stops being sound below 1.0, and the population it wins on is the 50/50 draw §4e exists to correct. A ticket dropping the ±10 %/±15 % pair is *What the fourth gate term is worth at the shipped pool depth*, which owns it. ⚠️ **§2.2.7's decline figures are now upper bounds** and its best-of-*m* table is unchanged, deliberately. ✅ **61 rewrote §2.2.3's split and struck a false sentence, and amended §2.2.1 and §2.2.8** — *"the boundary-touching complement is the **building** and is held at `s`"* failed on both halves. The Envelope is the bbox minus **at most two** notch rectangles (`notches_used` is 2 on 90,16 % and never more) while **37,6 %** of donors need three or more, so everything past the second is **inside the ring** — residue that reached an edge, not building. **The line is the notch spans, not the frame's border.** ⚠️ **What it leaves both remaining claimants**: the box shrinks p50 **1,7 %** / mean **2,2 %** under the re-based `s`, so every `experiments/warp/` number was measured on an over-sized box — and the correction lands with ADR 0028's widened void **or not at all**, because the over-sizing has been paying for the uncovered floor (Σ Space **+0,4 %** today, about **−1,9 %** if `s` moves alone). ✅ **59 wrote §2.2.9 and is done with it** — the warp-to-projection join, and it **refused** the Proposal-level screen it was raised to specify, so §2.2 gains no gate term. ⚠️ **What it leaves both remaining claimants is a re-reading of their own inputs**: 82,0 % of candidates that look starved on the warped rectangles are served by the projection, so a per-candidate figure off this file is *warp* fidelity and not plan yield — that bears on 63's decline rates directly. **63** wants §2.2's gate table itself — 60 measured the two dimensional terms as a **proxy for frame stretch** and a coarse one, so whether they survive as written is now a live question rather than a settled shape. ✅ **60 wrote §2.2.1 and §2.2.7 and is done with them**: all three terms bind, the bucket is not the pool, and the second limit names which pool each figure came off. A ticket touching the gate's *membership* is 63's; a ticket touching what §2.2.1 *says* the gate is is amending settled text. They are independent decisions on one file — an order, not a dependency — so take **one at a time** and let the done-test pick which. ✅ **53 added §2.2.8 and amended §1, §2.2.1, §2.2.2, §2.2.3, §2.2.4, §7 and §8**, so a ticket touching the void, the index record, the pre-rank order, the notch share or the warp objective is amending a settled shape rather than filling a gap. **It discharged both handoffs this file was owed rather than passing them a third time**: 56's §2.2.3 *"for free"* sentence is struck and the notch share is held, and 47's **`worst_room_iou`** is an index field, a hard gate at **0.30** (6.65 %, `conf: fitted`) and a rank above it. ⚠️ **What it leaves is an emitter obligation, not a decision**: `fit_rects.py` owes a **fourth** per-record field — the enclosed void components with their **donor owner** — and `experiments/warp/` owes the void variables plus an output for realised unassigned area the rig has never had. ⚠️ **The constrained notch and the charged void are ONE unmeasured cost** and should be measured once, for both. ⚠️ **56 hands §2.2.3 a sentence that has to change and a measured price for leaving it**: *"the notch is the part of the bbox no part covers — so it **warps along with everything else, for free**"* is what makes ADR 0020's *"every candidate delivers `interior` of floor by construction"* **false**, because the guarantee holds only while the *realised* notch share equals the recorded `s` the box was derived from. The notch is the one region of the frame carrying no target, so it is a free sink: measured, `covered ÷ interior` is **0.9833** free and **0.9986** held — **1,5 % of `interior`**, and **5,6 points** of plan-level `dim.statutory_min_area` (30,5 % → 24,9 %). The constraint is one bilinear equality on the gap variables the room areas already use (realised uncovered area = `s × W × H`), or the fixed point on the box `ring` uses. ⚠️ **This is NOT ADR 0003 consequence 7**, which fixes the *entrance edge* by side and says nothing about the notch's dimensions — the two are compatible and neither implies the other, which is why nothing had caught it. ⚠️ **Nobody has priced the constrained model**: the `ring` arms reach the invariant by re-sizing the box, not by constraining the solve, so its INFEASIBLE cost is unmeasured. ✅ **51 added §4.5 and amended §2.2.1, §2.2.4, §6.1, §7 and §8** — a ticket touching what the corpus is admitted on, the index record, the pre-rank order or the plan-quality terms is now amending a settled shape rather than filling a gap. It **discharged the half of `acceptance-thresholds.md` §13's handoff addressed to it**; **the half addressed to 53 is untouched and still open**. ⚠️ **53 inherits one caution from it**: §4.5's residue is quoted as a **floor** and not a size, because `frontage_reach` reads boundary contact and §2.2.6 says the conversion cannot tell `exterior` from `party` — the same necessary-not-sufficient trap a void gate would sit in. ⚠️ **47 hands §2.2 the whole of its own open question and could not write it**: donor fidelity is in **neither** the index record (eleven fields, none of them fidelity) nor the ranking (§2.2.4 pre-ranks on the *warp's* deviation, a fact about the fit to this Brief and not about whether the donor converted faithfully). Owed in three parts, specified ready to transcribe on ticket 47 — add **worst-room IoU** to the index record (`fit_rects.py` already emits per-room `iou`, no re-fit), **gate hard below 0.30** at a published cost of **6.65 %** of the index and `conf: fitted` rather than `verified`, and **rank on it above the gate** rather than gating at 0.50, which would cost 17.2 % of an index C13 already calls thin. **53 is new**, raised by 42 after it checked and refuted the premise it was handed — `model.no_unassigned_area` is hard, so the enclosed void is a *donor* problem, not the bar's. ~~51 is new and holds the corpus cost of a shipped hard rule — 43.3 % of real dwellings rejected by `win.habitable_has_window`, 23.0 points the kitchen alone.~~ **Closed, and the corpus cost was never this file's to hold**: it is **38.55 %** at index scale, of which **86.04 %** is reglazed by the opening layer, leaving **6.39 %**. It rewrote §2.2 into seven subsections and corrected §1, §4.4 and §5.1, so a ticket touching those is amending a settled shape rather than filling a gap. **48 hands it three things**: §2.2.3's stated-`shape` gate moves from notch **count** to notch **area share** (a material notch is ≥ 5 % of bbox — the count gate reports 90 % of real flats as U/T and 8.7 % as L, where the material reading gives **52.96 % L**); §2.2.1's index record must carry the notch share `s`, and the per-candidate `W × H = interior / (1 − s)` needs a home; and **"fill the notch"** is recorded as a live candidate design that would dissolve the stated-shape coverage cliff at the cost of re-proving ADR 0018's monotone-warp theorem. ⚠️ **46 declared it on resolution and changed a count this file states four times**: §2.2.1's *four new obligations* is now **five** — `frame_residual` joins them — and that same pass now carries the `dwelling_frame` swap, so **the pass may not be split and the conversion is frozen until it runs**. §2.2.4 gains a paragraph recording why a **pure donor fact is deliberately neither gated nor ranked** (no knee, and the existing pre-rank already puts a 4–8° donor at the 10,6th percentile) — without it the next reader reads an oversight. §6.1 gains the map's **only** hard exclusion of off-frame dwellings, and a ⚠️ that its four corpus rates are quoted **unfiltered** and move when the pass lands |
| `docs/spec/annotation.md` | **no claimant — 74 declared it on resolution; 32 and 36 closed.** ⚠️ **36 hands it three items and could not take them, and two of them are corrections to shipped text rather than additions.** (a) **A new general note**, drafted on ADR 0026: *all internal walls are shown as partitions at one thickness; load-bearing walls have not been identified.* **`arakəsmə` is `verified`** — AzDTN's own word at `az-finish-layer.md` cl. 8.24, already used in §8 — but **`yükdaşıyan` is unsourced and must not ship as written**; source it from `azdtn_2_17_1`, which `thickness.md` read first-hand for `t_int_bearing` cl. 6.9. (b) **Strike *"unless noted"* from general note 3** — *"All partitions `t_int` mm unless noted"* promises an exception mechanism the engine does not have, and **nothing is ever noted**. (c) **Add `draw.structural_disclaimer_present`, Drawing check 12 → 13**, on `draw.schedule_totals_close`'s precedent. ⚠️ **Note 7 is the trap**: *"structural performance is not specified"* is a statement about a **calculation**, and the missing claim is **identification** — a reader takes note 7 as *no structural calculations were run* and still assumes the wall drawn heavy is the heavy one. ✅ **31 and 16 both handed it something and both are discharged**: the eighteen Azerbaijani room names are consumed through `profiles.AZ.rooms.mapping.rooms.<key>.name_az` rather than transcribed — §14 uses them and §7 says where they come from — and the door schedule now carries the two-level mark scheme with `Handing` and `Swing` filled from §4.1/§4.2. Twelve sections moved and §14 was **re-derived at `t_int` 150**; a ticket touching the sheet mark, the mark scheme, tier 1, the tag ladder, the schedule totals or the setting-out datum is amending a settled shape rather than filling a gap. ⚠️ **The Drawing check is twelve predicates, not eleven** — `draw.schedule_totals_close`, because a totals row computed from exact areas disagrees with the column printed above it. ⚠️ **`draw.schedule_complete` now joins on `(kind, mark)`**: doors and windows number in separate spaces and a join on the number alone matches door 1 to window 1. ✅ **31 has handed it the eighteen Azerbaijani room names**, sourced and cited. ✅ **16 has filled the door schedule's `Handing` and `Swing` columns** with a stated rule rather than a promise — handing is *derived* from the door's position, so the schedule cell and the plan's swing arc cannot disagree — and hands it one new graphic: `door_living_glazed` is a **glazed** leaf and draws a glazing line where a solid one does not ✅ **74 added general note 9** (ADR 0036), the list's **first conditional** note — emitted only where the Plan holds a `living_dining_kitchen`, suppressed for `kitchen_dining`, conditioning on a **Room type** where every other note conditions on a region or an Envelope edge. ⚠️ **36's three items are untouched and still owed here** — they do not conflict with note 9 |
| `docs/adr/0020-…` | **no claimant — 61 closed; 56 and 60 declared it on resolution.** ✅ **`s` is re-based and it was never the quantity the ADR names** — [The notch is two components and a quarter of donors have more](tickets/61-the-notch-is-two-components-and-a-quarter-of-donors-have-more.md), **second amendment**. `s` is now the **`notches_used` spans'** share of the box, at most two rectangles, and nothing else. It was read off the *parts* complement, so it carried the `envelope_loss` and fit residue adjoining the notch: against the Envelope's own share `1 − bbox_fill × (1 + envelope_loss)` it runs **+0,0153** at p50, **+0,0191** mean, **more than two points high on 38,2 %** of donors. ⚠️ **The cost is the drawn ring, not the floor** — the emitted notch is about **1,9 points of the box larger than any real dwelling's**, ~**1,9 m²** on a 90 m² dwelling, in an edge ADR 0003 makes typed, drawn, dimensioned and exported. **Widening `s` to all touching components was measured and refused**: it moves a further half point *away* from the Envelope's share. ⚠️ **Consequence 3's material-notch table owes a partial re-measure** — `L` 52,96 % and `U`/`T` 25,42 % reproduce within 0,5 points, `rectangular` 15,67 % does not (21,6 %), and its 5,95 % at three-or-more material components **cannot exist** against an Envelope with two. Headline safe; **do not restate it** until the notch spans land. ✅ **57's fork is collapsed**: with `s` on the spans, the spans are the region, and there is no longer a choice between constraining them and constraining all uncovered-minus-void. ⚠️ **60 hands it one correction and could not make it**: `covered ÷ interior` **0,9833** is a **bucket** number, and under the shipped gate it is **0,9853** — so the notch guarantee is worth ~**1,3 %** of `interior` rather than **1,5 %**. The argument is unchanged and **61 did not take it** — its pass rewrote which cells `s` counts and left the bucket-versus-shipped-gate arithmetic untouched, so this correction is still owed and still one edit. ✅ Amendment: **`interior` is the Envelope's own area** at the finished inner face and the solve domain is a **third** quantity derived from the box — the ADR wrote `box = interior/(1 − s)` and never said which plane, and a shipped rig read it the other way. ⚠️ **And the ADR's own guarantee has a precondition it does not state**: *"every candidate delivers `interior` of floor by construction"* holds only while the *realised* notch share equals the recorded `s`, which `proposer.md` §2.2.3 explicitly denies. Worth **1,5 % of `interior`**. `rules.json` sees no change, which is the second time this ADR has ended there |
...
| `docs/spec/brief.md` | **no claimant — 74, 42, 48, 50 and 56 closed.** ⚠️ **50 hands it one bound and could not take it**: §9.4 bounds 1 and 3 must read `max(ergonomic minimum, region statutory floor)` rather than the ergonomic minimum alone, per ADR 0015. Σ hard minima for a one-otaq dwelling goes **9,0 m² → 26,5**, two-otaq 37,5, three 47,5, four 57,5 before the partition footprint — ordinary Baku flat sizes, and **nothing leaves C13's 3–10 band**. The 9,0 m² one-otaq flat the old floor admitted **is** the defect, restated at parse time. The message stays arithmetic — *your Envelope cannot hold n otaq* — never a compliance claim, which is how the rule stays inside C8. ⚠️ **42 declared it and wrote §9.4 bound 8, §3's nineteenth type and §9.1's fourth hard error rather than handing them on** — the same reason 44 refused a second handoff. It discharged §12's composition row and left three: which sentence leads when bound 8 contradicts bounds 1/3/6, a `taxça-mətbəx` type, and a Brief-nameable built-in wardrobe. 38 and 44 closed before it. 38 rewrote §9.4 and added §3.1, so a ticket touching either is amending a settled shape rather than filling a gap. **44 wrote §9.4 bound 6's `f_hi`/`f_lo` table, §5 rung 1's `f`, §12 and §13 into it** rather than handing them on, and it raises one new obligation: the eight-row table is inline prose and belongs in data beside `room-area-bands.md` §6.1's `k`, for `rules.json`'s holder. **31's two are still open**: whether a nineteenth type (`taxça-mətbəx`) is owed, and where the `(type, otaq_count) → target, width, name` resolution step lives. ~~**23's two became 48**~~ — **both discharged.** `shape` now leaves the `ResolvedBrief` entirely rather than getting a default, and the per-candidate Envelope is resolved by making **floor area the pool invariant and the box derived** (ADR 0020). 48 also rewrote §5 into §5/§5.1/§5.2, annotated §6, took §9.4 to **seven** bounds and added five §12 handoffs and two §13 limits, so a ticket touching any of those is amending a settled shape rather than filling a gap ✅ **§5 rung 1 does not under-deliver, and `f` is vindicated** — [The sizing rung under-delivers by four per cent, and `f` is not where to fix it](tickets/56-the-sizing-rung-under-delivers-on-the-warp-path.md). The ~4,2 % was **two defects in how the rig measured the Envelope** and neither was in this file: it eroded a 75 mm ring ADR 0001 does not lose (**3,7 % of `interior` at p50**), and it let the warp resize the notch, which ADR 0020's by-construction guarantee assumes it cannot (**1,5 %**). Corrected, Σ Space lands **+0,4 %** of `target_area` and `f = 0.0575` is untouched — the widening the ticket was raised to make would have oversized every Envelope on **both** proposer paths to compensate for an erosion the engine does not perform. ✅ **The `interior` reading is settled and was never a coin-flip**: the Envelope's own area at the finished inner face, because `CONTEXT.md` defines the solve domain as *"not the Envelope, and not the interior"* and `s` is a share of the **Envelope's** bbox. The domain is a **third** quantity, derived from the box, and new **§5.3** carries the three-plane table and the statement that it is source-independent. ⚠️ **What moved instead is `proposer.md` §2.2.3's, and 53 holds it**: *"the notch warps along with everything else, for free"* is what makes ADR 0020's guarantee false, worth **5,6 points** of plan-level `dim.statutory_min_area`. ⚠️ Rung 1's `f` had also been published with **no denominator** — *"the p50 of Σ Space area"*, which is not a quantity — now fixed at source. ✅ **74 added §7.1 and a §3 paragraph** (ADR 0036). §7.1 is a **disclosure that is deliberately not an Assumption**: the set is `ResolvedBrief \ StatedBrief`, *computed* (§1), so a **stated** type invents nothing and none of §6's three kinds reaches it — §6 closes the taxonomy and **a fourth kind was refused**, because an Assumption is something *we* filled in and this is a consequence of what *they* asked for. §3's nineteen types are **unchanged** |
| `docs/spec/homeowner-surface.md` | **45 and 75 — two claimants, and this row said “sole” until 77 recomputed it.** 75 declares this file and has since 71 raised it; neither ticket may be started while the other is claimed. ⚠️ **32 leaves it two items and could not take them**: §187 offers *the full two-sheet set (`A-101` plan, `A-102` schedules)* and those sheet numbers no longer exist — the set is `<job>-MH`, *Vərəq 1 / 2* (ADR 0024), and `annotation.md` §1 carries the translation so nothing is ambiguous meanwhile. And §2's *numbers use the decimal comma everywhere* now has a **shared implementation** rather than a parallel one: `annotation.md` §1.1 puts every rendered number through one formatter, and the preview is one of its three named call sites. Created by 13, which declared it on resolution rather than taking it quietly; 45 is the first ticket to claim it, and it inherits the **message locale** schema change if it moves before `rules.json`'s holder |
| `experiments/warp/`, `docs/research/proposer-architecture.md` | **62, 65 and 67 claim it — 69, 64, 63, 59, 54, 56, 57 and 60 closed.** ✅ **69 took every profile literal out of it** (ADR 0037): `MIN_SIDE`, `MARKET`, `ERG_AREA`, `COLLAPSE`, `GRID_MM` and `T_INT_MM` are now `profile_read`'s output, built at import, so no call site moved and no copy remains. `_check_floor_transcription` is **deleted** — it asserted the six values that *were* copied and so was blind to a seventh missing by omission. ⚠️ **Its handoff's *undisturbed* is false**: 59 dwellings' living floor moves 15,0 → 16,0 because the otaq guard now resolves, and the `MARKET` re-read moves **43,24 %** of rooms by a mean **+0,561 m²** — so **the `market`-arm re-run is owed to whichever of 62/65/67 goes next**, cheap, and the direction is provable (all cells up, no floor moved) even though the magnitude is not. ✅ **63 left it `stretch_terms.py`** — the frame-requirement DP, joined to `gate_effect.py`'s existing warps, so a new statistic off the gate question costs seconds and no solve. ⚠️ **Three traps, in the README**: quote §4e and never §4b (the rig is 50/50, a real bucket is 82.4 % refused, and the bias runs toward loosening); never quote the `m = 8` block (best-of-3 against best-of-6); and `ext` is a **control**, not a candidate — it is the two scalars in polar coordinates. ✅ **59 left it `project_join.py`** — the first thing on this map to put a warped Proposal through `project()`, with `--report`, `--planes` and a `--selftest` that asserts its warp is `absolute_area.run_one`'s exactly. It imports `solver-toy/` **and** `room-rectangles/` read-only and writes to neither. ⚠️ **Three traps, in the README.** `prop_starved × plan_starved` is not the confusion matrix and building it that way silently returns zeros — the projection *posts* the floor, so refusal is INFEASIBLE; watch `served_but_starved`, which must stay **0**. A refusal must be **attributed by ablation**, never assumed, because `infeasibility_core` only covers the softenable families. And the two rigs measure area on **different planes**, p50 **3,9 %** apart per Room — ticket 56's 75 mm ring, re-found inside `solver.py` where the 250 mm grid makes it unremovable. ✅ **60 left it `gate_effect.py`** (the paired within-Brief split of a bucket into gate-admitted and gate-refused donors) **and `gate_sites.py`** (which branch of the pre-60 gate fired and what it admitted; no warp, no solve) — and it **repaired the rig rather than only documenting it**: `gate_pool` is split into `admissible_pool` and `bucket_pool`, `absolute_area.py` takes `--pooldef=`, `fit_warp.py` takes `--pairing=`, and the six private copies of the gate are one. ⚠️ **Two traps.** `coverage_restated.py`, `gate_curve.py`, `room_area_spread.py` and `pool_fidelity.py` gate against a random same-room-count donor standing in for the Brief — a pre-ADR-0020 convention, **deliberately not unified**, because changing it re-bases **86,6 and 58,7**; the note is at the site. And `bucket_pool` is a **depth** stand-in only — never take a per-candidate statistic off it. `proposer-architecture.md` is unclaimed and 60 declared it for §7.5/§7.6. ✅ **57 left four probes** — `pool_depth.py` (three pool definitions, no solve), `best_of_m.py` (the nested best-of-*m* curve), `best_of_m_fit.py` (the zero-inflated fit) and `constrained_warp.py` (ADR 0020 and ADR 0028 posted **in** the solve for the first time, plus a `--census` mode). ⚠️ **Two traps it adds.** Realised notch and void must be read off the **frame**, never the millimetre geometry — `notch_share` flood-fills one cell per mm², which is ~80 million cells on solved geometry and never returns; use `frame_components` / `realised_frame_areas`. And **do not fit a plain Beta** to the curve: every Beta sends `E[p^m]` to zero, so it returned 0,45 % where 8,2 % was measured. ✅ **`proposer-architecture.md` §7.5 is un-staled** — it had carried pre-56 numbers throughout and its finding 2 headline is struck. ✅ **56 found the rig had been measuring the wrong region and published the correction**: `absolute_area.py` tiled the *Envelope box* and eroded every Room on all four sides, where ADR 0001 tiles the Envelope **dilated by `t_int/2`** and a boundary edge costs no floor — a 75 mm ring, **3,7 % of `interior` at p50**, larger than the level error it was raised to explain. `part_targets_cells` carried the same defect one level up and was **compensating** for it, which is why the fix moves the level a long way and the yield hardly at all. New `outside_of`, `space_m2` and `part_targets_cells` re-based on ADR 0001's plane, and **`ring` / `ringmarket` / `ringpool`**, the arms that hold the notch share the box was derived from. ⚠️ **Read the `ring` row and no other as what the engine delivers** — `calib` scales until Σ Space hits `target_area` and buys the rooms margin the Brief does not entitle them to, and reading it as *what a correct Envelope gives* is what put 2/5 of a hard rule's cost on one constant in `brief.md`. ⚠️ **The pre-56 rows are committed** at `series/absolute_area_pre56_rows.json.gz` (263 KB, five arms), on 44's rule — `out/` is gitignored, so a snapshot in it would have been a local-only claim — and the paired before/after is re-derivable in seconds — and `cross before` reproduces 54's published 30,7 % and `calib before` its 18,8 % exactly, which is how the rig is known to be the same rig. *(historical: 54 closed and discharged both obligations.)* ✅ **The measurement exists and it refutes the argument it was asked about**: `absolute_area.py`, four per-candidate arms plus best-of-pool, un-normalised targets, measured on the **Space** (`erode(⋃ parts, 75)`) rather than the centreline part. **31,1 % of candidates and 6,7 % of Briefs at pool-of-8** lose a Room below `dim.statutory_min_area` even when every target sits on `market_default` — [The warp has never been measured against a stated target area](tickets/54-the-warp-has-never-been-measured-against-a-stated-area.md). ✅ **48's obligation is discharged**: ADR 0018's p50 0.056 is confirmed as a proportion result and the absolute twin is now published. ⚠️ **Three traps recorded in the README**: never quote a `fit_warp` deviation as an area, never compound a per-candidate share into a Brief-level one (780× wrong), and never quote past one decimal — CP-SAT under a wall-clock cap gave 5,96 % and 5,78 % on two identical runs. ⚠️ **It leaves two numbers with unheld files**: `rules.json` owns the severity, and `brief.md` §5 rung 1's `f` is short by ~4,2 % on this path — do **not** fix that inside `f`, which is a correctly measured partition footprint of a different quantity |
| `docs/research/zoning.md`, `experiments/zoning/` | **`zoning.md` no claimant — 66 closed and released it; `experiments/zoning/` now has one (80). 43 declared both on resolution, unclaimed at the time; 30 created them.** ✅ **66 answered D10 and amended it in three places rather than merely taking it** (ADR 0042): the corpus rate is a **ceiling, not a target**, because §6.5 calls the − bucket a *violation* while D10 asked to match it; the social set needs **no second flag**, being `is_habitable` true and `is_sleeping` false, the construction two `zone.*` rules already use; and the pooled 17.4 % **must never be quoted alone**, because the tie mass runs 30.8 → 67.3 % monotone in habitable-room count and the three-bucket split is a mixture over the corpus size mix. It also amended **§6.7** — no *regulator* scores this either, in either lineage — and **§6.8**, whose *"not plausibly region-specific"* was argued for a rule and must not be read across to a scored term. ⚠️ **A ticket touching D10, §6.7 or §6.8 is now amending a settled shape rather than filling a gap.** ⚠️ **`experiments/zoning/` is no longer safe to treat as unclaimed**: 80 holds it for the private-copy sweep. ✅ **43 added §6, D10 and the directory's first README**, plus three probes (`entry_order.py`, `entry_order2.py`, `entry_depth_vs_transit.py`) that read the existing `zoning.json`/`zoning2.json` and run **no corpus pass** — so a new statistic about the entry sequence costs seconds. It imports `rectangularise/` read-only and edits nothing outside itself. ⚠️ **Six traps in the README, and the first is the one that will bite**: **social transit has three rates with three denominators** — **11,1 %** per sleeping Room (`proposer.md` §6.1 term 3), **18,2 %** per dwelling of 2 500, **25,9 %** per dwelling holding both classes — never put two in one sentence. ⚠️ **And the gradient has two**: **16,1 %** mean-against-mean is its *shape*, **17,4 %** min-against-min is what a *rule* would cost, and neither may be quoted without **51,0 %**, the tie mass that decided the ticket. ⚠️ **§2.2, §2.3 and D7 were amended in place** — D7's verdict stands, its stated reason (*cost unmeasured*) was never reached |
| `experiments/void/` | **new, no claimant — 53 created it.** Seven probes and a README; it imports `rectangularise/` and `warp/` and **edits neither**, the arrangement `envelope-exposure/` and `h8-frontage/` already use — both of those directories are declared by open tickets (46 and 57), so a probe in either would have been a merge hazard. ⚠️ **Four traps in the README**, and the first two are the ones that will bite: never quote a `void_census.py` figure beside a `parts_census.py` one — different regions against different references — and never read `treatment.py`'s `charged` row as a regression, because it reports a *worse* deviation only by measuring what the Room will actually hold. `is_it_a_duct.py` re-parses the 1.09 GB CSV; everything else costs seconds off `swiss_fit_k2.json` |
...
| `experiments/solver-toy/` | **43 — sole claimant again; 58 and 52 are closed, 29 before them.** ✅ **58 left it `real_arm.py` (the three-rung ladder, `results/REAL_ARM.jsonl`) and `real_typing.py`** (which constraint refuses a real dwelling a Brief — no solver, one second). ⚠️ **Its finding is about the harness, not the solver**: `ground_truth` cannot dissect a **per-dwelling** Envelope even inside ADR 0003's family (70 of 120 slots lost), and `assign_kinds` refuses a real dwelling on the **programme** — `COMPOSITION` wants a median 5 habitable Rooms against a median 4 cells both exterior-facing and habitable-sized. Both are the **toy's** placeholders and neither may be quoted as `room-constraints.json`'s cost. Previously: **43 and 58 — 52 is closed and 29 before it.** ✅ **52 left the directory two fixtures and a rule for choosing between them** (ADR 0029): `FIXTURE` defaults to `published`, bit-exact, and exists **only** to reproduce Parts I–III / ADR 0014 / ADR 0019 — **every new measurement is taken on `fixture="corpus"`**, which is fitted per count against 2,238 real dwellings and measured to strictly dominate (survivors 68,6 % → 85,7 %, McNemar p = 0,0005, no time cost, no slot lost). New here: `envelope_fit.py`, `fixture_delta.py`, `results/FIXTURE.jsonl`, `geometry.u_shape_true`, and a real-boundary walk in `all_faces()`. ⚠️ **The trap that will bite whoever runs this next**: `clear_t` **must** equal the solver's `t_int_mm` whenever `erode_minima` is on, or the ground truth stops being a witness and the model goes provably untileable — it returned OPTIMAL with 55 cells unassigned at every seed and **inverted 52's headline** from p = 0,0005 to p = 1,00. `sweep_ng.execute` threads it through; copy that. ⚠️ **`AREA_PER_ROOM_M2` stays 9.65 on purpose** and is not a number to correct. Previously: **43 — sole claimant, 29 closed.** The re-base it was waiting for happened and **moved nothing**, so the rig 43 prices its encoding against is the published one. 29 added `pinwheel.py`, `sweep_ng.py`, `report_ng.py`, `relation_margins.py`, `t_int_arithmetic.py`, `pinwheel_area_premium.py`, `corpus_guillotine.py` and left `_guillotine` the **default** on purpose. ⚠️ It also leaves a fixture defect for whoever runs the harness next: `AREA_PER_ROOM_M2` = 9.65 is below what the placeholder table needs at 7 and 8 rooms in **either** arm |
| `docs/adr/0003-…` | **no claimant — 61 and 47 closed.** ⚠️ **61 declared it on resolution for a price the cap's own evidence never carried.** 47 priced the cap as index thinning (6,65 %) and as a refused shape-family widening (4,17 % ceiling); neither is the price a **Room** pays. `envelope_approx` caps at two by leaving the rest **inside** the ring — `notches_needed` is 3 or more on **37,6 %** of donors and `envelope_loss` is p50 **1,78 %** of the domain, p90 **9,92 %**, mean **3,72 %** — and the docstring's *"costs a room nothing"* is true of the **conversion**, scored against the real dwelling, and false of the **engine**, which must tile the ring exactly and hands that floor to whichever Room the objective finds cheapest. **The cap does not move**: two is still the knee and the widening is still refused on measurement. What changed is that the loss it accepts now has an owner downstream (ADR 0028's amendment). ✅ **48's §7 correction is written**, after two holders declined to write blind: consequence 7 now reads *one ring per candidate, fixed before that candidate's solve*, and states the reason it is safe — the entrance edge is identified **by side, never by ring index**. ✅ **A second amendment closes the notch cap**: the cap is evidenced, the shape family is refused at a measured ceiling of 4.17 % of the corpus, and the cap bullet points at it. A ticket touching the cap, the shape family or the entrance edge is amending a settled shape rather than filling a gap |
| `docs/adr/0032-…` | **65 — sole claimant; 63 created it.** The gate's fourth term. ⚠️ **Its every figure is at `m = 3` and the shipped `m` is 8** — consequence 5 says so, and 65 is the ticket that discharges it. A ticket quoting an ADR 0032 number at production depth is quoting a bootstrap that saturates at three distinct warps an arm |
| `docs/adr/0033-…` | **no claimant — 70 closed** and amended it: the `KITCHEN_DINING` limb is **entailed, not transcribed**, and *the floor never fights a target* held for that limb only at **equality**. 64 created it. The warp's statutory floor. ⚠️ **Its `KITCHEN_DINING` limb floors a zone and the constraint is posted on the room** — [A zone floor is posted on the whole room](tickets/70-a-zone-floor-is-posted-on-the-whole-room.md); the referent and the force both drifted, and the profile's own `bridge` field said so before ADR 0033 was written. ⚠️ **Consequence 3's 4,6 % residual is not this ticket's** — *The posted floor is a seed-shape estimate* owns it. |
| `docs/adr/0028-…` | **no claimant — 61 declared it on resolution, unclaimed at the time.** ✅ **The void is bounded by the Envelope, not by the parts.** Enclosure is dropped as the test: a void is every complement component **other than the `notches_used` notch spans**. Nothing else in the ADR moves — charged to the donor's own receiving Room, weighted, carried as `voids`, **no donor refused**. Population **15,49 % → ~40 %** of donors, p50 still one component, cost still one `AddMultiplicationEquality` each — the arm 57 priced at **zero**. ⚠️ **§4's ownership purity (p50 1,00, ≥ 0,80 on 72,7 %) is measured on the enclosed population only**; the widened one borders fewer Rooms, so the fallback should fire *less* — a direction, not a measurement, and the emitting pass is where to check it |
| `experiments/rectangularise/`, `docs/research/rectangularisation.md` | **no claimant — 46 closed, and 58, 47, 27 and 40 before it.** ✅ **46 wrote §15 and left three probes** — `off_frame_gate.py`, `frame_choice.py`, `frame_residual.py` — plus `frame_of` / `frame_residual_of` in the second as the **reference implementation of a frame change that is specified and deliberately NOT applied**. ⚠️ **Three traps, in the README**: never quote a `void_census.py` band beside a §15 one (400 dwellings against 2,317, and the small one's last bands hold six and five — that is where the 0,167 came from); `off_frame_max` and `frame_residual` are different quantities and a cut on one does not transfer; and the modal frame **looks** like a coin flip (377/357) and is not (gains mean 0,923°, regressions 0,057°). ⚠️ **58's §14 handoff is untouched and still open**: the exact minimum rectangle partition of a real outline is **6** at the median against ADR 0003's 1–4, only **12,4 %** of the index is ≤ 3, and **39,3 %** needs more rectangles than it has rooms. 46 did not take it — it is an Envelope question, not a conversion one |
...
| `experiments/solver-toy/geometry.py` (`EXPOSURE_PRESETS`), `experiments/envelope-exposure/` | **no claimant — 49 and 52 both closed.** ✅ **52 discharged both handoffs 49 left here, in place.** `exterior_fraction`'s double-count is fixed in `geometry.py` by walking the real boundary, and this directory's `true_fraction.py` is **kept as the independent shapely check** it agrees with — 45 (count, preset) pairs, **0 mismatches**. ⚠️ **49's "that half is harmless" was wrong in one place nobody looked**: the phantom faces reached `frontage.py`'s H8 budget, so every arithmetic-death table on this map was computed through a numerator up to **32 %** too large — at twelve rooms `detached` read 68 000 mm of exterior run against a true 46 000. Re-checked at every cell: **zero verdicts change**. ⚠️ **`EXPOSURE_PRESETS` stays a constant four-vector**: 52 refused making it `n`-dependent, because the drift is `envelope_for`'s and an `n`-dependent preset would hide an Envelope defect inside a preset. Previously: **no claimant — 49 closed.** It re-fitted the presets on **exterior run per room** rather than on a fraction of perimeter, because a fraction does not transfer between dwellings whose perimeters differ and **H8 reads run**. `envelope-exposure/` now holds four probes and a committed 371 KB series (2,238 dwellings), so a later percentile costs seconds. ⚠️ **It also declared `docs/adr/0003-…`**, which the row below lists as **47's** — a collision this table did not record. 47 was unclaimed at the time; the amendment is a self-contained section at the foot of the file and touches nothing in the notch-cap material 47 will write. 48's outstanding §7 correction was **deliberately left for 47** rather than written blind |
...
| `experiments/solver-toy/`, `docs/research/solver-formulation.md`, `experiments/room-rectangles/`, `docs/research/room-rectangles.md` | **no claimant — 58, 52 and 43 all closed.** ✅ **43 wrote `solver-formulation.md` Part VI and deliberately wrote nothing to `experiments/solver-toy/`**, because it ran no solve: the ordering encoding was refused on the corpus before it was priced, so **no cost figure for an auxiliary integer exists** and none should be inferred. **The H-list is closed at H10** and the H-table now says so, so a ticket proposing an ordering constraint is re-opening a decision with a published corpus cost. ⚠️ **Part VI §VI.3 carries the fixture defect that would otherwise have died on a closed ticket**: `AREA_PER_ROOM_M2` = 9,65 against the 11,58 the placeholder table needs at 7–8 rooms in either arm, and no non-guillotine tiling at all below 7 — so the bottom half of C13's band is where this rig is least trustworthy and also the half nothing has measured. ✅ **58 wrote `solver-formulation.md` Part V**, so a ticket touching the real-boundary ladder, the `no_brief` decomposition or the wall-time comparison is amending a settled shape rather than filling a gap. ⚠️ **It also leaves 43 a number that bears on its own question**: wall p50 is **10,11 s** on a real Envelope against **0,19 s** on the corpus fixture, and p90 sits at the 15 s cap — so an encoding that adds variables is being priced against headroom that only exists on the fixture. Previously: **52 closed.** Historical: ⚠️ **51 hands it a named, unproven lead for the six-room mystery**: `probe_exposure` returns 0/5 at six rooms with 5 250 mm of frontage slack and nothing has identified why — and `kitchen.needs_window` went **`true` after** those presets were fitted, so the kitchen is a frontage claimant the arithmetic there may not carry. Not a claim; the one unexamined candidate. ⚠️ **51 also leaves both holders of this directory a question its own decision turns on**: what `select_relations`' positive-cost filter actually posts. A landlocked donor is only *provably* infeasible if the separations enclosing that Room survive selection — measure it and ADR 0025's refused gate becomes arguable. ⚠️ It shares **both** `solver-toy/` and `solver-formulation.md` with **43**, so the concurrency rule binds hard: not at once. Created by 49, which measured three structural defects it could not fix because the fixes change what the solver is *given*, not what it is measured against |
...
- [The statutory floor now has a price, and it is not the one it was posted at](tickets/55-does-the-statutory-floor-stay-hard-now-that-it-has-a-price.md)
  — **it stays hard, at all five limbs, and the market is what settles it.** The
  trigger ticket 50 named fired inside a week and firing it is what *vindicates*
  the argument: 50 never said *hard until it costs something*, it said the cost of
  hard is recoverable and the cost of soft is not. `competitive-landscape.md` §5.2
  — *code compliance is claimed by six vendors and implemented by approximately
  zero* — makes a curated, first-hand, **geometrically enforced** statutory floor
  the one thing this engine has that the surveyed market does not, and **3,6 % of
  yield does not buy it back**. The kitchen was the limb worth arguing and
  **dropping it removes the floor rather than lowering it**: ergonomic
  `kitchen.min_area` is **1,8 m²** (a 900 × 2100 galley strip) against a statutory
  8,0, and the Swiss p50 of 8,04 is evidence the floor sits *where people build*,
  not evidence it is too high.
  ✅ **What moved is the shape.** `max(ergonomic, statutory)` stops being asserted
  inside a rule statement and becomes the term **Hard area floor** — so a limb
  amendment is a **value** edit, which is what the per-limb table was built for and
  the composed sentence had been defeating.
  ✅ **`brief.md` gains bound 9** — a *stated* per-room target below its own hard
  floor, at `warn` — and it is **the first bound ADR 0015 declines rather than
  decides**: `model.no_unassigned_area` fixes Σ Space exactly and §9.3 targets are
  two-sided, so a kitchen stated at 6 m² **can** be delivered at 8 (`cross` p95
  is +3,30 m²). A `hard` bound there would refuse buildable Briefs. The ADR is
  followed *by* being declined.
  ✅ **§11 gains §11.1 and a stated hole.** A compliant Brief can starve with no
  field to edit, so the same-sentence guarantee does not reach it — said out loud
  rather than left silent, with a three-step escalation (deepen the pool → source B
  → a no-survivor sentence that names *no* Brief defect) that never touches the
  hard set.
  ⚠️ **3,6 % is pool-of-8 and an upper bound**, against production pools of
  **86.6** and **58.7**; and the correlation that would have defended reading it
  straight was the **level** error ticket 56 removed, unre-read since. Ticket 57.
  ⚠️ **Source B's per-room absolute area fidelity is unmeasured** — §11.1 step 2
  sends the starved Brief there and `proposer.md` §6.1 has no term for it.
  ⚠️ **Two shipped sentences were dead**: `brief.md` §9.2's *"`statutory_floor` is
  read by nothing in v1"* (every clause died with ticket 50) and §11's 58 m² worked
  example, reproducible from nothing — now quoting §3.1's measured per-otaq series,
  **26,5 / 37,5 / 47,5 / 57,5 m²**.
  ✅ **ADR 0027** generalises the one reason that was written nowhere else: *where a
  hard rule is what distinguishes this engine from what the market ships, its cost
  is a debt owed by the stage that produced the failure, never a reason to weaken
  the rule.* Scoped to this rule alone, and its **reversal trigger is not a yield
  number** — it is the transcription being wrong, or a competitor shipping a
  curated code bound to geometry rather than to an uploaded document.

- [A donor's enclosed void becomes area nobody asked for](tickets/53-a-donors-enclosed-void-becomes-area-nobody-asked-for.md)
  — **the void is charged to a Room and bounded in the warp, carried on the
  Proposal, and no donor is refused for having one** — ADR 0028, `proposer.md`
  §2.2.8. ⚠️ **The ticket's quantity was the wrong one**: `void_census.py`
  measures against the real dwelling and the engine only ever sees `parts[]`, so
  read on the parts frame over the whole index rather than the first 400 records
  in file order it is **6.73 %** at ≥ 0.5 m², not 10.0 — a room-count gradient
  (0.55 % at four rooms, **15.79 %** at ten) makes a file-order sample over-state
  by half. ⚠️ **And the premise inverted**: the warp does not carry the void, it
  **amplifies it 2.2×** (p50 0.50 → 0.81 m², p90 1.31 → **3.19**, growing in 62 %
  of cases), because `fit_warp.warp_model:146` prices worst-room deviation and the
  weighted sum and nothing else — the void is the objective's one unpriced region.
  ✅ **Not gated**: 11.74 % of the index after conversion-side absorption, 20.5 %
  at nine rooms, against 47's accepted 6.65 % and refused 17.2 % — and refusing a
  dwelling for a hole **ADR 0014's own two-rectangle cap** put there charges the
  corpus twice. **It is our residue, not the building's**: only **2.0 %** of the
  area is a dropped duct, and 96 % of components are below the 900 mm leg floor,
  so they are what `acceptance-bar.md` §9.1 already calls a **niche**. ✅ **The fix
  is one line and it is measured**: the void's area joins its receiving Room's
  `area_r` and carries a penalty — realised p90 **3.50 → 1.50 m²**, max 13.1 →
  8.1, **INFEASIBLE unchanged**, one `AddMultiplicationEquality` per component on
  15.5 % of candidates. ⚠️ **The biggest finding is in the deviation column and it
  reads backwards**: charged worst-room deviation is p50 **0.0959** against free's
  0.0652, and that is not a regression — it is the size of the understatement in
  every warp fidelity figure quoted on a voided candidate, because `free` measures
  a Room's parts and ignores the floor it is about to be handed. ✅ **A new
  Proposal field**, `voids: [(span, receiving_room)]`, on ADR 0014's test and
  against zoning's: the receiving Room is **not derivable** — largest shared edge
  agrees with the donor **28.4 %** and ties on 28.4 % of components, largest
  bordering Room 38.1 %, geometric absorption 24.1 % — and the solver cannot infer
  it, because H3 plus L1 corner displacement makes **every bordering Room's repair
  cost the same**, so an arbitrary 1.5 m² through `dim.max_area` is a tie broken by
  nothing. The receiving Room is the **donor's own**, recorded (watershed purity
  p50 **1.00**). ⚠️ **No published number moves and that is not reassurance** —
  `absolute_area.py` has no output for realised unassigned area at all, which is
  why this had to be measured outside the rig. ✅ **Both handoffs this file was
  owed are taken rather than passed a third time**: 56's §2.2.3 *"for free"*
  sentence is struck and the notch share is held, and 47's **`worst_room_iou`**
  lands as an index field, a hard gate at **0.30** (6.65 %, `conf: fitted`) and a
  rank above it rather than a 0.50 gate costing 17.2 %.

- [The toy Envelope is more compact than a real dwelling](tickets/52-the-toy-envelope-is-more-compact-than-a-real-dwelling.md) — **the fixture was handicapping the solver, and the correction is free.** ADR 0029, `solver-formulation.md` **Part IV**. `envelope_for` gains a second family fitted per count against 2,238 real dwellings on area, perimeter **and** bbox occupancy (within **0,7 %**, n = 5–11); the published family stays **bit-exact and default** because editing it in place moves the substrate under four closed decisions. 140 paired solves at the shipped config: survivors **68,6 % → 85,7 %**, **no slot** where published wins, exact McNemar **p = 0,0005**, and **time does not move** on an Envelope +12–26 % floor and +25–32 % perimeter. ⚠️ **The cause was never the notch share**: *every* notch `l_shape` and `u_shape` cut is a **corner** notch, which adds **no perimeter at all** — the toy's boundary is exactly `2(W+H)` at every count against a real dwelling's 6–12 % excess, so no share and no count could have fixed it. The fix is ADR 0003's **missing shape**, a mid-edge notch (`u_shape_true`), the U in rect/L/U/T the generator never emitted. ⚠️ **This ticket's own headline row was the noise cell** — n = 12 is **17 dwellings** at 34,6 % bbox excess against 11,8 % at eleven; in the well-sampled band the ratio is 0,91–0,97, not 0,68. ⚠️ **`AREA_PER_ROOM_M2` is kept at 9.65** and was never a number to correct — it is the published fixture's own constant; the honest per-count figures are 10,83–11,77. ⚠️ **The blast radius was wider than the ticket knew**: ADR 0019 / Part III ran half its grid at the stale `corpus_median` too. Both studies hold exposure **fixed within each pair**, so both McNemar results stand and their rates are **conservative** — re-worded, not re-run. ✅ **§II.2's arithmetic-death table is overturned**: no count in the band is dead. ✅ **The six-room mystery is diagnosed and it was never frontage** — `assign_kinds` starves for cells that are both exterior-facing and habitable-sized, median 3 against a fixed requirement of 4. ✅ **The bottom of C13's band goes from "below 7" to one cell**: n = 5 now solves 10/10, n = 6 is all that is left. ⚠️ **`clear_t` must equal the solver's `t_int_mm`** whenever `erode_minima` is on — getting it wrong returned OPTIMAL with 55 cells unassigned and **inverted this ticket's headline** from p = 0,0005 to p = 1,00.

- [Every Envelope the solver has seen is invented](tickets/58-every-envelope-the-solver-has-seen-is-invented.md) — **the real-boundary arm is run, and the solver is not what fails on one.** ADR 0030, `solver-formulation.md` **Part V**, `rectangularisation.md` **§14**. A three-rung ladder — `corpus` / `cap` / `real` — 360 paired solves at the shipped config. **Of the 57 real slots that reached the solver, 55 returned a candidate valid on every hard predicate other than exact tiling** — 2 tiled exactly and survived, **53 failed on left-over floor alone** — and only **2** went INFEASIBLE; the residue is a median **1,06 m²**, ADR 0028's enclosed void at plan scale. ⚠️ **The arms diverge before the solve** — `no_brief` is **14 / 70 / 63** — so the survivor columns (106 / 48 / **2** of 120) are not a solver comparison. ⚠️ **The widest-reach number is the middle rung**: `cap` is ADR 0003's own object sampled **per dwelling** instead of per count, and it loses **70 of 120** slots in `ground_truth` where `corpus` loses 14 — **per-dwelling sampling alone costs more than half the survivors**, and every fixture on this map is a per-count median, which is dissectible in a way its own population is not. ⚠️ **A real outline is not an Envelope by a wide margin**: exact minimum partition (Lipski/Ohtsuki, verified on eight shapes) is **6 rectangles** at the median, 12 at p90, 44 max, against **2** for the published fixture and **4** for ADR 0029's — which sits at the corpus **p25**. Only **12,4 %** of the index is ≤ 3, **39,3 %** needs more rectangles than it has rooms, and **1,4 %** has every part wide enough for a habitable room. ⚠️ **The pre-registered decision rule fired and was the wrong rule** — it reopens the 15 s budget at 104–0, p = 0,0000, and the loss is not in the solve. Recorded, not reinterpreted. ⚠️ **The budget is not refuted but its headroom was the fixture's**: wall p50 **0,19 → 3,30 → 10,11 s**, p90 at the cap on both real-sampled arms, while no survivor waited over 1,68 s to become valid. ✅ **Item 4 is answered *no* on evidence**: `EXPOSURE_PRESETS` stays — a third of a real boundary is off its own bbox, but the notch branch recovers the typing (`exterior_fraction` 0,766 real against 0,786 cap). ✅ **The six-room hole belongs to `envelope_for(6)`, not to six rooms** — `cap` survives **4/14** there where `corpus` gives 0/14. ⚠️ **`assign_kinds` refuses a real dwelling on the PROGRAMME, not on size or daylight**: `COMPOSITION` wants a median 5 habitable Rooms against a median 4 cells both exterior-facing and habitable-sized — short in **50,0 %** of dwellings at `corpus_median`. That is the **toy's** `composition`/`STANDARDS`, **not** `room-constraints.json`, and must not be quoted as the shipped table's cost. ⚠️ **A converted dwelling is not a witness for its own boundary** — its rectangles are fitted to the cap Envelope, a superset, so against the true outline they fail H1 *and* H3 (seven of the first eight slots); it reads as a coordinate bug and is not one. The true-mask re-fit is materially harder: **11 of 71** dwellings (15,5 %) cannot be re-fitted to their own boundary at the same 10 s budget. `fit_rects.py` is **not edited** — the domain is substituted at its call boundary.

- [The dwelling that is built on two angles](tickets/46-the-dwelling-that-is-built-on-two-angles.md) — **the gate was already refusing 40 % of them, unlabelled, and the rank was already demoting the rest.** ADR 0031, `rectangularisation.md` **§15**. The population is **4,79 %** of the index at ≥ 10° off frame, not the ticket's ~3 % — but `worst_room_iou ≥ 0,30` takes **39,6 %** of it and **28,6 % of everything that gate removes is off-frame**, so the residue actually decided on is **67 dwellings, 2,89 %**. Kept and labelled: **`frame_residual`** — area-weighted mean deviation from the dwelling axis, degrees, **no threshold inside it** — is a twelfth index field and a **fifth** obligation on the same `fit_rects.py` pass. It earns its place because it is not derivable from the record: at fixed `worst_room_iou` an off-frame donor still loses **5–11 cell-agreement points**. **It gates and ranks nothing anyway**, which is the surprising half: the decline is smooth with **no knee**, so a partition would be the fitted constant §2.2.4 exists to refuse, and the existing pre-rank already puts a 4–8° donor at the **10,6th percentile** of a 58–87 pool that draws 8. *A gate is for a candidate that is wrong; a rank is for one that is worse.* **The frame itself changes** to the area-weighted modal angle — counted a coin flip (377/357), weighed not close (gains mean **0,923°**, regressions mean **0,057°**, net **+327,4°**) — and **only riding the pass §2.2.1 already mandates**, never alone, because it re-bases `swiss_fit_k2.json`. Three consumers, **two** answers: retrieval and training keep, source B's **evaluation baseline excludes** — the one place a hard cut costs nothing and buys truth. Re-framing per wing refused **on scope and unpriced**: two frames meeting at an angle is unexpressible in ADR 0003's ring, which ADR 0030 just measured at 6 rectangles against 1–4. ⚠️ **ADR 0017's 0,167 is a six-dwelling median** — the true figure is **0,397** — struck in all three places it was quoted. ⚠️ **The open gap is on the warp, not here**: whether a sheared donor yields a worse *Plan* is unmeasured, because 57 holds `experiments/warp/`.

- [What best-of-pool is worth at production pool depth](tickets/57-what-best-of-pool-is-worth-at-production-depth.md) — **deepening the pool buys about one point and §11.1 step 1 is not the answer it was posted as.** `proposer-architecture.md` §7.6/§7.7, `experiments/warp/{pool_depth,best_of_m,best_of_m_fit,constrained_warp}.py`. On the published statistic's own convention an **eightfold** deepening moves starvation **4,1 % → 3,1 %**, flat by m ≈ 12, and at **7–10 rooms it moves nothing** (8.4 % from m = 6 to 64). Under it sits a floor no depth reaches — **π = 2,8 %** [0,3–5,6], **5,3 %** [0,0–11,2] at 7–10. ⚠️ **The correlation did NOT collapse when 56 removed the level error**: conditional decline climbs 35.2 → 50.7 → 73.5 → 88.9 % by the seventh candidate, so steps 2 and 3 are load-bearing and **source B's unmeasured area fidelity is now the urgent gap**. ⚠️ **`gate_pool` is not the shipped gate** — it returns the whole multiset bucket, p50 **81/37** against §2.2.1's **9/5**, so §2.2.7's *"a pool of 87 is a pool of 8 here"* is right about the gate and wrong about the rig that produced the number it annotates (ticket 60). ⚠️ **A plain Beta mixture was fitted and rejected** — every Beta sends `E[p^m]` to zero and returned 0,45 % against a measured 8,2 %; the model needs a point mass at `p = 1`. ✅ **Both obligations discharged**: `absolute_area.py` now records the realised hole (`bbox = Σ Space + erosion + notch + void`), and the genuinely constrained warp costs **2,6 %** of candidates — **all of it the notch**, ADR 0028's void charge costs **zero** — rising to **8,8 %** if the invariant is held exactly. ✅ **§7.5's pre-56 numbers are corrected** and its finding 2 headline struck: *"two fifths of the damage is one fixable constant"* was measuring the two rig defects 56 found.

- [The rig gate is not the shipped gate](tickets/60-the-rig-gate-is-not-the-shipped-gate.md) — **the rig is repaired, and the gate's two dimensional terms are not inert after all.** `proposer.md` §2.2.1/§2.2.7, `proposer-architecture.md` §7.5/§7.6, `experiments/warp/{gate_effect,gate_sites}.py`. `gate_pool` diverged **three** ways — its primary branch skipped terms 2 and 3 (**82,4 %** of the pool it handed the warp is floor the gate refuses), its fallback skipped term **1**, serving retrieval on **3,0 %** of Briefs off a *different room programme*, and its blank rate was **0,5 %** against the gate's **~13,4 %**. Split into `admissible_pool` (the gate, the default) and `bucket_pool` (kept, named, and only where **depth** is the quantity). ✅ **The area and aspect terms buy 8,6 points of decline** — 27,6 % against **36,2 %**, paired *within* one Brief, sign test **p = 0,0001**, worst-room deviation 68 % worse at p50, and a **dose** rather than a threshold (28,3 → 55,2 % on aspect). They never enter the warp's arithmetic: the mechanism is the **cut-line frame** they stretch, so the gate is a cheap proxy for stretch and §2.2's *"stretched 40 % in proportion"* reason was aimed at the wrong quantity. ⚠️ **At Brief level it is a wash (p = 0,74)** — member quality is a *per-candidate* property, which is why the rig's error lands on some published numbers and not others. ✅ **`ring` — the row this map says to read — improves 7,8 points**: plans losing a Room **24,9 % → 17,1 %**, Rooms under a floor 10,1 → **6,2 %**, Σ Space unmoved at +0,5 % so **56's level result stands**. ⚠️ **And it costs reach, the other way**: `ringpool` starvation **3,6 % → 4,4 %** and `briefs_with_no_usable_candidate` **105 of 600** — about **one Brief in six** reaches source B rather than a warp. The two errors sit on different statistics and do not cancel. ✅ **ADR 0018 survives** a *third* pairing defect found in `fit_warp.py` (**22,5 %** of its pairs gate-refused): worst-room p50 0,111 → **0,095**, **p90 unchanged**, relation theorem unmoved at zero. ⚠️ **One handoff it could not take** — ADR 0020's `covered ÷ interior` **0,9833** is a bucket number, gated **0,9853**, so the notch guarantee is ~**1,3 %** of `interior` rather than 1,5 %; that file is 61's

- [Can a starved candidate be refused before the solve](tickets/59-can-a-starved-candidate-be-refused-before-the-solve.md) — **no Proposal-level screen, and the warp finally reaches the projection solve.** `proposer.md` §2.2.9, `acceptance-bar.md` §11.1, `experiments/warp/project_join.py`. 291 candidates, 273 reaching the solve, 61 Briefs, plus a 34-candidate k ≤ 2 check arm. ✅ **The question was mis-posed and the rig says so rather than the argument**: `dim.statutory_min_area` is `site: both`, so the projection **posts** it and a returned Plan is never starved — starvation appears only as INFEASIBLE, and `served_but_starved` is **0** on every run. ✅ **The screen is refused on three independent grounds.** It is **unsound at 82,0 %** — 41 of 50 Proposal-starved candidates are served [71,4–92,6], 91,7 % at 4–6 rooms and 73,1 % at 7–10, and **88,9 %** on the k ≤ 2 arm whose area binding is *stricter*, so the k = 1 restriction is not manufacturing it. Its only **sound** form is arithmetic that cannot fire — Σ hard floors against the candidate's own derived box is p50 **0,566**, **max 0,736**, zero firings in 273. And it would sit on the **wrong side of the expensive step**: the projection costs **less than the warp that feeds it**, wall p50 **0,145 s** against warp p50 0,674. ✅ **The mechanism is `acceptance-bar.md` §3.2's own sentence, measured**: the solve does not create floor, it moves it — Σ Space p50 **0,0000** — shrinking a Room above its floor to feed the one below, and the median starved candidate has exactly **one** Room short. ⚠️ **So a per-candidate starvation figure on this map is not plan yield**; §2.2.7's decline rates are *warp* fidelity and four fifths of them dissolve in the solve. ✅ **§11.1 step 1 is priced and the ticket's own arithmetic was off by an order of magnitude**: one extra pool member costs a **mean 2,17 s**, not 10,9 — the 10,11 s it quoted is 58's **real-boundary** arm, which no candidate ever presents — so `POOL_DEPTH_ON_STARVATION = 16` past 57's knee, on the ~3 % of Briefs that reach the step. ✅ **The bigger change is the ordering: §11.1 now declares starvation on the Plan, never on the Proposal**, and that alone takes Brief-level starvation **3,28 % → 1,64 %**. ⚠️ **That halving is 61 Briefs and 2 starved cases** — a direction, not a number; the Plan-level twin of §7.6's best-of-*m* curve is what this hands on. ⚠️ **The projection reads a perimeter Room 3,9 % smaller than the bar does** — `solver.py` erodes all four sides where ADR 0001 does not erode at the Envelope boundary, and on a 250 mm grid the 75 mm ring is unrepresentable, so it is **structural**: **1,51 %** of 1 786 warped Rooms clear their floor on the bar's plane and fail on the solver's, and **every false pass is inside that population**. Ticket 56's ring, where it cannot be removed. ⚠️ **`solver-toy/solver.py` has no ADR 0014** — one rectangle per Room — and `room-rectangles/solver_parts.py` binds area per **part** where ADR 0014 binds it per **Room**; two shipped decisions, one rig, neither implemented cleanly. ⚠️ **The toy's H8/H9/H10 had to be posted soft**: only **54** of 273 warped candidates satisfy the toy's full hard set (H9 129, H10 104, H8 75) and **none of those counts is `room-constraints.json`'s cost** — 58's finding from the other side. ⚠️ **What it refused is the *filter*, not a third site** — posting the floor as a **constraint inside the warp** changes what the warp emits rather than what survives it, none of the three grounds transfers to it, and it is now [Should the warp post the statutory floor](tickets/64-should-the-warp-post-the-statutory-floor.md).

- [The notch is two components and a quarter of donors have more](tickets/61-the-notch-is-two-components-and-a-quarter-of-donors-have-more.md) — **the third component is not a third notch, it cannot be one, and `s` was measured on the wrong rectangle.** ADR 0020 second amendment, ADR 0028 amendment, `proposer.md` §2.2.1/§2.2.3/§2.2.8, an ADR 0003 note, `CONTEXT.md`. The Envelope is the bbox minus **at most two** notch rectangles — `notches_used` is 2 on **90,16 %** and never more, while `notches_needed` is 3+ on **37,6 %** — so everything past the second is **inside the ring**: **27,2 %** of donors, p50 **1,25 m²**, **89,7 %** perfectly rectangular, **99,7 %** at a corner or edge distinct from the first two. Residue that reached an edge, not building. **The cap is untouched** (0,30 % of donors carry three material components) and its downstream price — `envelope_loss` mean **3,72 %** of the domain, handed to whichever Room is cheapest — is a note on ADR 0003. **`s` is now the `notches_used` spans' share and nothing else**: read off the parts complement it ran **+0,0191** mean above the Envelope's own share, > 2 points on **38,2 %** of donors, which is ~**1,9 m² of invented notch** on a 90 m² dwelling in a typed, drawn, dimensioned, exported edge. **Widening `s` to all touching components — the ticket's own proposal — was measured and refused**: it moves a further half point away from the object it names. The floor goes to **ADR 0028's void, whose test moves from *enclosed* to *inside the Envelope***, at the population 57 priced at zero. ⚠️ **The two land together or neither lands**: the over-sized box was paying for the uncovered floor, and `s` alone takes Σ Space **+0,4 % → about −1,9 %**. ⚠️ **No new technology and no refactor** — the notch spans are a **sixth** field `proposer.md` §2.2 already promised and `fit_rects.py` never emitted, one computation with the widened `voids`, on the pass the conversion is already frozen for.

- [The gate measures stretch with two blunt scalars](tickets/63-the-gate-measures-stretch-with-two-blunt-scalars.md) — **the stretch has a closed form, it was inside the warp's own model, and it JOINS the two scalars rather than replacing them.** ADR 0032, `proposer.md` §2.2/§2.2.1/§2.2.4/§2.2.7, `experiments/warp/stretch_terms.py`. The gate is **four terms**: `req = max(W_req/W, H_req/H) ≤ 1`, an interval DP over the donor's own frame spans that returns the smallest box extent it admits at the ergonomic floor — microseconds, no solve, no new dependency, no new index field, and the cut is **1.0 because that is where the warp's hard constraint sits**, not because anything was fitted. **Sound**: `req > 1` violates a necessary condition of the model the warp solves, so it refuses only candidates the warp would have declined — **103 of 103**. Per-candidate decline **27.6 % → 22.9 %** with best-of-pool p50, p90 and served rate *identical*, because a dead member was never the best member; better than free, since `m` is a warp budget and a corpse wastes a draw (served **88.1 % → 89.4 %** at equal `m`). ⚠️ **Replacement is refused on measurement, against the market prior**: Graph2Plan filters on programme and ranks shape, but dropping the two scalars moves best-of-pool **p90 0.2303 → 0.2543** — the pair buys *proportion*, the bound buys *feasibility*, and they are orthogonal. ⚠️ **Two traps that decide how far to trust every number above.** `gate_effect`'s population is 50/50 and a real bucket is 82.4 % refused, so the probe's §4b/§4c figures are biased *toward* loosening — quote §4e, which reweights by each Brief's own stratum sizes. And the **`m = 8` block is confounded** (best-of-3 against best-of-6 by the rig's `--k`), which is why this is decided at `m = 3` while the shipped `m` is 8 — raised as *What the fourth gate term is worth at the shipped pool depth*. ⚠️ **The ticket's own first candidate was pruned, not measured**: the per-axis frame-extent ratio is a bijection with the two scalars up to the void share, agreeing with them on **89.4 %** of candidates — the incumbent in polar coordinates. `req ≤ 0.7` dominates on four axes on the *biased* population and is refused for all three reasons at once

- [What an ordered entry sequence costs the solver](tickets/43-what-an-ordered-entry-sequence-costs-the-solver.md) — **no new solver machinery is owed, the H-list closes at H10, and the property collapses a *third* way.** `zoning.md` **§6** and **D10**, `solver-formulation.md` **Part VI**, three probes in `experiments/zoning/` — **no corpus pass and no solve**: item 3 was taken first, as the ticket directed, and it made item 2 moot, so **no cost figure exists and none should be quoted**. ⚠️ **The ticket's own first two candidates are the negation of the slogan they encode** — the nearest social Room sits at hop **1** in **73,4 %** of dwellings, so *entry → hall → living* **means** living at hop 1 and *no otaq at hop 1* forbids it (it holds on **9,6 %**; the habitable reading **1,8 %**). ✅ **The first hop is a construction, not a predicate**: `openings.md` §7 hosts the entrance on the invented `hall`, so the engine is at **100 %** against the corpus's 93,2 % and R3 restates it. ⚠️ **The only reading needing the integers is a tie half the time** — strict **31,6 %**, **tie 51,0 %**, inverted **17,4 %** (1 in 5,8), against the ticket's own bar of one in six. ✅ **And the judgement the fog patch deferred is measured false**: entry-depth inversion is **negatively** associated with `proposer.md` §6.1's social transit — χ² **34,55**, p ≈ **4,2 × 10⁻⁹**, OR **0,354**, 38 in the both-cell against 78,9 expected — and **15,2 %** of dwellings invert with no transit defect at all, so the three shipped terms do **not** capture *reads as designed*. The gradient was therefore **unowned**, and is raised as [What the entry-depth gradient is worth as a fifth evaluation term](tickets/66-what-the-entry-depth-gradient-is-worth-as-a-fifth-evaluation-term.md). ⚠️ **Quote 17,4 % for a rule's cost and 16,1 % for the gradient's shape** — min-based against mean-based, `zoning.md` §2.2 amended in place. ⚠️ **D7's verdict stands and its stated reason does not**: *cost unmeasured* was never reached. ✅ **15 s, τ = 4 and Part II's percentiles are untouched**, so 62, 64 and 65 may keep quoting them. ⚠️ **`solver-toy`'s `AREA_PER_ROOM_M2` = 9,65 is below what the table needs at 7–8 rooms in either arm** (both need 11,58) — carried into Part VI §VI.3 rather than dying on a closed ticket, and it bounds any future pricing work to 8+ rooms or a fixed fixture

- [Should the warp post the statutory floor](tickets/64-should-the-warp-post-the-statutory-floor.md) — **yes, hard, per Room, on the bar plane, single pass — and it is ADR 0027's debt being paid by the stage that ADR named.** ADR 0033, `experiments/warp/floor_warp.py`. The warp `proposer.md` §2.2.2 specifies emits **31,6 %** of candidates carrying a Room below the law (median miss **1,356 m²**, tail to **8,444** — a bedroom at 1,6 m² against a floor of 10); posted, **4,6 %**, and the residual is grid dust from a seed-shape estimate. Costs 8,66 % of candidates paired, worst-room deviation p50 **+0,0000**, and it is **linear** — `area_r` already exists, so unlike ADR 0028's void it adds no product at all. ⚠️ **The candidate rate is the wrong denominator and §11.1 says so**: at Brief level (m = 8, 199 Briefs) it costs **1,51** points of service and buys **4,02** points of *legal* service, and the share of served Briefs holding a floor-clean candidate goes **94,27 % → 100 %**. ⚠️ **A two-pass fallback was measured and REFUSED** though it loses zero candidates and halves violations — every second-pass candidate violates by construction, so it buys a rate and no invariant, and the Briefs it recovers are recovered by serving a plan below the law. ⚠️ **The session's own opening recommendation was two-pass and the Brief-level arm reversed it**; the pre-committed second-pass threshold failed and is recorded as mis-specified rather than dropped. Raised three: *The posted floor is a seed-shape estimate*, *The projection discards a fifth of the guarantees the warp now buys*, *The law is a hand copy and it now shapes rooms*.

- [A zone floor is posted on the whole room](tickets/70-a-zone-floor-is-posted-on-the-whole-room.md) — **kept at 6,0 and reclassified: a profile cell now declares what it measures, and a part may floor but never target.** ADR 0034, `room-constraints.json` `mapping.referent_model`. ⚠️ **The ticket's own framing was half wrong and correcting it inverted option 1**: every legal kitchen-diner holds a ≥6 m² kitchen zone, so 6,0 on the room refuses a *strict subset* of what the law refuses — **zero false refusals**, and emptying the limb would have dropped the room to the ergonomic **4,6**, below even the zone figure. ✅ **Two research tickets were fired and both changed the answer.** No AZ instrument publishes a whole-room figure — swept AzDTN 2.7-1 … 2.7-11, and there is no dining-zone figure either. But AzDTN's ladder is about *eating* (`taxça-mətbəx` 5 no eating / `mətbəx` 8 unzoned / diner **zone** 6), so the 6 relaxes the *cooking part* and **the kitchen-diner is the larger room with the smaller kitchen part** — posting 6,0 on it **inverts the norm's own ordering**, flooring the zoned room 2,0 m² below its unzoned equivalent. That inversion is left in the hard set **deliberately** and carried by the *target*: all three candidate floors (8,0 taxonomic / 7,9 ergonomic / 12,0 Kazakh) are `engine_choice`, and the statutory tier is 13-for-13 `verified` — which *is* ADR 0027's market argument. ⚠️ **8,0 was not refused for costing yield; it costs none.** ✅ `living_dining_kitchen` gains the map's first **composed** floor — 15,0/16,0 + 6,0 = **21,0 / 22,0**, disjoint by «ayrıca zonaları» — and `living_dining` is marked `undetermined`, **fail-open, changing nothing**. ⚠️ **A figure this map nearly shipped is disqualified**: the Swiss `KITCHEN_DINING` median **23,67** is a *dining room* — 39 of its 41 rooms sit in dwellings that also carry a separate `KITCHEN` — and it is 21 layouts, not 41 rooms. The target is **18,8**, corroborated by MİDA at 17,37. ⚠️ **MİDA publishes per-room areas after all** — 318 Baku geometries — so `what_could_not_be_obtained.market_practice` is false and `bathroom_combined` 3,8 sits **above** practice; raised as 73. Also: a mandatory **gas rule** explains why no AZ corpus holds an open-plan LDK, recorded not enforced; six `clear_widths_mm` citations corrected; 71 and 72 raised.

- [The market tier has an Azerbaijani source now](tickets/73-the-market-tier-has-an-azerbaijani-source-now.md) — **the tier is re-fitted to Baku, four cells move up, and the one cell the ticket led with is refused.** ADR 0035, `room-constraints.json` `market_evidence` + `sources.az_mida_2026`. `living_room_2plus` 16,0 → **17,6**, `bedroom_double` 12,0 → **13,2**, `bedroom_single` 9,0 → **11,5**, and `wc` gains its first cell at **2,1** — which retires a **Swiss** rung-2 fallback of 1,85. ⚠️ **The ticket's framing was wrong and the correction is the whole decision.** It led with `bathroom_combined` 3,8 against 63,5 % of Baku bathrooms being smaller; that cell is **REFUSED** and does not move. The defect worth fixing is **at dwelling level and invisible per cell**: five of six cells sat within 10 % of MİDA practice while the **summed** targets ran **11 % under** what Baku builds (3 otaq 51,6 against a 58,3 m² room budget), because ADR 0023 made `dim.market_default_area` two-sided and a recommended *minimum* was being used as a *centre*. After: **57,2**, −2 %. ✅ **The rule that governs every future market reading is the durable part** — MİDA is the subsidised state fund, so its sample is biased **low**: MİDA *above* a target is evidence against the bias and is strong, MİDA *below* is what the bias predicts and is weak, therefore **MİDA may raise a cell and may never lower one**. C14's shape on a soft target for a different reason — legal force there, sample bias here. ⚠️ **A `bathroom` row in the research note is a mis-match and is now flagged at source**: MİDA's eight-name vocabulary has no separate `hamam otağı`, so there is **no MİDA evidence for a bath-only room** and 3,51 is `bathroom_combined`'s figure. ✅ **Raising a target raises a HARD cap, and that is now settled without amending C14** — [Nineteen room types and nine area-band classes](tickets/71-nineteen-room-types-and-nine-area-band-classes.md), ADR 0038. `dim.max_area` has **two limbs of different character**: `k × target_area` is a **proportionality band around a requested size** and moves with the request by design, identically whether the request came from a Homeowner (C4, sovereign) or from the profile's measured practice (ADR 0035, may only move up); `absolute_cap` is the **region-free outer bound and no profile touches it**, which is where C14's *no profile may weaken a predicate* lives. C14's guarantee never attached to a band around a number the Brief itself supplies. ⚠️ **What makes that safe rather than merely stated is a build-time gate owed to `gate_check.py`** — `market_default ≤ absolute_cap` of the resolved class — **run** over the 11 profile cells and all passing, tightest `kitchen` at 9,0 / 20,59 = **2,29×**. `dim.stated_target_implausible` is widened from a *stated* target to **any** resolved one, because the check is about the number and not its provenance; `rule_count` stays **43**. The derived 318-plan schedule is **committed** (raw harvest is not; its md5 is the guard), and `verify_shipped_cells.py` reproduces all four constants from it — it caught a live defect mid-flight, `bedroom_single` rounded down while `wc` rounded up, fixed by publishing the rounding rule rather than the value. 238 `gate_check` and 233 `ergonomic_check` gates pass. Raised **74** (the gas hob) and **75** (C13's Azerbaijani evidence).

- [A gas hob decides whether the open-plan type is buildable in AZ](tickets/74-a-gas-hob-decides-whether-the-open-plan-type-is-buildable.md) — **the type ships, disclosed and never enforced — and its hard floor turned out to be unlicensed and was refusing every real Baku room of its own kind.** ADR 0036, `brief.md` §7.1, `annotation.md` general note 9. AzDTN 2.13-1 cl. 8.31 puts the gas hob in a `mətbəx otağı` and cl. 4.7 files a kitchen-**diner** inside that word, so `kitchen_dining` is compliant and `living_dining_kitchen` needs an electric hob. **Refusing the type was refused**: it would refuse the one open-plan dwelling the state housing fund builds, and enforcing a fuel rule is the C8 breach outright. ⚠️ **The evidence everyone was quoting was misread, including by ADR 0034.** *"MİDA publishes zero multi-room open-plan plans in 318 geometries"* is withdrawn: MİDA builds **5** of them (1,57 %) and **all five carry a separate bedroom** — one-bedroom flats with an open-plan kitchen-living room, LDK p50 **17,37 m²**, `nrooms: 0` being a sales label. The narrow fact — zero with a *separate `Qonaq otağı`* — is what survives, so practice **agrees** with the norm at 2+ otaq and **departs** from it at one bedroom, and neither refusal nor enforcement fits. ⚠️ **ADR 0034 decision 4 states its licence and breaks it in the same decision**: an entailed sum needs *"the norm's own type definition"*, and AzDTN defines **no open-plan type** — so neither the `compose_with` nor the underlying `part` read was ever licensed, and the 21,0 / 22,0 it produced sat **above all five** real rooms, 3,3–5,9 m² clear, against a soundness rule reading *"never rejects a room the law admits"*. `az_area` → **`null`**, the eleventh silent key; the floor falls to the ergonomic **8,5**. ⚠️ **C14 is not weakened and this is the map's first withdrawal of a profile raise** — the base is the ergonomic minimum and this returns to it, because a raise posted on an ungranted read was never a valid exercise of C14. **Reuse the reasoning, not the precedent.** ✅ **`kitchen_dining` keeps its limb and gains a `gas_note` saying it is compliant** — cl. 4.7 plus `«ayrıca zonaları»` is exactly the licence, so same rule, opposite result, and the difference is whether the norm has a word for the room. ⚠️ **An Assumption could not carry the disclosure**: the set is `ResolvedBrief \ StatedBrief`, *computed*, and a **stated** type invents nothing — §6 closes the taxonomy at three kinds and a fourth was refused. ⚠️ **A third defect fell out**: `floors_for` has **no open-plan limb**, so the warp posted `None` where the bar demanded 22,0 — **ADR 0033's invariant did not cover this type**, and none of ADR 0034's four owed gates checks the licence anyway. A **fifth gate** goes to 69 with it. ⚠️ **Rung 2 for this type is Swiss and runs 2,1× the only AZ measurement that exists** (36,5 against 17,37) — recorded on the cell, not fixed, because the market tier is ADR 0035's and n = 5 is thin. `gate_check.py` 238 → **235**, all three the removed guard entries

- [The law is a hand copy and it now shapes rooms](tickets/69-the-law-is-a-hand-copy-and-it-now-shapes-rooms.md) — **the profile is READ rather than copied, and the ticket was raised on a risk that had already happened: three of the eight copies were wrong when it was taken.** ADR 0037, `profile_read.py`, `room-constraints.json` `ergonomic.corpus_label_map`. ⚠️ **`MARKET` sat four cells behind ADR 0035 one day after it landed**, `HABITABLE` omitted `DINING` — whose `counts_as_otaq` is true — so a living-plus-dining dwelling took the **15,0** one-otaq floor where the guard says **16,0**, and `MIN_SIDE` had no `KITCHEN_DINING` row. ⚠️ **Item 1's shape was impossible**: `floors_for` reimplemented the `when_otaq_count` guard rather than copying it, and the lookup had **no key** — the copies are keyed by *corpus label*, the profile by *ergonomic key*, and the bridge existed only as end-of-line comments in four Python dicts. **Ticket 31's own defect one vocabulary upstream.** ✅ Third vocabulary published, one accessor, `data/` a hard dependency with no fallback, `_check_floor_transcription` **deleted** as structurally blind to omission. ✅ **ADR 0034's four owed gates land and ADR 0036's fifth makes the licence a FIELD** — a gate over `compose_with` alone would pass **vacuously forever**, since ADR 0036 emptied it on every row. `gate_check.py` 235 → **384**, and **15/15 data mutations were caught** plus a re-copied literal, which is the evidence rather than the count. ⚠️ **The handoff's *`experiments/warp/` is undisturbed* is FALSE**: `STAT_FLOOR`'s values held but the floor a dwelling *gets* moved on **59** dwellings, and the `MARKET` re-read moves **43,24 %** of rooms by a mean **+0,561 m²** — so every published `market`-arm number was measured at the old tier. **Direction provable, magnitude not**: every cell moved up and no floor moved, so the starvation figures are conservative, not wrong. The re-run is **62/65/67's**. ⚠️ **And the re-run is not a task anyone has to remember**: `MARKET` is now computed at import, so any re-run of any market-arm rig already uses the new tier. What is owed is **not quoting the published figures**, which is why this warning sits on the row rather than in a ticket.

- [Nineteen room types and nine area-band classes](tickets/71-nineteen-room-types-and-nine-area-band-classes.md) — **a hard rule was unevaluable on nine of the nineteen Room types, and one of them is in every Plan.** ADR 0038, `room-constraints.json` `ergonomic.area_band_classes`. `dim.max_area` states `k[type]`, the table is `k[class]`, and the bridge was **absent** rather than `null` — so the rule silently did not fire on nine types, eight of them reachable and Brief-nameable. **`hall` is the one that matters**: `resolve` invents one wherever the Brief names none, so **100 % of Plans carried a Room with no upper bound** and 27 m² of hall on a 90 m² dwelling passed every rule in the file — §9.3's 40 m² WC through a different door. ✅ **The ticket's *borrow or earn* framing is refused for a third option**: **containment**, a type takes the class whose measured population *contains* it, which borrows nothing because `CORRIDOR` **is** the three circulation types merged. A target must be type-specific; a band is a dispersion statistic over a population, and that population is the merge. Sixteen of nineteen resolve with no invention. ✅ **The two compound types are now the best-evidenced rows, not the worst** — composed from the same donor sums that already produced their targets, `kitchen_dining` **49,77 / 2,64** and `living_dining_kitchen` **69,00 / 1,86**, with borrowing measured at −3 % to **+39 %** and biased lenient by a mechanism. ✅ **The C14 lever `az-market-default-against-practice.md` left unowned is closed without amending C14**: `k × target` is a band around a *request* and moves with it by design, `absolute_cap` is the region-free bound no profile touches, and a build-time headroom gate makes it safe rather than asserted — **run**, 11 cells, all passing, tightest `kitchen` at **2,29×**. **No ADR 0023 value moved and `rule_count` stays 43.** ⚠️ **One row is an analogy and says so** — `utility` takes `bathroom` on the plumbed-appliance mechanism, not `storeroom`; **MİDA's Baku schedules carry no utility room and no storeroom at all**. ⚠️ **ADR 0037 was under-scoped**: two more private copies in `experiments/acceptance-thresholds/`, keys disagreeing with the shipped table, and **`dim.max_area`'s published 3,11 % corpus cost was measured with `kitchen_dining` silently exempt** — number survives, provenance does not. ⚠️ **What it could not fix is ticket 76**: `k × target` is a CH ratio times an AZ number, runs 62 %–114 % of the class's own cap and **inverts**, `living` at 41,4 m² against `living_dining`'s 35,6.

- [The projection discards a fifth of the guarantees the warp now buys](tickets/68-the-projection-discards-a-fifth-of-the-guarantees-the-warp-now-buys.md) — **the solver reads the bar plane, and three of the ticket's own premises were the answer.** ADR 0039, `acceptance-bar.md` §11.1 rewritten, `solver-formulation.md` **Part VII**, `CONTEXT.md`'s **Space plane** retired to one quantity. ⚠️ **19,5 % was measured at the wrong site** — `project_join.planes()` runs no solver, the rule is `site: both`, so a short Room is *re-sized* and a refusal can only be INFEASIBLE: **14 of 273**, all fourteen the floor's by ablation, **5,1 %**. ⚠️ **There was no domain to dilate** — `space_m2` erodes `parts ∪ outside`, so the domain boundary already IS the exterior inner face; a dilated domain or a shifted `W_env + t_int ≡ 0 (mod 250)` lattice both solve a problem that does not exist. ⚠️ **The seam is seven `site: both` rules and it runs both ways** — five floors where the solver is stricter, and `dim.max_area`, a **cap**, where it is the lenient one and so does not bind where `model.no_unassigned_area` sends surplus. The encoding subtracts the band **per side** off `Envelope.all_faces()`, is affine in H4's existing product and spends **no second multiplication**; its literals are **biconditional** where H8's are forward-only. ⚠️ **Nothing is implemented or measured** — raised as *The bar plane is derived and the solver has never run it*, which carries ADR 0039's own fallback. Discharges ticket 70's `KITCHEN_DINING` limb handoff.

- [The bar plane is derived and the solver has never run it](tickets/77-the-bar-plane-is-derived-and-the-solver-has-never-run-it.md) — **the encoding is built, it is exact, and it fits — and three of ADR 0039's supporting claims were wrong.** ADR 0040, `solver-formulation.md` **Part VIII**, `experiments/plane-accounting/` new. ✅ **11 892 Rooms, worst `|identity − space_m2|` = 0,0 mm².** ✅ **The cost is MEASURABLE where II.1's arithmetic was not** — 2,36× variables, +16,4 % total solve time, 284 slower against 23 faster, only 6 of 35 inside their own six-seed spread — **and it fits**: time to first Plan 0,110 → 0,284 s against a 15 s cap, cap exhaustion 17 → **16**. Decision 6's fallback not selected. ✅ **1,30 %, and the plane is all of it** — 4 of 307 INFEASIBLE on the incumbent, all four the floor's by ablation, **0 of 307** on the bar plane; **5,1 % retired** from `acceptance-bar.md` §11.1, which was measured on an unposted warp *and* a pre-ADR-0037 population. ✅ **`dim.max_area` binds and is nearly free** — 10 Rooms of 1 993 over their band, **every one a BATHROOM**, worst 10,2 m² over; +6 constraints, 0 new INFEASIBLE; posted on the bar plane it leaves **0**, on the solver plane **2**. ⚠️ **`Envelope.all_faces()` is the wrong set** — it returns an enclosed void's faces and a Room's edge on a void erodes. ⚠️ **The corner residual is TWO-SIGNED and decision 5's *conservative on every floor* is withdrawn** — 5,47 % of Rooms read larger; the corner term **alone** makes it 36,43 %, 6,7× worse. It changes **0 verdicts** on 1 993 Rooms, so the drop stands and the reason changes. Raised **78** over the two-part Room, where the missing term is 0,165 m² rather than dust.

- [What the bar plane owes a two-part Room](tickets/78-what-the-bar-plane-owes-a-two-part-room.md) — **the join is one length and 13 variables, it fits, and the false refusals turned out to be the BINDING SITE rather than the plane.** ADR 0041, `solver-formulation.md` **Part IX**, `experiments/plane-accounting/parts_plane.py` new. ✅ **`[B](U) = Σ_p [B](p) + 37 500·J`** — `AddNoOverlap2D` already covers a Room's own part pair and two interior-disjoint rectangles meet in at most **one** maximal segment, so there are **no contact literals between a Room's own parts**: exactly **13 variables** per two-part Room against the boundary term's O(sides × faces). ✅ **It fits** — 1,82× variables, 1,51× constraints, **+13,2 %** total solve, first Plan 0,257 → 0,623 s against a 15 s cap; the join term is **1,5 points** of that. ✅ **5 of 284 INFEASIBLE on the incumbent and ALL FIVE are the binding site** — moving the area floor from the primary part to the Room, which §9.1 already specifies, rescues every one for **+2 variables**; the plane rescues **0** more, so `project_join` LIMIT 3's *"strictly stricter"* has a price: **1,76 %**. ✅ **One vertex rule replaces `corners − reflex`** — `w = I − nU·[nO ≥ 1]`, exact on **11 740 Rooms at 0,0 mm²** over L, T, Z and rectangle, with Part VIII's pair as its one-rectangle case; **the L's own reflex corner is not a third sign**, it is the same −5 625, and a flush join end is the case naive per-part counting gets **wrong** rather than misses. ✅ **`dim.max_area` binds at Room level too** — 10 of 1 961, **one of them a two-part Z**, and no longer all bathrooms; posted it leaves **0**, and it is the one delta in the run a sign test cannot separate from zero (p = 0,86). ⚠️ **The join term buys ONE verdict in 1 961** — `site: both` re-sizes rather than refuses, and a two-part Room's headroom over its floor is p50 **9,37 m²** against a join term of 0,300; only **1 of 345** has less. ⚠️ **The 15 s headroom at two parts is not one part's** — the incumbent already exhausts the cap on **90 of 284 (31,7 %)** against 17 of 307. ✅ **And the seed sweep separates the two costs**: over six seeds on 19 stratified candidates, `B − A` is **outside** the candidate's own spread on 15 of 19 (median +0,336 s, p = 0,00073) while **`B − Bn` is inside it on 18 of 19 (94,7 %)**, p = 0,167 — the join term is a systematic +8,4 ms across 284 paired candidates and **undetectable on any single one**. Raised **79** over the shape: two rectangles are an L only **55,2 %** of the time.

- [What the entry-depth gradient is worth as a fifth evaluation term](tickets/66-what-the-entry-depth-gradient-is-worth-as-a-fifth-evaluation-term.md) — **the term is taken, and its corpus rate is a CEILING rather than a target — which is a defect in §6.1's shape, not in D10's proposal.** ADR 0042, `proposer.md` §6.1 (four terms → **five**), `zoning.md` D10/§6.7/§6.8. The property survived its own strongest counter-test: the benign reading, that a shallow private Room is a deliberate front-office which Alexander places at **both** ends of his own gradient, **fails** — the shallow Room is a terminal cell in **4.9 %** of inversions against **26.5 %** when ordered, so the corpus's inversions are bedrooms off the hall. ⚠️ **Three of D10's own specifications were wrong**: matching the rate (§6.5 calls the same bucket a *violation*), the three-bucket statistic (confounded — tie mass runs **30.8 → 67.3 %** monotone in room count), and the fear that a second node flag was needed (it is not). ⚠️ **The scoring defect is §6.1-wide and only term 5's is settled** — term 4 is *fidelity* by §4.5's own ruling, term 3 a defect rate, term 2 open; **81** owns the audit and 66 does not block on it. ⚠️ **Four of the five terms are uncomputable today** because `is_sleeping` is not in the data; **80** lands it rather than handing it on a third time.

## Not yet specified

In scope, not yet sharp enough to ticket. Graduates as the frontier advances.

- **A sovereign target below the law, and which one wins.** ADR 0033 measured
  `moved_rooms = 0` — the warp's floor never fights a target, because under
  `dim.market_default_area` every target already sits at or above its floor. That
  holds *only while it holds*. `brief.md` §9.4 bounds 1 and 3 still read the
  ergonomic minimum alone rather than `max(ergonomic, statutory)` — ticket 50
  handed that over and could not take it — so a Brief may state a kitchen at 6 m²
  against a floor of 8 and pass parse. The warp will then deliver 8 and `brief.md`
  §6.1 calls the stated target **sovereign**. Nothing on this map says a stated
  target stops being sovereign at the point it becomes illegal, and three files
  would have to agree on the answer: `brief.md`'s bound, `proposer.md`'s weights,
  and whatever the Homeowner is told. ⚠️ Adjacent to *A request and a result in one
  typeface*, which is the same collision seen from the surface — a Homeowner
  reading their own request back as a promise — and the two may be one ticket once
  the bound is settled. Not sharp yet because the owed §9.4 edit decides half of it
  and `brief.md` is unclaimed.
...
<!-- The ordered entry sequence: CLEARED. Graduated to *What an ordered entry
     sequence costs the solver*, which is closed — no new solver machinery is
     owed and the H-list closes at H10 (`zoning.md` §6, `solver-formulation.md`
     Part VI). The residue left on this patch — whether the three cheap
     properties already shipped capture most of what *reads as designed* means —
     is **measured false**, not deferred again: entry-depth inversion is
     negatively associated with social transit (χ² 34,55, p ≈ 4,2 × 10⁻⁹) and
     15,2 % of dwellings invert with no transit defect at all. What survives is
     a decision, not fog, and it is ticket 66. -->
...
- **Structural and services reality** — load-bearing walls, plumbing stacks, risers. The
  ...
  ⚠️ **The services half now has a corpus number and it is small.** ADR 0028 separated
  the enclosed void from the dropped `SHAFT`/`VOID`/`LIGHTWELL`/`ELEVATOR`/`STAIRCASE`/
  `TECHNICAL_AREA` entities the conversion discards: **1.4 % of components and 2.0 % of
  the void area** is a real riser, about 0.3 m² on one dwelling in fifty. v1 has no object
  for it — no room type, and an enclosed pocket fails `circ.potential_reachability`
  regardless — so it is charged to a bordering Room like the rest and `proposer.md` §8
  says so. **That 2 % is what a riser object would buy**, and it is the honest size of
  the prize rather than an argument that the hook is unnecessary.
...
- **Whether the solve grid should be finer than 250 mm** — ⚠️ **load-bearing now, not the
  ...
  *Whether a Room may be more than one rectangle*, says the deletion is wider than
  ...
  ✅ **One of the grid’s charges is not the grid’s and has been removed** — ADR 0039. The 75 mm over-erosion at the Envelope boundary was read as a quantisation cost (*“unremovable at a 250 mm grid”*) and is arithmetic: the domain boundary already **is** the exterior inner face. It bites hardest exactly where this patch says the grid is charging — a 4–6-room dwelling is nearly all perimeter Rooms — so any future re-measurement of the deletion must run on the corrected plane or it re-prices the grid for a defect that is gone. ⚠️ It does **not** touch ADR 0007’s congruence, which is the mechanism the deletion was attributed to, so this narrows the patch rather than closing it.
...
- **Whether the harness needs a Brief generator that works on a real boundary.**
  ⚠️ **Raised by *Every Envelope the solver has seen is invented*, ADR 0030, and
  it is instrument work rather than v1 work — which is why it is here and not a
  ticket.** `scenarios.ground_truth` dissects `Envelope.parts` and gives every
  part at least one room, so it cannot pose a Brief on a real outline at all:
  **96 %** of real dwellings have a part no room fits, and a **per-dwelling**
  Envelope inside ADR 0003's own family already loses **70 of 120** slots. The
  replacement is a dissector for general rectilinear regions that searches for a
  partition whose parts can each hold a room — the single change that would make
  a real-boundary arm a clean measurement. **The trigger is a decision that needs
  one**: today ADR 0030 records the gap as a stated limit and no open decision
  waits on it. If one appears — a v1 claim about yield on real geometry, or a
  Proposer evaluated end-to-end — this graduates, and the cost estimate starts
  from the fact that the minimum partition (median **6** rectangles) is a floor
  and a room-sized partition may need more.

- **Whether the *shipped* room programme over-demands habitable rooms the way the
  toy's does.** ⚠️ Ticket 58 measured that `scenarios.COMPOSITION` wants a median
  **5** habitable Rooms where a real dwelling supplies a median **4** cells that
  are both exterior-facing and habitable-sized — short in **50,0 %** of dwellings
  at `corpus_median`. **That is the toy's placeholder and nothing else**, and it
  was deliberately not quoted against `room-constraints.json`. The open question
  is whether an analogue exists on the shipped side, and it is not yet sharp
  because `resolve` does not impose a mix — the Homeowner names the rooms, and
  C13 plus `brief.md` §9.4's bounds are what constrain them. It becomes a ticket
  the moment someone can state it as a predicate over a `ResolvedBrief` rather
  than as an analogy to the toy. Adjacent evidence already exists and should be
  read first: ADR 0023's hard registry rejects **84,41 %** of real Swiss
  dwellings, of which `win.habitable_has_window` is most of the cost.
