...
| Artifact | Claimed by |
|---|---|
| `CONTEXT.md` | **no claimant** — 31, 38 and 23 closed. 38 declared it: **Pre-image bound** and **Invented circulation** are new terms, **Acceptance bar** reads *one declaration, three consumers*. **23 declared it too**: **Warp** and **Relation provenance** are new, and **Warp budget**'s old reading — *"before its arrangement stops being a real home's"* — is marked **false**, because a monotone warp cannot damage an arrangement at any budget |
...
| `docs/spec/proposer.md` | **no claimant — 23 closed.** It rewrote §2.2 into seven subsections and corrected §1, §4.4 and §5.1, so a ticket touching those is amending a settled shape rather than filling a gap |
...
- **A Homeowner shown candidates whose outlines differ.** ADR 0018 makes the Envelope's
  ...
  whether it should present them at all. It touches `homeowner-surface.md`, `brief.md`'s
  Assumption surface and the DXF/IFC sheet, so it is not one ticket's question yet.
  Adjacent to the *stated one gets none* asymmetry already recorded under **Variant
  generation and ranking**, and probably resolves with it.
